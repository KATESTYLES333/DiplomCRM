...
if not exists (select * from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = 'MEETINGS' and COLUMN_NAME = 'EMAIL_REMINDER_TIME') begin -- then
	print 'alter table MEETINGS add EMAIL_REMINDER_TIME int null default(-1)';
	alter table MEETINGS add EMAIL_REMINDER_TIME int null default(-1);
end -- if;
GO
...
if not exists (select * from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = 'MEETINGS' and COLUMN_NAME = 'SMS_REMINDER_TIME') begin -- then
	print 'alter table MEETINGS add SMS_REMINDER_TIME int null default(-1)';
	alter table MEETINGS add SMS_REMINDER_TIME int null default(-1);
end -- if;
GO
