...
if not exists (select * from INFORMATION_SCHEMA.TABLES where TABLE_NAME = 'CONTACTS_USERS' and TABLE_TYPE = 'BASE TABLE')
  begin
	...
	create index IDX_CONTACTS_USERS_CONTACT_ID on dbo.CONTACTS_USERS (CONTACT_ID, DELETED, USER_ID   )
	create index IDX_CONTACTS_USERS_USER_ID    on dbo.CONTACTS_USERS (USER_ID   , DELETED, CONTACT_ID)
