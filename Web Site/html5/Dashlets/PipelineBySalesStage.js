										...
										$('#' + sLayoutPanel + '_divChartHTML5').bind('jqplotDataClick', function(ev, seriesIndex, pointIndex, data)
										{
											...
											if ( lstASSIGNED_USER_ID != null )
											{
												...
												else ( Crm.Config.enable_dynamic_assignment() )
												{
													...
													var lstASSIGNED_SET_LIST = document.getElementById(sActionsPanel + '_ctlEditView_ASSIGNED_SET_LIST');
													for ( var i = 0; i < lstASSIGNED_SET_LIST.length; i++ )
													{
														if( sASSIGNED_SET_LIST.length > 0 )
															sASSIGNED_SET_LIST += ',';
														sASSIGNED_SET_LIST += lstASSIGNED_SET_LIST[i].value;
													}
												}
											}
