...
{
	this.m_bFeatureOR     = true  ;  //OR keyword
	this.m_bFeatureAND    = true  ;  //AND keyword
	this.m_bFeatureNOT    = true  ;  //NOT keyword
	this.m_bFeatureMIMUS  = true  ;  //- symbol
	this.m_bFeaturePLUS   = true  ;  //+ symbol
	this.m_bFeatureSTAR   = true  ;  //* symbol
	this.m_bFeatureSINGLE = false ;  //Single letter filtering.  01/17/2005 Paul.  Don't do this. 
	this.m_bFeatureLESS   = true  ;  // < or > symbol
	this.m_bFeatureEXCL   = true  ;  //! symbol
	this.m_bFeatureEQUAL  = true  ;  //= sumbol
	this.m_bIsOracle      = false ;
	this.m_bIsDB2         = false ;
	this.m_bIsMySQL       = false ;
	this.m_bIsPostgreSQL  = false ;
	this.m_bIsSQLServer   = bIsSQLServer;
	this.m_sTermOR        = 'OR'  ;
	this.m_sTermAND       = 'AND' ;
	this.m_sTermNOT       = 'NOT' ;
	this.m_sInput         = ''    ;
...
SearchBuilder.prototype.Init = function(str)
{
	this.ParseInput(str);
}

SearchBuilder.prototype.IsEmptyString = function(str)
{
	if ( str == null || str == '' )
	{
		return true;
...
SearchBuilder.prototype.EscapeSql = function(str)
{
	// 06/14/2015 Paul.  We should be replacing all and not just the first item. 
	return str.replaceAll('\'', '\'\'');
}

SearchBuilder.prototype.EscapeLike = function(str)
{
	// 06/14/2015 Paul.  We should be replacing all and not just the first item. 
	var sEscaped = this.EscapeSql(str);
	sEscaped = sEscaped.replaceAll('\\', '\\\\');
	sEscaped = sEscaped.replaceAll('%' , '\\%');
	sEscaped = sEscaped.replaceAll('_' , '\\_');
	return sEscaped;
}

SearchBuilder.prototype.ParseInput = function(sInput)
{
	try
	{
	...
	{
		SplendidError.SystemAlert(e, 'ParseInput');
	}
}

// http://www.regular-expressions.info/reference.html
// http://www.java2s.com/Open-Source/Javascript/Library/jshaskell/base/src/GHC/Unicode.js.htm
SearchBuilder.prototype.isAlphaNum = function(c)
{
	return /^\w$/.test(c);
}

SearchBuilder.prototype.BuildQuery = function(sCondition, sField, sInput)
{
	var sbSqlQuery  = '';
	var nNotFlag    = 0;
	var bOrFlag     = false;
...
SearchBuilder.prototype.ApplyFilter = function(sField, sInput)
{
	var nSuccess    = 0;
	var nNotFlag    = 0;
	var bOrFlag     = false;
