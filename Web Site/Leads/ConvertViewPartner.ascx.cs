...
{
	/// <summary>
	///		Summary description for ConvertViewPartner.
	/// </summary>
	public class ConvertViewPartner : InlineEditControl
	{
		protected Guid                   gpartner_ID                          ;
		...
		protected CheckBox               chkCreatePartner                     ;
		protected HtmlInputHidden        txtSELECT_partner_ID                 ;
		protected RequiredFieldValidatorForHiddenInputs reqSELECT_partner_ID  ;
		protected TextBox                txtSELECT_partner_NAME               ;
		...
		{
			get { return gpartner_ID; }
			set { gpartner_ID = value; }
		}
		...
		{
			if ( chkCreatePartner.Checked )
			{
				this.ValidateEditViewFields           ("Leads.ConvertViewPartner");
				this.ApplyEditViewValidationEventRules("Leads.ConvertViewPartner");
			...
			{
				reqSELECT_partner_ID.Enabled = !chkCreatePartner.Checked && (Sql.ToBoolean(Page.Items["chkCreateRequest.Checked"]) && sBusinessMode == "B2B");
				reqSELECT_partner_ID.Validate();
			}
		}
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			if ( chkCreatePartner.Checked )
			{
				...
				this.ApplyEditViewPreSaveEventRules("Leads.ConvertViewPartner", null);
				SqlProcs.spPartners_Update
					( ref gpartner_ID
					, gASSIGNED_USER_ID
					, new DynamicControl(this, "NAME"                       ).Text
					, new DynamicControl(this, "partner_TYPE"               ).SelectedValue
					...
					, new DynamicControl(this, "partner_NUMBER"             ).Text
					, gTEAM_ID
					, sTEAM_SET_LIST
					...
					// 10/27/2017 Paul.  Add Partners as email source. 
				...
				SplendidDynamic.UpdateCustomFields(this, trn, gpartner_ID, sTABLE_NAME, dtCustomFields);
				SqlProcs.spPartners_InsRelated(gpartner_ID, sPARENT_TYPE, gPARENT_ID, trn);
				
				if ( chkCreateNote.Checked )
				{
					ctlConvertViewNote.RESOURCE_ID       = gPARENT_ID       ;
					...
					ctlConvertViewNote.Save(gpartner_ID, "Partners", trn);
				}
			}
			else
			{
				gpartner_ID = Sql.ToGuid(txtSELECT_partner_ID.Value);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				reqSELECT_partner_ID.DataBind();
				Guid gID = Sql.ToGuid(Request["ID"]);
				if ( !IsPostBack )
				{
					chkCreatePartner.Attributes.Add("onclick", "return ToggleCreatePartner();");
					chkCreateNote   .Attributes.Add("onclick", "toggleDisplay('divCreatePartnerNote');");
					if ( !Sql.IsEmptyGuid(gID) )
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							sSQL = "select *                     " + ControlChars.CrLf
							     + "  from vwLEADS_ConvertPartner" + ControlChars.CrLf;
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 )
										{
											...
											spartner_NAME = Sql.ToString(rdr["NAME"]);
											this.ApplyEditViewPreLoadEventRules ("Leads.ConvertViewPartner", rdr);
											this.AppendEditViewFields           ("Leads.ConvertViewPartner", tblMain, rdr);
											this.ApplyEditViewPostLoadEventRules("Leads.ConvertViewPartner", rdr);
										}
									}
								}
							}
							if ( !Sql.IsEmptyString(spartner_NAME) )
							{
								...
								using ( IDbCommand cmd = con.CreateCommand() )
								{
									cmd.CommandText = sSQL;
									Security.Filter(cmd, m_sMODULE, "edit");
									Sql.AppendParameter(cmd, spartner_NAME, "NAME");

									if ( bDebug )
										RegisterClientScriptBlock("SQLCode", Sql.ClientScriptBlock(cmd));

									Guid gpartner_ID = Sql.ToGuid(cmd.ExecuteScalar());
									if ( !Sql.IsEmptyGuid(gpartner_ID) )
									{
										txtSELECT_partner_ID.Value  = gpartner_ID.ToString();
										txtSELECT_partner_NAME.Text = spartner_NAME;
									}
								}
							}
						}
					...
					{
						this.AppendEditViewFields      ("Leads.ConvertViewPartner", tblMain, null);
						this.ApplyEditViewNewEventRules("Leads.ConvertViewPartner");
					}
				}
			}
			catch(Exception ex)
