...
{
	/// <summary>
	///		Summary description for ConvertViewRequest.
	/// </summary>
	public class ConvertViewRequest : InlineEditControl
	{
		protected Guid                   gREQUEST_ID                      ;
		...
		protected CheckBox               chkCreateRequest                 ;
		// 11/03/2008 Paul.  Allow selection of an request. 
		protected HtmlInputHidden        txtSELECT_REQUEST_ID             ;
		protected TextBox                txtSELECT_REQUEST_NAME           ;
		protected RequiredFieldValidatorForHiddenInputs reqSELECT_REQUEST_ID;
		...
		{
			get { return gREQUEST_ID; }
			set { gREQUEST_ID = value; }
		}
		...
		{
			if ( chkCreateRequest.Checked )
			{
				this.ValidateEditViewFields           ("Leads.ConvertViewRequest");
				this.ApplyEditViewValidationEventRules("Leads.ConvertViewRequest");
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			if ( chkCreateRequest.Checked )
			{
				...
				this.ApplyEditViewPreSaveEventRules("Leads.ConvertViewRequest", null);
				Guid gB2C_RESOURCE_ID = gPARENT_ID;
				...
				SplendidDynamic.UpdateCustomFields(this, trn, gREQUEST_ID, sTABLE_NAME, dtCustomFields);
				SqlProcs.spRequests_InsRelated(gREQUEST_ID, sPARENT_TYPE, gPARENT_ID, trn);
				...
				if ( chkCreateNote.Checked )
				{
					...
					ctlConvertViewNote.Save(gREQUEST_ID, "Requests", trn);
				}
			}
			else
			{
				// 11/03/2008 Paul.  Allow selection of an request. 
				gREQUEST_ID = Sql.ToGuid(txtSELECT_REQUEST_ID.Value);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				reqSELECT_REQUEST_ID.DataBind();
				Guid gID = Sql.ToGuid(Request["ID"]);
				if ( !IsPostBack )
				{
					chkCreateRequest.Attributes.Add("onclick", "return ToggleCreateRequest();");
					chkCreateNote       .Attributes.Add("onclick", "toggleDisplay('divCreateRequestNote');");
					if ( !Sql.IsEmptyGuid(gID) )
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							sSQL = "select *                         " + ControlChars.CrLf
							     + "  from vwLEADS_ConvertRequest" + ControlChars.CrLf;
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 )
										{
											...
											this.ApplyEditViewPreLoadEventRules ("Leads.ConvertViewRequest", rdr);
											this.AppendEditViewFields           ("Leads.ConvertViewRequest", tblMain, rdr);
											this.ApplyEditViewPostLoadEventRules("Leads.ConvertViewRequest", rdr);
										}
									}
								}
							}
					...
					{
						this.AppendEditViewFields      ("Leads.ConvertViewRequest", tblMain, null);
						this.ApplyEditViewNewEventRules("Leads.ConvertViewRequest");
					}
				}
			}
			catch(Exception ex)
