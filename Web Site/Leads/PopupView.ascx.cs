...
namespace SplendidCRM.Leads
{
	...
	public class PopupView : SplendidControl
	{
		...
		// 01/13/2016 Paul.  When assignign an account to a lead, we need to make sure that the user has edit access. 
		protected bool          bEditableOnly     = false;
			...
			try
			{
				...
				else if ( e.CommandName == "NewRecord" )
				{
					...
					if ( mgrAjax != null )
					{
						...
						ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "NewRecord", "SelectLead('" + gID.ToString() + "', '" + Sql.EscapeJavaScript(sNAME) + "');", true);
					}
					else
					{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				// 01/13/2016 Paul.  When assignign an account to a lead, we need to make sure that the user has edit access. 
				bEditableOnly = Sql.ToBoolean(Request["EditableOnly"]);
