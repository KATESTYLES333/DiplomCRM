<%@ Control CodeBehind="ProspectLists.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Campaigns.ProspectLists" %>
<script runat="server">
/**********************************************************************************************************************
...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtons" Module="ProspectLists" SubPanel="divCampaignsProspectLists" Title="ProspectLists.LBL_MODULE_NAME" Runat="Server" />

<div id="divCampaignsProspectLists" style='<%= "display:" + (CookieValue("divCampaignsProspectLists") != "1" ? "inline" : "none") %>'>
		...
		<Columns>
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<asp:ImageButton Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "edit", "ASSIGNED_USER_ID") >= 0 && !Sql.IsProcessPending(Container) %>' CommandName="ProspectLists.Edit" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "PROSPECT_LIST_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_EDIT") %>' SkinID="edit_inline" Runat="server" />
					<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "edit", "ASSIGNED_USER_ID") >= 0 && !Sql.IsProcessPending(Container) %>' CommandName="ProspectLists.Edit" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "PROSPECT_LIST_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_EDIT") %>' Runat="server" />
					&nbsp;
