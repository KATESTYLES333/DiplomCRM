...
{
	/// <summary>
	/// Summary description for TrackDetailView.
	/// </summary>
	public class TrackDetailView : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 )
										{
											...
											// 05/31/2015 Paul.  Combine ModuleHeader and DynamicButtons. 
											ctlDynamicButtons.AppendButtons(m_sMODULE + ".TrackDetailView", Sql.ToGuid(rdr["ASSIGNED_USER_ID"]), rdr);
											// 04/04/2008 Paul.  Add the ROI links.
											ctlDynamicButtons.AppendLinks  (m_sMODULE + ".TrackLinkView"  , Sql.ToGuid(rdr["ASSIGNED_USER_ID"]), rdr);
											// 08/22/2012 Paul.  Save the name so that it can be used when creating prospect lists from the campaign. 
										...
										else
										{
											...
											// 03/20/2008 Paul.  Dynamic buttons need to be recreated in order for events to fire. 
											// 05/31/2015 Paul.  Combine ModuleHeader and DynamicButtons. 
											ctlDynamicButtons.AppendButtons(m_sMODULE + ".TrackDetailView", Guid.Empty, null);
											// 04/04/2008 Paul.  Add the ROI links.
											ctlDynamicButtons.AppendLinks  (m_sMODULE + ".TrackLinkView"  , Guid.Empty, null);
				...
				else
				{
					...
					Page.Items["NAME"] = ctlDynamicButtons.Title;
				}
		...
		{    
			...
			m_sMODULE = "Campaigns";
			SetMenu(m_sMODULE);
			this.AppendDetailViewRelationships(m_sMODULE + ".TrackDetailView", plcSubPanel);
			if ( IsPostBack )
			{
