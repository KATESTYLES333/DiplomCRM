...
namespace SplendidCRM.Campaigns
{
	...
	public class Leads : SubPanelControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					{
						string sSQL  = "select ID from vwCAMPAIGNS_LEADS where CAMPAIGN_ID = '" + gID.ToString() + "'";
						string sNAME = Sql.ToString(Page.Items["NAME"]) + " - " + L10n.Term("Campaigns.LBL_CAMPAIGN_LEAD_SUBPANEL_TITLE");
						if ( sNAME.Length > 50 )
							sNAME = Sql.ToString(Page.Items["NAME"]).Substring(0, Sql.ToString(Page.Items["NAME"]).Length - (sNAME.Length - 50)) + " - " + L10n.Term("Campaigns.LBL_CAMPAIGN_LEAD_SUBPANEL_TITLE");
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
				     + "  from vwCAMPAIGNS_LEADS" + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwCAMPAIGNS_LEADS", Sql.ClientScriptBlock(cmd));
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Campaigns." + m_sMODULE, arrSelectFields, Page_Command);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
