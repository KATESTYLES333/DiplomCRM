			...
			try
			{
					...
					if ( grdMain.SelectedItem != null )
					{
						...
						if ( vw != null && vw.Count >= nItemIndex )
						{
							...
							if ( dtMain.Rows.Count > 0 )
							{
								...
								Guid gEMAIL_ID = ImapUtils.ImportMessage(Context, sPARENT_TYPE, gPARENT_ID, sSERVER_URL, nPORT, bMAILBOX_SSL, sEMAIL_USER, sDECRYPTED_PASSWORD, Security.USER_ID, gASSIGNED_USER_ID, gTEAM_ID, sTEAM_SET_LIST, sFOLDER_ID, sUNIQUE_ID);
				...
				else if ( e.CommandName == "Import.Save" )
				{
					if ( !Sql.IsEmptyString(hidSelectedUniqueID.Value) )
					{
						...
						ImapUtils.ImportMessage(Context, sPARENT_TYPE, gPARENT_ID, sSERVER_URL, nPORT, bMAILBOX_SSL, sEMAIL_USER, sDECRYPTED_PASSWORD, Security.USER_ID, gASSIGNED_USER_ID, gTEAM_ID, sTEAM_SET_LIST, sFOLDER_ID, sUNIQUE_ID);
					}
					ctlImportView.Visible = false;
				}
