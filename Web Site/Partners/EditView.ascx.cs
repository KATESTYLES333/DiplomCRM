				...
				try
				{
					...
					if ( Page.IsValid )
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
									...
									// 08/06/2009 Paul.  PARTNER_NUMBER now uses our number sequence table. 
									...
									SqlProcs.spPARTNERS_Update
										...
										, new DynamicControl(this, rowCurrent, "PARTNER_TYPE"               ).SelectedValue
										...
										, new DynamicControl(this, rowCurrent, "PARTNER_NUMBER"             ).Text
						...
						if ( !Sql.IsEmptyString(RulesRedirectURL) )
							...
							Response.Redirect("~/Partners/view.aspx?ID=" + gPARENT_ID.ToString());
						else
			...
			try
			{
				...
				if ( !IsPostBack )
				{
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 && (SplendidCRM.Security.GetRecordAccess(dtCurrent.Rows[0], m_sMODULE, "edit", "ASSIGNED_USER_ID") >= 0) )
										{
											...
											{
												new DynamicControl(this, "PARTNER_NUMBER").Text = String.Empty;
											}
											// 12/09/2008 Paul.  Throw an exception if the record has been edited since the last load. 
					...
					else
					{
						...
						// 05/09/2010 Paul.  The Parent can only be an Partner in this context. 
						if ( !Sql.IsEmptyGuid(gPARENT_ID) && sPARENT_TYPE == "Partners" )
						{
