...
namespace SplendidCRM.Partners
{
	...
	public class Resources : SubPanelControl
	{
			...
			try
			{
				switch ( e.CommandName )
				{
					...
					case "Resources.Delete":
					{
						...
						{
							SqlProcs.spPARTNERS_RESOURCES_Delete(gID, gRESOURCE_ID);
						}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				*/
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwPartners_Resources", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spPARTNERS_RESOURCES_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spPARTNERS_RESOURCES_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gRESOURCE_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spPARTNERS_RESOURCES_Update(gID, gRESOURCE_ID);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
			...
			if ( !IsPostBack )
			{
				...
				// 02/21/2010 Paul.  The partner needs to be initialized when the page first loads. 
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwPartners_Resources";
