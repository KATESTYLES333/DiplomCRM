...
namespace SplendidCRM.Partners
{
	...
	public class Documents : SubPanelControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					{
						Guid gDOCUMENT_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spPARTNERS_DOCUMENTS_Delete(gID, gDOCUMENT_ID);
						BindGrid();
						break;
					}
					case "Documents.GetLatest":
					{
						...
						{
							SqlProcs.spPARTNERS_DOCUMENTS_GetLatest(gID, gDOCUMENT_ID);
						}
						BindGrid();
						break;
					}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bEditView && IsPostBack && arrUPDATED.Count > 0 )
					{
						...
						arrSelectFields.Remove("PARTNER_ASSIGNED_USER_ID"     );
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     ...
						     + "     , @PARTNER_ID               as PARTNER_ID                   " + ControlChars.CrLf
						     + "     , @PARTNER_NAME             as PARTNER_NAME                 " + ControlChars.CrLf
						     + "     , @PARTNER_ASSIGNED_USER_ID as PARTNER_ASSIGNED_USER_ID     " + ControlChars.CrLf
						     + "  from vwDOCUMENTS" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Sql.AddParameter(cmd, "@PARTNER_ID"              , gID);
						Sql.AddParameter(cmd, "@PARTNER_NAME"            , Sql.ToString(Page.Items["NAME"            ]));
						Sql.AddParameter(cmd, "@PARTNER_ASSIGNED_USER_ID", Sql.ToGuid  (Page.Items["ASSIGNED_USER_ID"]));
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwPartners_DOCUMENTS", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spPARTNERS_DOCUMENTS_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spPARTNERS_DOCUMENTS_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gDOCUMENT_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spPARTNERS_DOCUMENTS_Update(gID, gDOCUMENT_ID);
					}
			...
			try
			{
				...
				if ( !IsPostBack )
				{
					...
					// 09/26/2017 Paul.  Add Archive relationship view. 
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwPartners_DOCUMENTS";
