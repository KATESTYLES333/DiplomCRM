...
namespace SplendidCRM.Partners
{
	...
	public class DetailView : SplendidControl
	{
		...
		protected HiddenField txtRESOURCE_ID    ;
        ...
        protected void Page_Command(Object sender, CommandEventArgs e)
		{
			try
			{
				...
				{
					SqlProcs.spPARTNERS_Delete(gID);
		...
		private void AddCopyButtons()
		{
			...
			{
				Control ctlBILLING_ADDRESS_HTML  = FindControl("BILLING_ADDRESS_HTML_LABEL");
				if ( ctlBILLING_ADDRESS_HTML != null )
				{
					ImageButton imgCopy = new ImageButton();
					imgCopy.SkinID = "copy_inline";
					imgCopy.OnClientClick = "return CopyBillingToResource();";
					imgCopy.Attributes.Add("style", "padding-left: 2px;");
					ctlBILLING_ADDRESS_HTML.Parent.Controls.Add(imgCopy);
				}
				Control ctlSHIPPING_ADDRESS_HTML = FindControl("SHIPPING_ADDRESS_HTML_LABEL");
				if ( ctlSHIPPING_ADDRESS_HTML != null )
				{
					ImageButton imgCopy = new ImageButton();
					imgCopy.SkinID = "copy_inline";
					imgCopy.OnClientClick = "return CopyShippingToResource();";
					imgCopy.Attributes.Add("style", "padding-left: 2px;");
					ctlSHIPPING_ADDRESS_HTML.Parent.Controls.Add(imgCopy);
				}
			}
			else
			{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				// 08/21/2010 Paul.  Copy Partner address to Resources. 
				if ( !Sql.IsEmptyString(txtRESOURCE_ID.Value) )
				{
					string[] arrID = txtRESOURCE_ID.Value.Split(',');
					if ( arrID != null )
					{
						System.Collections.Stack stk = Utils.FilterByACL_Stack(m_sMODULE, "edit", arrID, "Resources");
						if ( stk.Count > 0 )
						{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										while ( stk.Count > 0 )
										{
											...
											SqlProcs.spPARTNERS_RESOURCES_CopyAddress(sIDs, gID, txtADDRESS_TYPE.Value, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
					...
					if ( !Sql.IsEmptyGuid(gID) )
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							sSQL = "select " + m_sVIEW_NAME + ".*"                                           + ControlChars.CrLf
							     ...
							     + "     , (case when vwPartners_USERS.PARTNER_ID is null then 0 else 1 end) as EXCHANGE_FOLDER" + ControlChars.CrLf
							     + "  from            " + m_sVIEW_NAME                                       + ControlChars.CrLf
							     + "  left outer join vwPartners_USERS                                     " + ControlChars.CrLf
							     + "               on vwPartners_USERS.PARTNER_ID = " + m_sVIEW_NAME + ".ID" + ControlChars.CrLf
							     + "              and vwPartners_USERS.USER_ID    = @SYNC_USER_ID          " + ControlChars.CrLf;
