...
namespace SplendidCRM.Partners
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Partner Partners_partner_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "Partners", "list");
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Partners.ERR_partner_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] Partners_partner_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or partner_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] Partners_NAME_List(string prefixText, int count)
		{
			return Partners_partner_NAME_List(prefixText, count);
		}

		// 07/27/2010 Paul.  Add methods to be used by Quotes, Orders and Invoices. 
		[WebMethod(EnableSession=true)]
		public string[] Partners_BILLING_partner_NAME_List(string prefixText, int count)
		{
			return Partners_partner_NAME_List(prefixText, count);
		}

		[WebMethod(EnableSession=true)]
		public string[] Partners_SHIPPING_partner_NAME_List(string prefixText, int count)
		{
			return Partners_partner_NAME_List(prefixText, count);
		}
		...
		[WebMethod(EnableSession=true)]
		public string[] Partners_PARENT_NAME_List(string prefixText, int count)
		{
			return Partners_partner_NAME_List(prefixText, count);
		}

		// 10/18/2011 Paul.  The HTML5 Offline Client needs the get as parent name. 
		[WebMethod(EnableSession=true)]
		public Partner Partners_PARENT_NAME_Get(string sNAME)
		{
			return Partners_partner_NAME_Get(sNAME);
		}

		// 03/08/2016 Paul.  The HTML5 Offline Client needs the get for the Quotes, Orders Invoices. 
		[WebMethod(EnableSession=true)]
		public Partner Partners_BILLING_partner_NAME_Get(string sNAME)
		{
			return Partners_partner_NAME_Get(sNAME);
		}
		...
		// 03/08/2016 Paul.  The HTML5 Offline Client needs the get for the Quotes, Orders Invoices. 
		[WebMethod(EnableSession=true)]
		public Partner Partners_SHIPPING_partner_NAME_Get(string sNAME)
		{
			return Partners_partner_NAME_Get(sNAME);
		}
	}
