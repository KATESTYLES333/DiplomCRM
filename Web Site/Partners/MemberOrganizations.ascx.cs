	...
	public class MemberOrganizations : SplendidControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					{
						// 09/04/2005 Paul.  This partner was the parent, so clear it. 
						Guid gpartner_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spPARTNERS_PARENT_Update(gpartner_ID, Guid.Empty);
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				con.Open();
				string sSQL;
				// 02/25/2008 Paul.  Access rights on the buttons are dependent on partner ownership.
				...
				*/
				...
				m_sVIEW_NAME = "vwPartners_MEMBERS";
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					Security.Filter(cmd, m_sMODULE, "list", "partner_ASSIGNED_USER_ID");
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwPartners_MEMBERS", Sql.ClientScriptBlock(cmd));
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gpartner_ID) )
			{
				try
				{
					// 09/04/2005 Paul.  This partner becomes the parent of the selected partner. 
					SqlProcs.spPARTNERS_PARENT_Update(gpartner_ID, gID);
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
			...
			if ( !IsPostBack )
			{
				...
				ctlDynamicButtons.AppendButtons("Partners.MemberOrganizations", gASSIGNED_USER_ID, gID);
				...
				ctlNewRecord.PARENT_TYPE = "Partners";
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("partner_ASSIGNED_USER_ID");
			// 11/26/2005 Paul.  Add fields early so that sort events will get called. 
			// 06/07/2015 Paul.  Must include Page_Command in order for Preview to fire. 
			this.AppendGridColumns(grdMain, "Partners.MemberOrganizations", arrSelectFields, Page_Command);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Partners.MemberOrganizations", Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
