		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			if (e.CommandName == "Save")
			{
				...
				if (Page.IsValid)
				{
					...
					try
					{
						string sBUSINESS_RULE = new DynamicControl(this, "BUSINESS_RULE").Text;
						if (!Sql.IsEmptyString(sBUSINESS_RULE))
						{
							SplendidRulesTypeProvider typeProvider = new SplendidRulesTypeProvider();
							RuleValidation validation = new RuleValidation(typeof(DynamicButtonThis), typeProvider);
						...
						using (IDbTransaction trn = Sql.BeginTransaction(con))
						{
							try
							{
								...
								SqlProcs.spDYNAMIC_BUTTONS_Update
									...
									, new DynamicControl(this, "BUSINESS_RULE").Text
									, new DynamicControl(this, "BUSINESS_SCRIPT").Text
									, trn
									);
								trn.Commit();
							}
							...
							// 06/10/2010 Paul.  Clear the cache after updating the view. 
								...
								using (DbDataAdapter da = dbf.CreateDataAdapter())
								{
									...
									using (DataTable dtCurrent = new DataTable())
									{
										...
										if (dtCurrent.Rows.Count > 0)
										{
											...
											// 05/31/2015 Paul.  Combine ModuleHeader and DynamicButtons. 
											ctlDynamicButtons.Title = Sql.ToString(rdr["VIEW_NAME"]);
