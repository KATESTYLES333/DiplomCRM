...
function LayoutDetailViewRelationshipUI()
{
	...
	this.SORT_DIRECTIONS = new Array();
	this.SORT_DIRECTIONS.push('asc' );
	this.SORT_DIRECTIONS.push('desc');

	this.FIELDS = new Array();
	this.TABLE_FIELDS = new Array();
}

LayoutDetailViewRelationshipUI.prototype.RebuildLayout = function()
{
...
LayoutDetailViewRelationshipUI.prototype.PageCommand = function(sCommandName, sCommandArguments)
{
	...
	if ( sCommandName == 'Save' )
	{
		try
		{
			...
			var obj = new Object();
			obj.DETAILVIEWS_RELATIONSHIPS = this.RebuildLayout();

			AdminLayout_Update('DETAILVIEWS_RELATIONSHIPS', this.DETAIL_NAME, obj, function(status, message)
			{
				if ( status == 1 )
				{
		...
		{
			SplendidError.SystemError(e, 'LayoutDetailViewRelationshipUI.PageCommand');
		}
	}
...
LayoutDetailViewRelationshipUI.prototype.AddProperty = function(tblProperties, sFieldName, sFieldValue)
{
	var tr = null;
	var tdLabel = null;
	var tdField = null;
...
LayoutDetailViewRelationshipUI.prototype.AddTextBoxProperty = function(tblProperties, sFieldName, sFieldValue, sWidth)
{
	var tr = null;
	var tdLabel = null;
	var tdField = null;
...
LayoutDetailViewRelationshipUI.prototype.AddCheckBoxProperty = function(tblProperties, sFieldName, sFieldValue)
{
	var tr = null;
	var tdLabel = null;
	var tdField = null;
...
LayoutDetailViewRelationshipUI.prototype.AddListBoxProperty = function(tblProperties, sFieldName, sFieldValue, sListName, bAllowNone)
{
	var tr = null;
	var tdLabel = null;
	var tdField = null;
...
LayoutDetailViewRelationshipUI.prototype.ShowProperty = function(sFieldName, bVisible)
{
...
LayoutDetailViewRelationshipUI.prototype.GetPropertyVisibility = function(sFieldName)
{
	var bVisible = false;
	var tr = document.getElementById('tblProperties_tr' + sFieldName);
	if ( tr != null )
...
LayoutDetailViewRelationshipUI.prototype.GetPropertyValue = function(sFieldName)
{
	var sValue = '';
...
LayoutDetailViewRelationshipUI.prototype.SaveProperties = function()
{
	if ( this.divSelectedLayoutField != null )
	{
		...
		obj.DETAIL_NAME          = this.divSelectedLayoutField.DETAIL_NAME         ;
		...
		obj.TABLE_NAME           = this.divSelectedLayoutField.TABLE_NAME          ;
		obj.PRIMARY_FIELD        = this.divSelectedLayoutField.PRIMARY_FIELD       ;
		obj.SORT_FIELD           = this.GetPropertyValue('SORT_FIELD'             );
		obj.SORT_DIRECTION       = this.GetPropertyValue('SORT_DIRECTION'         );
...
LayoutDetailViewRelationshipUI.prototype.CancelProperties = function()
{
	this.divSelectedLayoutField = null;
...
LayoutDetailViewRelationshipUI.prototype.LoadProperties = function(divLayoutField)
{
	...
	//this.AddProperty        (tblProperties, 'DETAIL_NAME'         , Sql.ToString (divLayoutField.DETAIL_NAME         ));
	...
	if ( !Sql.IsEmptyString(divLayoutField.TABLE_NAME) )
	{
		AdminLayout_GetRelationshipFields(divLayoutField.TABLE_NAME, divLayoutField.MODULE_NAME, function(status, message)
		{
			if ( status == 0 || status == 1 )
			{
				this.TABLE_FIELDS = message;
				this.FIELDS = new Array();
				var bSortFound = false;
				for ( var i = 0; i < this.TABLE_FIELDS.length; i++ )
				{
					var lay = this.TABLE_FIELDS[i];
					this.FIELDS.push(lay.DATA_FIELD);
					if ( lay.DATA_FIELD == divLayoutField.SORT_FIELD )
						bSortFound = true;
				}
				// 03/14/2016 Paul.  The ActivityStream will have a special sort, so if value not found, then insert. 
				if ( !bSortFound )
					this.FIELDS.push(divLayoutField.SORT_FIELD);
				this.AddListBoxProperty (tblProperties, 'SORT_FIELD'        , Sql.ToString (divLayoutField.SORT_FIELD        ), this.FIELDS         , false);
				this.AddListBoxProperty (tblProperties, 'SORT_DIRECTION'    , Sql.ToString (divLayoutField.SORT_DIRECTION    ), this.SORT_DIRECTIONS, false);
			}
			else
			{
				if ( !Sql.IsEmptyString(divLayoutField.SORT_FIELD) )
				{
					this.FIELDS = new Array();
					this.FIELDS.push(divLayoutField.SORT_FIELD);
					this.AddListBoxProperty (tblProperties, 'SORT_FIELD'        , Sql.ToString (divLayoutField.SORT_FIELD        ), this.FIELDS         , false);
...
LayoutDetailViewRelationshipUI.prototype.LoadFromLayout = function()
{
	var tblLayout = document.getElementById('tblLayout');
...
LayoutDetailViewRelationshipUI.prototype.CreateLayoutObject = function(src)
{
	var obj = new Object();
	obj.ID                   = Sql.ToString (src.ID                  );
	obj.DETAIL_NAME          = Sql.ToString (src.DETAIL_NAME         );
	...
	obj.TABLE_NAME           = Sql.ToString (src.TABLE_NAME          );
	obj.PRIMARY_FIELD        = Sql.ToString (src.PRIMARY_FIELD       );
	obj.SORT_FIELD           = Sql.ToString (src.SORT_FIELD          );
	obj.SORT_DIRECTION       = Sql.ToString (src.SORT_DIRECTION      );
	return obj;
}

LayoutDetailViewRelationshipUI.prototype.RenderField = function(div)
{
	...
	$(td2_2).text(Sql.ToString(div.SORT_FIELD) + ' ' + Sql.ToString(div.SORT_DIRECTION));
...
LayoutDetailViewRelationshipUI.prototype.SetLayoutObject = function(div, src)
{
	div.ID                   = Sql.ToString (src.ID                  );
	div.DETAIL_NAME          = Sql.ToString (src.DETAIL_NAME         );
	...
	div.TABLE_NAME           = Sql.ToString (src.TABLE_NAME          );
	div.PRIMARY_FIELD        = Sql.ToString (src.PRIMARY_FIELD       );
	div.SORT_FIELD           = Sql.ToString (src.SORT_FIELD          );
	div.SORT_DIRECTION       = Sql.ToBoolean(src.SORT_DIRECTION      );
	this.RenderField(div);
}

LayoutDetailViewRelationshipUI.prototype.LayoutAddField = function(jDropTarget, jDragged)
{
...
LayoutDetailViewRelationshipUI.prototype.BindRow = function(tblLayout, tr)
{
	var context = this;
	tr.className             = 'grab';
...
LayoutDetailViewRelationshipUI.prototype.BindColumn = function(tblLayout, td, lay)
{
	var context = this;
	...
	*/
	...
	{
		div.ID                   = lay.ID                  ;
		div.DETAIL_NAME          = lay.DETAIL_NAME         ;
		...
		div.TABLE_NAME           = lay.TABLE_NAME          ;
		div.PRIMARY_FIELD        = lay.PRIMARY_FIELD       ;
		div.SORT_FIELD           = lay.SORT_FIELD          ;
		div.SORT_DIRECTION       = lay.SORT_DIRECTION      ;
		this.RenderField(div);
	}
}

LayoutDetailViewRelationshipUI.prototype.LayoutAddRow = function(nPosition, lay)
{
	var tblLayout = document.getElementById('tblLayout');
...
LayoutDetailViewRelationshipUI.prototype.LoadView = function()
{
...
LayoutDetailViewRelationshipUI.prototype.Load = function()
{
	try
	{
		AdminLayoutClear();
		...
		bgPage.AuthenticatedMethod(function(status, message)
		{
			if ( status == 1 )
			{
				bgPage.Terminology_LoadModule(this.MODULE_NAME, function(status, message)
				{
					if ( status == 0 || status == 1 )
					{
						AdminLayout_GetDetailViewRelationships(this.DETAIL_NAME, function(status, message)
						{
							if ( status == 1 )
							{
								this.LAYOUT = message;
	...
	{
		SplendidError.SystemError(e, 'LayoutDetailViewRelationshipUI.Load');
	}
}

LayoutDetailViewRelationshipUI.prototype.Reload = function()
{
	try
	{
		AdminLayout_GetDetailViewRelationships(this.DETAIL_NAME, function(status, message)
