    ...
    <asp:TableRow>
        ...
        <asp:TableCell HorizontalAlign="Right" Visible="false">
            <div id="divExport" visible='<%# !IsMobile && !PrintView && SplendidCRM.Security.GetUserAccess(sModule, "export") >= 0 %>' runat="server">
                ...
                <asp:Button ID="btnExport" CommandName="Export1" OnCommand="Page_Command" CssClass="button" Text='<%# L10n.Term(".LBL_EXPORT_BUTTON_LABEL") %>' ToolTip='<%# L10n.Term(".LBL_EXPORT_BUTTON_TITLE") %>' AccessKey='<%# L10n.AccessKey(".LBL_EXPORT_BUTTON_KEY") %>' runat="server" />
            </div>
        </asp:TableCell>
    </asp:TableRow>
</asp:Table>
