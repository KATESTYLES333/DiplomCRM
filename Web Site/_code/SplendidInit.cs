					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						using ( IDataReader rdr = cmd.ExecuteReader() )
						{
							...
							while ( rdr.Read() )
							{
								...
								Application["Modules." + sMODULE_NAME + ".Exists"      ] = (Sql.IsEffiProz(con) || Sql.IsEmptyString(sRELATIVE_PATH) || File.Exists(Context.Server.MapPath(sRELATIVE_PATH + "default.aspx")) || File.Exists(Context.Server.MapPath(sRELATIVE_PATH + "popup.aspx")));
