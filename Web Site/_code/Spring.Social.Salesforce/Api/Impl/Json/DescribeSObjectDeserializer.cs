...
namespace Spring.Social.Salesforce.Api.Impl.Json
{
	...
	class DescribeSObjectDeserializer : IJsonDeserializer
	{
		public object Deserialize(JsonValue json, JsonMapper mapper)
		{
			DescribeSObject obj = null;
			if ( json != null && !json.IsNull )
			{
				obj = new DescribeSObject();
				...
				if ( jsonUrls != null && !jsonUrls.IsNull )
				{
					...
					obj.UrlDetail      = jsonUrls.ContainsName("uiDetailTemplate") ? jsonUrls.GetValue<string >("uiDetailTemplate") : String.Empty;
					obj.UrlEdit        = jsonUrls.ContainsName("uiEditTemplate"  ) ? jsonUrls.GetValue<string >("uiEditTemplate"  ) : String.Empty;
					obj.UrlNew         = jsonUrls.ContainsName("uiNewTemplate"   ) ? jsonUrls.GetValue<string >("uiNewTemplate"   ) : String.Empty;
				}
				obj.Fields             = mapper.Deserialize<List<Field>            >(json.GetValue("fields"            ));
				obj.RecordTypeInfos    = mapper.Deserialize<List<RecordTypeInfo>   >(json.GetValue("recordTypeInfos"   ));
				obj.ChildRelationships = mapper.Deserialize<List<ChildRelationship>>(json.GetValue("childRelationships"));
			}
			return obj;
		}
	}
