...
namespace SplendidCRM
{
	public class EmailUtils
	{
		...
		*/
		...
		public static void SendTestMessage(HttpApplicationState Application, string sSmtpServer, int nSmtpPort, bool bSmtpAuthReq, bool bSmtpSSL, string sSmtpUser, string sSmtpEncryptedPassword, string sFromAddress, string sFromName, string sToAddress, string sToName)
		{
			string sX509Certificate = Sql.ToString (Application["CONFIG.smtpcertificate"]);
			// 01/17/2017 Paul.  New SplendidMailClient object to encapsulate SMTP, Exchange and Google mail. 
			SplendidMailClient client = new SplendidMailSmtp(Application, sSmtpServer, nSmtpPort, bSmtpAuthReq, bSmtpSSL, sSmtpUser, sSmtpEncryptedPassword, sX509Certificate);
								...
								try
								{
									...
									if ( bOFFICE365_OAUTH_ENABLED )
									{
										...
										if ( Sql.IsEmptyString(sMAILBOX) )
											sMAILBOX = "Inbox";
										DataTable dt = ExchangeUtils.GetFolderMessages(Context, String.Empty, String.Empty, gMAILBOX_ID, sMAILBOX, bONLY_SINCE, sEXCHANGE_WATERMARK);
										foreach ( DataRow row in dt.Rows )
										{
											// 12/12/2017 Paul.  Azure is dropping the connection, but continuing the loop and generating lots of errors.  Just exit and wait to try again. 
											if ( con.State != ConnectionState.Open )
