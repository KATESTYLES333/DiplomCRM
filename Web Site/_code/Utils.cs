	...
	public class ArchiveUtils
	{
		...
		public string MoveData(string sModuleName, string[] arrID)
		{
			...
			if ( arrID != null )
			{
				...
				if ( arrID.Length > 100 || (arrID.Length > 10 && bIncludeActivities) )
				{
		...
		private void MoveDataInternal()
		{
			try
			{
				System.Collections.Stack stk = Utils.FilterByACL_Stack(this.ModuleName, "archive", arrID, Crm.Modules.TableName(this.ModuleName));
				if ( stk.Count > 0 )
				{
					DbProviderFactory dbf = DbProviderFactories.GetFactory(Context.Application);
					using ( IDbConnection con = dbf.CreateConnection() )
