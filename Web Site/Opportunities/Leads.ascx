...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtons" Module="Leads" SubPanel="divOpportunitiesLeads" Title="Leads.LBL_MODULE_NAME" Runat="Server" />
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript(".NTC_DELETE_CONFIRMATION") %>')">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "delete", "ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Leads.Delete" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "LEAD_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_DELETE") %>' SkinID="delete_inline" Runat="server" />
						<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "delete", "ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Leads.Delete" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "LEAD_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_DELETE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
