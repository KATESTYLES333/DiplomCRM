...
namespace SplendidCRM.Opportunities
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Opportunity OPPORTUNITIES_OPPORTUNITY_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Opportunities.ERR_OPPORTUNITY_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] OPPORTUNITIES_OPPORTUNITY_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or OPPORTUNITY_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] OPPORTUNITIES_NAME_List(string prefixText, int count)
		{
			return OPPORTUNITIES_OPPORTUNITY_NAME_List(prefixText, count);
		}
	}
}
