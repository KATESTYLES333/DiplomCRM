<%@ Control CodeBehind="MyPipelineBySalesStage.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Opportunities.xaml.MyPipelineBySalesStage" %>
...
<Canvas x:Name="container" Loaded="mainCanvasLoaded" MouseMove="whenMouseMoves" MouseLeave="mainCanvasMouseLeave" xmlns="http://schemas.microsoft.com/client/2007" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml">
	<Canvas x:Name="bound_box" <%= L10n.IsLanguageRTL() ? "FlowDirection=\"RightToLeft\"" : String.Empty %> Canvas.Top="10" Canvas.Left="10" Width="330" Height="380" Background="White">
		<TextBlock Text="<%# HttpUtility.HtmlEncode(sGraphData_Title) %>" Canvas.Top="3" Canvas.Left="102" Foreground="#393839" FontFamily="Arial" FontSize="12" FontWeight="Bold" />

		<Canvas Canvas.Top="24">
			<Canvas.Clip>
				<RectangleGeometry Rect="0, 0, 332, 226" />
			</Canvas.Clip>
			<Canvas x:Name="myScroller1_Container" Canvas.Left="322" Height="226">
...
<asp:Repeater DataSource="<%# nlDataRows %>" runat="server">
	<HeaderTemplate>
				<Canvas x:Name="y_labels" Canvas.Top="4" Canvas.Left="0" Width="110" Height="<%# nlDataRows.Count * 20 %>">
	</HeaderTemplate>
	<ItemTemplate>
	...
	<FooterTemplate>
				...
				<Canvas x:Name="grid" Canvas.Top="0" Canvas.Left="120" Width="<%# nGridWidth + nGridWidth / nAxis_xData_length + 1 %>" Height="<%# nGridHeight %>">
					<Rectangle Width="201" Height="<%# nGridHeight %>" Stroke="#D6D3D6" StrokeThickness="1" Fill="#08080808" />
