<%@ Control CodeBehind="PopupView.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Opportunities.PopupView" %>
<script runat="server">
/**********************************************************************************************************************
...
<div id="divPopupView">
	...
	<SplendidCRM:SearchView ID="ctlSearchView" Module="Opportunities" IsPopupSearch="true" ShowSearchTabs="false" Visible="<%# !PrintView %>" Runat="Server" />
	...
	function SelectChecked()
	{
		if ( window.opener != null && window.opener.ChangeOpportunity != null )
		{
			...
			window.opener.ChangeOpportunity(sSelectedItems, '');
			window.close();
		}
		else
		{
	...
	function Clear()
	{
		if ( window.opener != null && window.opener.ChangeOpportunity != null )
		{
			window.opener.ChangeOpportunity('', '');
			window.close();
		}
		else
		{
	...
	<SplendidCRM:ListHeader Title="Opportunities.LBL_LIST_FORM_TITLE" Runat="Server" />
	...
	<asp:UpdatePanel UpdateMode="Conditional" Visible='<%# !Sql.ToBoolean(Application["CONFIG.disable_popupview_inline"]) %>' runat="server">
		<ContentTemplate>
			<asp:Button ID="btnCreateInline" CommandName="NewRecord.Show" OnCommand="Page_Command" Text='<%# L10n.Term(m_sMODULE + ".LNK_NEW_OPPORTUNITY") %>' CssClass="button" style="margin-bottom: 4px;" Visible="<%# !this.IsMobile %>" runat="server" />
