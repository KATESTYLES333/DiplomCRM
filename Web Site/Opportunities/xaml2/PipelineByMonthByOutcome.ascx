<%@ Control CodeBehind="PipelineByMonthByOutcome.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Opportunities.xaml2.PipelineByMonthByOutcome" %>
<UserControl 
...
<asp:Repeater DataSource="<%# dsMain.Tables %>" runat="server">
	<ItemTemplate>
			<Style x:Key="<%# (Container.DataItem as System.Data.DataTable).TableName %>_ColumnDataPointStyle" TargetType="charting:ColumnDataPoint">
				...
				<Setter Property="Template">
					<Setter.Value>
						<ControlTemplate TargetType="charting:ColumnDataPoint" xmlns:vsm="clr-namespace:System.Windows;assembly=System.Windows">
...
<asp:Repeater DataSource="<%# dsMain.Tables %>" runat="server">
	<ItemTemplate>
				<splendid:StackedColumnSeries ItemsSource="{StaticResource <%# (Container.DataItem as System.Data.DataTable).TableName %>}" IndependentValueBinding="{Binding Label}" DependentValueBinding="{Binding Value}" DataPointStyle="{StaticResource <%# (Container.DataItem as System.Data.DataTable).TableName %>_ColumnDataPointStyle}" Foreground="#395163" FontFamily="Arial" FontSize="11" IsSelectionEnabled="True" />
	</ItemTemplate>
</asp:Repeater>
			</charting:Chart.Series>
			<charting:Chart.Axes>
				<charting:LinearAxis Orientation="Y" ShowGridLines="True" GridLineStyle="{StaticResource GridLineStyle}" Minimum="<%# nAxis_yData_min %>" Maximum="<%# nAxis_yData_max %>" Foreground="#395163" FontWeight="Normal" FontFamily="Arial" FontSize="9" AxisLabelStyle="{StaticResource LinearAxisLabelStyle}" />
				<charting:CategoryAxis Orientation="X" Foreground="#395163" FontWeight="Normal" FontFamily="Arial" FontSize="11" AxisLabelStyle="{StaticResource CategoryAxisLabelStyle}" />
			</charting:Chart.Axes>
		</charting:Chart>
		<Canvas>
