	...
	public class ListView : SplendidControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										while ( stk.Count > 0 )
										{
											...
											SqlProcs.spOPPORTUNITIES_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.ACCOUNT_ID, ctlMassUpdate.OPPORTUNITY_TYPE, ctlMassUpdate.LEAD_SOURCE, T10n.ToServerTime(ctlMassUpdate.DATE_CLOSED), ctlMassUpdate.SALES_STAGE, ctlMassUpdate.PRIMARY_TEAM_ID, ctlMassUpdate.TEAM_SET_LIST, ctlMassUpdate.ADD_TEAM_SET, ctlMassUpdate.TAG_SET_NAME, ctlMassUpdate.ADD_TAG_SET, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
							...
							using ( IDbConnection con = dbf.CreateConnection() )
							{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										...
										{
											string sIDs = Utils.BuildMassIDs(stk);
											SqlProcs.spOPPORTUNITIES_MassDelete(sIDs, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
				...
				// 04/04/2010 Paul.  Add support for Exchange Sync. 
				else if ( e.CommandName == "Sync" )
				{
					...
					if ( arrID != null )
					{
						...
						{
							SqlProcs.spOPPORTUNITIES_MassSync(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}
				else if ( e.CommandName == "Unsync" )
				{
					...
					if ( arrID != null )
					{
						...
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spOPPORTUNITIES_MassUnsync(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				// 01/10/2015 Paul.  The HTML5 Dashboard Charts will pass parameters in URL.  Convert to a SavedSearch. 
				if ( !IsPostBack )
				{
					string sLEAD_SOURCE      = Sql.ToString(Request["LEAD_SOURCE"     ]);
					string sSALES_STAGE      = Sql.ToString(Request["SALES_STAGE"     ]);
					string sASSIGNED_USER_ID = Sql.ToString(Request["ASSIGNED_USER_ID"]);
					string sDATE_CLOSED      = Sql.ToString(Request["DATE_CLOSED"     ]);
					if ( !Sql.IsEmptyString(sLEAD_SOURCE) || !Sql.IsEmptyString(sSALES_STAGE) || !Sql.IsEmptyString(sASSIGNED_USER_ID) || !Sql.IsEmptyString(sDATE_CLOSED) )
					{
						Guid gID = Guid.Empty;
						...
						if ( !Sql.IsEmptyString(sLEAD_SOURCE) )
						{
							...
							XmlUtil.SetSingleNodeAttribute(xml, xField, "Name", "LEAD_SOURCE");
							...
							xValue.InnerText = sLEAD_SOURCE;
						}
						if ( !Sql.IsEmptyString(sSALES_STAGE) )
						{
							XmlNode xField = xml.CreateElement("Field");
							XmlUtil.SetSingleNodeAttribute(xml, xField, "Name", "SALES_STAGE");
							XmlUtil.SetSingleNodeAttribute(xml, xField, "Type", "ListBox");
							xSearchFields.AppendChild(xField);
							XmlNode xValue = xml.CreateElement("Value");
							xField.AppendChild(xValue);
							xValue.InnerText = sSALES_STAGE;
						}
						if ( !Sql.IsEmptyString(sASSIGNED_USER_ID) )
						{
							XmlNode xField = xml.CreateElement("Field");
							XmlUtil.SetSingleNodeAttribute(xml, xField, "Name", "ASSIGNED_USER_ID");
							XmlUtil.SetSingleNodeAttribute(xml, xField, "Type", "ListBox");
							xSearchFields.AppendChild(xField);
							XmlNode xValue = xml.CreateElement("Value");
							xField.AppendChild(xValue);
							xValue.InnerText = sASSIGNED_USER_ID;
						}
						if ( !Sql.IsEmptyString(sDATE_CLOSED) )
						{
							DateTime dtDATE_CLOSED = Sql.ToDateTime(sDATE_CLOSED);
							if ( dtDATE_CLOSED != DateTime.MinValue )
							{
								XmlNode xField = xml.CreateElement("Field");
								XmlUtil.SetSingleNodeAttribute(xml, xField, "Name", "DATE_CLOSED");
								XmlUtil.SetSingleNodeAttribute(xml, xField, "Type", "DateRange");
								xSearchFields.AppendChild(xField);
								XmlNode xAfter = xml.CreateElement("After");
								xField.AppendChild(xAfter);
								xAfter.InnerText = dtDATE_CLOSED.ToShortDateString();
								XmlNode xBefore = xml.CreateElement("Before");
								xField.AppendChild(xBefore);
								xBefore.InnerText = dtDATE_CLOSED.AddMonths(1).ToShortDateString();
							}
						}
						SqlProcs.spSAVED_SEARCH_Update(ref gID, Security.USER_ID, String.Empty, m_sMODULE, xml.OuterXml, String.Empty, Guid.Empty);
						Session.Remove("vwSAVED_SEARCH." + m_sMODULE);
