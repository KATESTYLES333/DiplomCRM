...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtons" Module="Project" SubPanel="divOpportunitiesProjects" Title="Project.LBL_PROJECT_SUBPANEL_TITLE" Runat="Server" />
			...
			<asp:TemplateColumn  HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript("Opportunities.OPPORTUNITY_REMOVE_PROJECT_CONFIRM") %>')">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, "Opportunities", "remove", "OPPORTUNITY_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Project.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "PROJECT_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' SkinID="delete_inline" Runat="server" />
						<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, "Opportunities", "remove", "OPPORTUNITY_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Project.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "PROJECT_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_REMOVE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
