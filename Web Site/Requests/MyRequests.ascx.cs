...
{
	/// <summary>
	///		Summary description for MyRequests.
	/// </summary>
	public class MyRequests : DashletControl
	{
		...
		protected void Bind(bool bBind)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				m_sVIEW_NAME = "vwRequests_MyList";
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwRequests_MyList", Sql.ClientScriptBlock(cmd));
					...
					using ( DbDataAdapter da = dbf.CreateDataAdapter() )
					{
						...
						{
							using ( DataTable dt = new DataTable("vwRequests_MyList") )
							{
								ds.Tables.Add(dt);
								// 08/16/2005 Paul.  Instead of TOP, use Fill to restrict the records. 
								// 04/02/2006 Paul.  Start record should be 0. 
								// 03/04/2010 Paul.  The Request Top Count should be configurable. 
								int nRequestCount = Sql.ToInteger(Session["USER_SETTINGS/REQUEST_TOP_COUNT"]);
								if ( nRequestCount <= 0 )
									nRequestCount = Sql.ToInteger(Application["CONFIG.request_top_count"]);
								if ( nRequestCount <= 0 )
									nRequestCount = 5;
								da.Fill(ds, 0, nRequestCount, "vwRequests_MyList");
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( this.Visible && !Sql.IsEmptyString(sDetailView) )
			{
				...
				vwFields.RowFilter = "CONTROL_NAME = '~/Requests/MyRequests'";
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Requests.MyRequests", arrSelectFields);
		}
		#endregion
	}
}
