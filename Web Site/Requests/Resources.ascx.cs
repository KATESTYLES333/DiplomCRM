...
namespace SplendidCRM.Requests
{
	...
	public class Resources : SubPanelControl
	{
			...
			try
			{
				switch ( e.CommandName )
				{
					...
					case "Resources.Remove":
					{
						...
						{
							SqlProcs.spREQUESTS_RESOURCES_Delete(gID, gRESOURCE_ID);
						}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bEditView && IsPostBack && arrUPDATED.Count > 0 )
					{
						...
						arrSelectFields.Remove("REQUEST_ASSIGNED_USER_ID");
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "     , ID                            as RESOURCE_ID                  " + ControlChars.CrLf
						     + "     , NAME                          as RESOURCE_NAME                " + ControlChars.CrLf
						     + "     , @REQUEST_ID               as REQUEST_ID              " + ControlChars.CrLf
						     + "     , @REQUEST_NAME             as REQUEST_NAME            " + ControlChars.CrLf
						     + "     , @REQUEST_ASSIGNED_USER_ID as REQUEST_ASSIGNED_USER_ID" + ControlChars.CrLf
						     + "  from vwRESOURCES" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Sql.AddParameter(cmd, "@REQUEST_ID"              , gID);
						Sql.AddParameter(cmd, "@REQUEST_NAME"            , Sql.ToString(Page.Items["NAME"            ]));
						Sql.AddParameter(cmd, "@REQUEST_ASSIGNED_USER_ID", Sql.ToGuid  (Page.Items["ASSIGNED_USER_ID"]));
					...
					else
					{
						...
						Sql.AppendParameter(cmd, gID, "REQUEST_ID");
					}
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwRequests_Resources", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								// 10/05/2017 Paul.  Add Archive relationship view. 
								this.ApplyGridViewRules("Requests." + this.LayoutListView, dt);
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spREQUESTS_RESOURCES_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spREQUESTS_RESOURCES_Update(gPARENT_ID, gUPDATE_ID, String.Empty, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gRESOURCE_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spREQUESTS_RESOURCES_Update(gID, gRESOURCE_ID, String.Empty);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
			...
			if ( !IsPostBack )
			{
				...
				// 10/05/2017 Paul.  Add Archive relationship view. 
				ctlDynamicButtons.AppendButtons("Requests." + this.LayoutListView, gASSIGNED_USER_ID, gID);
				...
				ctlNewRecord.PARENT_TYPE = "Requests";
				// 10/04/2010 Paul.  Cache the Partner for use when creating a contact inline. 
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("REQUEST_ASSIGNED_USER_ID");
			// 11/26/2005 Paul.  Add fields early so that sort events will get called. 
			// 06/07/2015 Paul.  Must include Page_Command in order for Preview to fire. 
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwRequests_Resources";
			if ( ArchiveViewExists() )
				m_sVIEW_NAME = m_sVIEW_NAME + "_ARCHIVE";
			this.LayoutListView = m_sMODULE + (ArchiveView() ? ".ArchiveView" : String.Empty);
			this.AppendGridColumns(grdMain, "Requests." + this.LayoutListView, arrSelectFields, Page_Command);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Requests." + this.LayoutListView, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
