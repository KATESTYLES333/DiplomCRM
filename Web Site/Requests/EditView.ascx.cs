	...
	public class EditView : SplendidControl
	{
					...
					if ( Page.IsValid )
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								...
								{
									// 08/07/2015 Paul.  Revenue Line Items. 
									if ( Sql.ToString(Application["CONFIG.RequestsMode"]) == "Revenue" )
									{
										...
										SqlProcs.spREQUESTS_Update
											...
											, new DynamicControl(this, rowCurrent, "REQUEST_TYPE").SelectedValue
											...
											, new DynamicControl(this, rowCurrent, "B2C_RESOURCE_ID"  ).ID
											...
											, new DynamicControl(this, rowCurrent, "REQUEST_NUMBER").Text
											// 11/30/2017 Paul.  Add ASSIGNED_SET_ID for Dynamic User Assignment. 
											, new DynamicControl(this, rowCurrent, "ASSIGNED_SET_LIST" ).Text
                                            , new DynamicControl(this, rowCurrent, "ORIGINATOR_ID").ID
                                            , new DynamicControl(this, rowCurrent, "ORIGINATOR").Text
                                            , new DynamicControl(this, rowCurrent, "RFP").Text
                                            , new DynamicControl(this, rowCurrent, "COLLAB").Text
										...
										foreach ( DataRow row in dtLineItems.Rows )
										{
											if ( row.RowState != DataRowState.Deleted )
											{
												...
												string   sREQUEST_TYPE    = Sql.ToString  (row["REQUEST_TYPE"   ]);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				{
					// 08/07/2015 Paul.  Revenue Line Items. 
					ctlEditLineItemsView.Visible = (Sql.ToString(Application["CONFIG.RequestsMode"]) == "Revenue");
					...
					if ( !Sql.IsEmptyGuid(gID) || !Sql.IsEmptyGuid(gDuplicateID) )
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							sSQL = "select " + m_sVIEW_NAME + ".*"                                                    + ControlChars.CrLf
							     ...
							     + "     , (case when vwRequests_USERS.REQUEST_ID is null then 0 else 1 end) as EXCHANGE_FOLDER" + ControlChars.CrLf
							     + "  from            " + m_sVIEW_NAME                                                + ControlChars.CrLf
							     + "  left outer join vwRequests_USERS                                         " + ControlChars.CrLf
							     + "               on vwRequests_USERS.REQUEST_ID = " + m_sVIEW_NAME + ".ID" + ControlChars.CrLf
							     + "              and vwRequests_USERS.USER_ID        = @SYNC_USER_ID          " + ControlChars.CrLf;
							using ( IDbCommand cmd = con.CreateCommand() )
							{
								cmd.CommandText = sSQL;
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 && (SplendidCRM.Security.GetRecordAccess(dtCurrent.Rows[0], m_sMODULE, "edit", "ASSIGNED_USER_ID") >= 0) )
										{
											...
											{
												new DynamicControl(this, "REQUEST_NUMBER").Text = String.Empty;
											}
											// 12/09/2008 Paul.  Throw an exception if the record has been edited since the last load. 
					...
					else
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							if ( !Sql.IsEmptyGuid(gQUOTE_ID) )
							{
								...
								using ( IDbCommand cmd = con.CreateCommand() )
								{
									...
									using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
									{
										if ( rdr.Read() )
										{
											new DynamicControl(this, "PARTNER_ID"  ).ID   = Sql.ToGuid  (rdr["BILLING_PARTNER_ID"  ]);
											new DynamicControl(this, "PARTNER_NAME").Text = Sql.ToString(rdr["BILLING_PARTNER_NAME"]);
											new DynamicControl(this, "AMOUNT"      ).DecimalValue = C10n.ToCurrency(Sql.ToDecimal(rdr["TOTAL_USDOLLAR"]));
											// 05/01/2013 Paul.  Add Resources field to support B2C. 
											new DynamicControl(this, "B2C_RESOURCE_ID"  ).ID   = Sql.ToGuid  (rdr["BILLING_RESOURCE_ID"  ]);
											//new DynamicControl(this, "B2C_RESOURCE_NAME").Text = Sql.ToString(rdr["BILLING_RESOURCE_NAME"]);
										}
									}
								}
							}

							// 09/18/2008 Paul.  Partner should be prepopulated if created from Partner. 
							Guid gPARENT_ID = Sql.ToGuid(Request["PARENT_ID"]);
							sSQL = "select *         " + ControlChars.CrLf
							...
							if ( !Sql.IsEmptyGuid(gPARENT_ID) )
							{
								...
								{
									cmd.CommandText = sSQL;
									// 10/07/2010 Paul.  Filter by Partners module. 
								...
								// 09/08/2010 Paul.  Prefill the Partner for the Request from a Resource parent. 
								sSQL = "select *         " + ControlChars.CrLf
								...
								using ( IDbCommand cmd = con.CreateCommand() )
								{
									...
									using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
									{
										if ( rdr.Read() )
										{
											new DynamicControl(this, "PARTNER_ID"  ).ID   = Sql.ToGuid  (rdr["PARTNER_ID"  ]);
											new DynamicControl(this, "PARTNER_NAME").Text = Sql.ToString(rdr["PARTNER_NAME"]);
											// 05/01/2013 Paul.  Add Resources field to support B2C. 
											new DynamicControl(this, "B2C_RESOURCE_ID"  ).ID   = Sql.ToGuid  (rdr["ID"          ]);
											//new DynamicControl(this, "B2C_RESOURCE_NAME").Text = Sql.ToString(rdr["NAME"        ]);
										}
									}
								}
							}
