...
namespace SplendidCRM.Requests
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Request Requests_REQUEST_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "Requests", "list");
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Requests.ERR_REQUEST_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] Requests_REQUEST_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or REQUEST_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] Requests_NAME_List(string prefixText, int count)
		{
			return Requests_REQUEST_NAME_List(prefixText, count);
		}
	}
}
