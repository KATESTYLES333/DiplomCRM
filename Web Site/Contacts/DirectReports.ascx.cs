...
namespace SplendidCRM.Contacts
{
	...
	/// </summary>
	public class DirectReports : SplendidControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					{
						Guid gCONTACT_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spCONTACTS_REPORTS_TO_Update(gCONTACT_ID, Guid.Empty);
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				con.Open();
				string sSQL;
				// 02/25/2008 Paul.  Access rights on the buttons are dependent on contact ownership.
				...
				*/
				...
				m_sVIEW_NAME = "vwCONTACTS_DIRECT_REPORTS";
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					Security.Filter(cmd, m_sMODULE, "list", "DIRECT_REPORT_ASSIGNED_USER_ID");
					...
					cmd.CommandText += grdMain.OrderByClause("LAST_NAME", "asc");

					if ( bDebug )
						RegisterClientScriptBlock("vwCONTACTS_DIRECT_REPORTS", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								this.ApplyGridViewRules(m_sMODULE + ".DirectReports", dt);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gCONTACT_ID) )
			{
				try
				{
					// 09/05/2005 Paul.  The selected contact will report to the currently displayed contact. 
					SqlProcs.spCONTACTS_REPORTS_TO_Update(gCONTACT_ID, gID);
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
			...
			if ( !IsPostBack )
			{
				...
				//Page.DataBind();
				// 03/13/2014 Paul.  Use existing account information. 
				...
				ctlNewRecord.REPORTS_TO_ID = gID;
				// 04/28/2008 Paul.  Make use of dynamic buttons. 
				Guid gASSIGNED_USER_ID = Sql.ToGuid(Page.Items["ASSIGNED_USER_ID"]);
				ctlDynamicButtons.AppendButtons("Contacts.DirectReports", gASSIGNED_USER_ID, gID);
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Contacts.DirectReports", arrSelectFields, Page_Command);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Contacts.DirectReports", Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
