<%@ Control Language="c#" AutoEventWireup="false" Codebehind="MassUpdate.ascx.cs" Inherits="SplendidCRM.Accounts.MassUpdate" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
<script runat="server">
/**********************************************************************************************************************
...
<SplendidCRM:MassUpdateButtons ID="ctlDynamicButtons" SubPanel="divAccountsMassUpdate" Title=".LBL_MASS_UPDATE_TITLE" Runat="Server" />

<div id="divAccountsMassUpdate" style='<%= "display:" + (CookieValue("divAccountsMassUpdate") != "1" ? "inline" : "none") %>'>
		...
		<asp:TableRow>
			<asp:TableCell>
				...
				<asp:Table Width="100%" CellPadding="0" CellSpacing="0" runat="server">
					<asp:TableRow>
						<asp:TableCell Width="15%" CssClass="dataLabel"><asp:Label Text='<%# L10n.Term("Accounts.LBL_TYPE") %>' runat="server" /></asp:TableCell>
						<asp:TableCell Width="35%" CssClass="dataField"><asp:DropDownList ID="lstACCOUNT_TYPE" DataValueField="NAME" DataTextField="DISPLAY_NAME" Runat="server" /></asp:TableCell>
						<asp:TableCell Width="15%" CssClass="dataLabel"><asp:Label Text='<%# L10n.Term("Accounts.LBL_INDUSTRY") %>' runat="server" /></asp:TableCell>
