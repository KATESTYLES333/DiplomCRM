...
namespace SplendidCRM.Accounts
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Account ACCOUNTS_ACCOUNT_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Accounts.ERR_ACCOUNT_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] ACCOUNTS_ACCOUNT_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or ACCOUNT_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] ACCOUNTS_NAME_List(string prefixText, int count)
		{
			return ACCOUNTS_ACCOUNT_NAME_List(prefixText, count);
		}

		// 07/27/2010 Paul.  Add methods to be used by Quotes, Orders and Invoices. 
		[WebMethod(EnableSession=true)]
		public string[] ACCOUNTS_BILLING_ACCOUNT_NAME_List(string prefixText, int count)
		{
			return ACCOUNTS_ACCOUNT_NAME_List(prefixText, count);
		}

		[WebMethod(EnableSession=true)]
		public string[] ACCOUNTS_SHIPPING_ACCOUNT_NAME_List(string prefixText, int count)
		{
			return ACCOUNTS_ACCOUNT_NAME_List(prefixText, count);
		}
		...
		[WebMethod(EnableSession=true)]
		public string[] ACCOUNTS_PARENT_NAME_List(string prefixText, int count)
		{
			return ACCOUNTS_ACCOUNT_NAME_List(prefixText, count);
		}
		...
		[WebMethod(EnableSession=true)]
		public Account ACCOUNTS_PARENT_NAME_Get(string sNAME)
		{
			return ACCOUNTS_ACCOUNT_NAME_Get(sNAME);
		}

		// 03/08/2016 Paul.  The HTML5 Offline Client needs the get for the Quotes, Orders Invoices. 
		[WebMethod(EnableSession=true)]
		public Account ACCOUNTS_BILLING_ACCOUNT_NAME_Get(string sNAME)
		{
			return ACCOUNTS_ACCOUNT_NAME_Get(sNAME);
		}
		...
		// 03/08/2016 Paul.  The HTML5 Offline Client needs the get for the Quotes, Orders Invoices. 
		[WebMethod(EnableSession=true)]
		public Account ACCOUNTS_SHIPPING_ACCOUNT_NAME_Get(string sNAME)
		{
			return ACCOUNTS_ACCOUNT_NAME_Get(sNAME);
		}
	}
