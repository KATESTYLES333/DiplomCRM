		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					// 01/13/2016 Paul.  Provide ability to assign and unassign accounts to a lead. 
					case "Leads.Remove":
					{
						Guid gLEAD_ID = Sql.ToGuid(e.CommandArgument);
						if ( bEditView )
						...
						{
							string[] arrID = new string[] { gLEAD_ID.ToString() };
							System.Collections.Stack stk = Utils.FilterByACL_Stack(m_sMODULE, "edit", arrID, "LEADS");
							if ( stk.Count > 0 )
							{
								SqlProcs.spACCOUNTS_LEADS_Delete(gID, gLEAD_ID);
							}
						}
						BindGrid();
						break;
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bEditView && IsPostBack && arrUPDATED.Count > 0 )
					{
						...
						arrSelectFields.Remove("ASSIGNED_USER_ID");
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     ...
						     + "     , @ASSIGNED_USER_ID as ASSIGNED_USER_ID" + ControlChars.CrLf
						...
						Sql.AddParameter(cmd, "@ASSIGNED_USER_ID", Sql.ToGuid   (Page.Items["ASSIGNED_USER_ID"]));
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwACCOUNTS_LEADS", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spACCOUNTS_LEADS_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spACCOUNTS_LEADS_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
			...
			if ( !Sql.IsEmptyString(txtLEAD_ID.Value) )
			{
				try
				{
					...
					if ( arrID != null )
					{
						...
						{
							// 01/13/2016 Paul.  The ACCOUNT_ID field is being modified, so we need the the edit right. 
							System.Collections.Stack stk = Utils.FilterByACL_Stack(m_sMODULE, "edit", arrID, "LEADS");
							if ( stk.Count > 0 )
							{
								DbProviderFactory dbf = DbProviderFactories.GetFactory();
									...
									using ( IDbTransaction trn = Sql.BeginTransaction(con) )
									{
										try
										{
											while ( stk.Count > 0 )
											{
												...
												SqlProcs.spACCOUNTS_LEADS_MassUpdate(sIDs, gID, trn);
											}
											trn.Commit();
										}
										catch(Exception ex)
