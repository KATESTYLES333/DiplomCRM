...
namespace SplendidCRM.Resources
{
	...
	/// </summary>
	public class Notes : SubPanelControl
	{
		...
		protected SplendidCRM.Notes.NewRecord ctlNewRecord   ;
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					{
						arrSelectFields.Remove("NOTE_ID"          );
						arrSelectFields.Remove("NOTE_NAME"        );
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "     , ID                        as NOTE_ID          " + ControlChars.CrLf
						     + "     , NAME                      as NOTE_NAME        " + ControlChars.CrLf
					...
					else
					{
						...
						cmd.CommandText += "   and (PARENT_ID = @PARENT_ID)" + ControlChars.CrLf;
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwRESOURCES_Notes", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								{
									this.CreateEditViewRelationships(dt, "NOTE_ID");
								}
							}
						}
					}
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			/*foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spRESOURCES_NOTES_Delete(gDELETE_ID, gPARENT_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spRESOURCES_NOTES_Update(gUPDATE_ID, gPARENT_ID, trn);
			}*/
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gNOTE_ID) )
			{
				try
				{
					if ( bEditView )
					{
						this.UpdateEditViewRelationship(gNOTE_ID);
					}
					else
					{
						//SqlProcs.spRESOURCES_NOTES_Update(gNOTE_ID, gID);
					}
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwRESOURCES_Notes";
