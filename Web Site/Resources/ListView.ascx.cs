...
namespace SplendidCRM.Resources
{
	...
	public class ListView : SplendidControl
	{
			...
			try
			{
								...
								using (IDbTransaction trn = Sql.BeginTransaction(con))
								{
									try
									{
										while (stk.Count > 0)
										{
											...
											SqlProcs.spRESOURCES_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.partner_ID, ctlMassUpdate.REPORTS_TO_ID, ctlMassUpdate.PRIMARY_TEAM_ID, ctlMassUpdate.TEAM_SET_LIST, ctlMassUpdate.ADD_TEAM_SET, ctlMassUpdate.TAG_SET_NAME, ctlMassUpdate.ADD_TAG_SET, trn);
										}
										trn.Commit();
									}
									catch (Exception ex)
							...
							using (IDbConnection con = dbf.CreateConnection())
							{
								...
								using (IDbTransaction trn = Sql.BeginTransaction(con))
								{
									try
									{
										...
										{
											string sIDs = Utils.BuildMassIDs(stk);
											SqlProcs.spRESOURCES_MassDelete(sIDs, trn);
										}
										trn.Commit();
									}
									catch (Exception ex)
				...
				else if (e.CommandName == "Sync")
				{
					...
					if (arrID != null)
					{
						...
						{
							SqlProcs.spRESOURCES_MassSync(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}
				else if (e.CommandName == "Unsync")
				{
					...
					if (arrID != null)
					{
						...
						if (!Sql.IsEmptyString(sIDs))
						{
							SqlProcs.spRESOURCES_MassUnsync(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}
		...
		{
			//
			// CODEGEN: This Resource is required by the ASP.NET Web Form Designer.
			//
			InitializeComponent();
			base.OnInit(e);
		}
