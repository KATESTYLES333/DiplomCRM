...
namespace SplendidCRM.Resources
{
	...
	public class PopupView : SplendidControl
	{
			...
			try
			{
				...
				else if ( e.CommandName == "NewRecord" )
				{
					...
					if ( mgrAjax != null )
					{
						...
						ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "NewRecord", "SelectResource('" + gID.ToString() + "', '" + Sql.EscapeJavaScript(sNAME) + "');", true);
					}
					else
					{
		...
		protected void grdMain_OnSelectMethod(int nCurrentPageIndex, int nPageSize)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					// 09/09/2009 Paul.  The Resources popup may need to filter on the partner. 
					FilterPartner(cmd);
		...
		private void FilterPartner(IDbCommand cmd)
		{
			// 02/10/2008 Paul.  Allow searching of a specific partner. 
			Guid gpartner_ID = Sql.ToGuid(Request["partner_ID"]);
			// 08/21/2010 Paul.  We need a way to override the previous behavior and filter by the specified partner. 
			bool bFilterPartner = Sql.ToBoolean(Request["FilterPartner"]);
			if ( !bMultiSelect || bFilterPartner )
			{
				if ( !Sql.IsEmptyGuid(gpartner_ID) )
				{
					Sql.AppendParameter(cmd, gpartner_ID, "partner_ID");
					Guid   gPARENT_ID   = gpartner_ID;
					...
					try
					{
						...
						// 06/23/2008 Paul.  Keep the read-only partner name, but remove the change buttons. 
						new DynamicControl(ctlSearchView, "partner_ID"          ).ID      = gPARENT_ID  ;
						new DynamicControl(ctlSearchView, "partner_NAME"        ).Text    = sPARENT_NAME;
						new DynamicControl(ctlSearchView, "partner_ID_btnChange").Visible = false;
						new DynamicControl(ctlSearchView, "partner_ID_btnClear" ).Visible = false;
						if ( !IsPostBack )
						{
							// 08/03/2010 Paul.  We have converted from a Change field to an AutoComplete. 
							// We now need to disable editing the partner name. 
							TextBox partner_NAME = ctlSearchView.FindControl("partner_NAME") as TextBox;
							if ( partner_NAME != null )
							{
								partner_NAME.Enabled = false;
							}
							// 08/03/2010 Paul.  Set the default partner for a new contact. 
				...
				{
					// 07/02/2006 Paul.  Quote needs to select only Resources for a specified Partner. 
					// 06/23/2008 Paul.  Partner name filtering should only
					string spartner_NAME = Sql.ToString(Request["partner_NAME"]);
					new DynamicControl(ctlSearchView, "partner_NAME").Text = spartner_NAME;
					Sql.AppendParameter(cmd, spartner_NAME, Sql.SqlFilterMode.Exact, "partner_NAME");
				}
			}
		}
