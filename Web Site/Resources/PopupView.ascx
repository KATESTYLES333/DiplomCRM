<%@ Control CodeBehind="PopupView.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Resources.PopupView" %>
<script runat="server">
/**********************************************************************************************************************
...
<div id="divPopupView">
	...
	function SelectResource(sPARENT_ID, sPARENT_NAME)
	{
		if ( window.opener != null && window.opener.ChangeResource != null )
		{
			window.opener.ChangeResource(sPARENT_ID, sPARENT_NAME);
			window.close();
		}
		else
		{
	...
	function SelectChecked()
	{
		if ( window.opener != null && window.opener.ChangeResource != null )
		{
			...
			window.opener.ChangeResource(sSelectedItems, '');
			window.close();
		}
		else
		{
	...
	<asp:UpdatePanel UpdateMode="Conditional" Visible='<%# !Sql.ToBoolean(Application["CONFIG.disable_popupview_inline"]) %>' runat="server">
		<ContentTemplate>
			<asp:Button ID="btnCreateInline" CommandName="NewRecord.Show" OnCommand="Page_Command" Text='<%# L10n.Term(m_sMODULE + ".LNK_NEW_CONTACT") %>' CssClass="button" style="margin-bottom: 4px;" Visible="<%# !this.IsMobile %>" runat="server" />
