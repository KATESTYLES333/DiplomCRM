...
namespace SplendidCRM.Resources
{
	...
	/// </summary>
	public class Requests : SubPanelControl
	{
		...
		protected HtmlInputHidden txtREQUEST_ID;
		...
		protected SplendidCRM.Requests.NewRecord ctlNewRecord   ;
        ...
        protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					//	Response.Redirect("~/Requests/edit.aspx?PARENT_ID=" + gID.ToString());
					//	break;
					case "Requests.Edit":
					{
						Guid gREQUEST_ID = Sql.ToGuid(e.CommandArgument);
						Response.Redirect("~/Requests/edit.aspx?ID=" + gREQUEST_ID.ToString());
						break;
					}
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					{
						arrSelectFields.Remove("REQUEST_ID"          );
						arrSelectFields.Remove("REQUEST_NAME"        );
						arrSelectFields.Remove("RESOURCE_ID"              );
						arrSelectFields.Remove("RESOURCE_NAME"            );
						arrSelectFields.Remove("RESOURCE_ASSIGNED_USER_ID");
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "     , ID                        as REQUEST_ID          " + ControlChars.CrLf
						     + "     , NAME                      as REQUEST_NAME        " + ControlChars.CrLf
						     + "     , @RESOURCE_ID               as RESOURCE_ID              " + ControlChars.CrLf
						     + "     , @RESOURCE_NAME             as RESOURCE_NAME            " + ControlChars.CrLf
						     + "     , @RESOURCE_ASSIGNED_USER_ID as RESOURCE_ASSIGNED_USER_ID" + ControlChars.CrLf
						     + "  from vwRequests" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Sql.AddParameter(cmd, "@RESOURCE_ID"              , gID);
						Sql.AddParameter(cmd, "@RESOURCE_NAME"            , Sql.ToString(Page.Items["NAME"            ]));
						Sql.AddParameter(cmd, "@RESOURCE_ASSIGNED_USER_ID", Sql.ToGuid  (Page.Items["ASSIGNED_USER_ID"]));
					...
					else
					{
						...
						cmd.CommandText += "   and (RESOURCE_ID = @RESOURCE_ID or B2C_RESOURCE_ID = @B2C_RESOURCE_ID)" + ControlChars.CrLf;
						Sql.AddParameter(cmd, "@RESOURCE_ID"    , gID);
						Sql.AddParameter(cmd, "@B2C_RESOURCE_ID", gID);
					}
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwRESOURCES_Requests", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								// 10/05/2017 Paul.  Add Archive relationship view. 
								this.ApplyGridViewRules("Resources." + this.LayoutListView, dt);
								...
								{
									this.CreateEditViewRelationships(dt, "REQUEST_ID");
								}
							}
						}
					}
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spREQUESTS_RESOURCES_Delete(gDELETE_ID, gPARENT_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spREQUESTS_RESOURCES_Update(gUPDATE_ID, gPARENT_ID, String.Empty, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			Guid gREQUEST_ID = Sql.ToGuid(txtREQUEST_ID.Value);
			if ( !Sql.IsEmptyGuid(gREQUEST_ID) )
			{
				try
				{
					if ( bEditView )
					{
						this.UpdateEditViewRelationship(gREQUEST_ID);
					}
					else
					{
						SqlProcs.spREQUESTS_RESOURCES_Update(gREQUEST_ID, gID, String.Empty);
					}
					...
					txtREQUEST_ID.Value = String.Empty;
				}
				catch(Exception ex)
				{
					SplendidError.SystemError(new StackTrace(true).GetFrame(0), ex);
			...
			if ( !IsPostBack )
			{
				...
				// 10/05/2017 Paul.  Add Archive relationship view. 
				ctlDynamicButtons.AppendButtons("Resources." + this.LayoutListView, gASSIGNED_USER_ID, gID);
				// 05/01/2013 Paul.  Add Resources field to support B2C. 
				if ( Sql.ToString(Application["CONFIG.BusinessMode"]) == "B2C" )
				{
					ctlNewRecord.B2C_RESOURCE_ID = gID;
				}
				else
				{
					// 09/08/2010 Paul.  Prefill the Partner for the Request. 
					Guid gPARTNER_ID = Sql.ToGuid(Page.Items["PARTNER_ID"]);
					ctlNewRecord.PARTNER_ID  = gPARTNER_ID;
					// 09/08/2010 Paul.  Set the PARENT_ID. 
					ctlNewRecord.PARENT_ID   = gID;
					ctlNewRecord.PARENT_TYPE = "Resources";
				}
			}
		}
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("REQUEST_ID"  );
			arrSelectFields.Add("ASSIGNED_USER_ID");
			// 11/26/2005 Paul.  Add fields early so that sort events will get called. 
			// 06/07/2015 Paul.  Must include Page_Command in order for Preview to fire. 
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwRESOURCES_Requests";
			if ( ArchiveViewExists() )
				m_sVIEW_NAME = m_sVIEW_NAME + "_ARCHIVE";
			this.LayoutListView = m_sMODULE + (ArchiveView() ? ".ArchiveView" : String.Empty);
			this.AppendGridColumns(grdMain, "Resources." + this.LayoutListView, arrSelectFields, Page_Command);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Resources." + this.LayoutListView, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
