...
namespace SplendidCRM.Resources
{
	...
	public class EditView : SplendidControl
	{
						...
						using (IDbConnection con = dbf.CreateConnection())
						{
							...
							using (IDbTransaction trn = Sql.BeginTransaction(con))
							{
								try
								{
									...
									// 02/20/2008 Paul.  PARTNER_NAME was added for importing. 
									// 03/13/2008 Paul.  Remove @PARTNER_NAME and create spResources_Import instead. This will become a pattern for import.
									SqlProcs.spRESOURCES_Update
										...
										, new DynamicControl(this, rowCurrent, "PARTNER_ID").ID
										...
										, new DynamicControl(this, rowCurrent, "RESOURCE_NUMBER").Text
										// 11/30/2017 Paul.  Add ASSIGNED_SET_ID for Dynamic User Assignment. 
										, new DynamicControl(this, rowCurrent, "ASSIGNED_SET_LIST").Text

										, new DynamicControl(this, rowCurrent, "SKYPE").Text
										, new DynamicControl(this, rowCurrent, "CVTOOL_LINK").Text
										, new DynamicControl(this, rowCurrent, "ENGAGED").Checked
										, new DynamicControl(this, rowCurrent, "ENGAGED_TILL").DateValue
										, new DynamicControl(this, rowCurrent, "ENGAGED_JIRAPROJECT").Text
										, new DynamicControl(this, rowCurrent, "SCNSOFT_ACCOUNT").Text
										, trn
										);
		...
		// 08/31/2010 Paul.  Update the Address information if the Partner changes. 
		private void UpdatePartner(Guid gPARTNER_ID, bool bUpdateBilling, bool bUpdateShipping)
		{
			DbProviderFactory dbf = DbProviderFactories.GetFactory();
			using (IDbConnection con = dbf.CreateConnection())
			{
				string sSQL;
				sSQL = "select *         " + ControlChars.CrLf
					 + "  from vwPartners" + ControlChars.CrLf
					 + " where ID = @ID  " + ControlChars.CrLf;
				using (IDbCommand cmd = con.CreateCommand())
				{
					cmd.CommandText = sSQL;
					Sql.AddParameter(cmd, "@ID", gPARTNER_ID);
					con.Open();
					using (IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow))
					{
						if (rdr.Read())
						{
							new DynamicControl(this, "PARTNER_ID").ID = Sql.ToGuid(rdr["ID"]);
							new DynamicControl(this, "PARTNER_NAME").Text = Sql.ToString(rdr["NAME"]);
							if (bUpdateBilling)
							{
			...
			try
			{
				...
				if (!IsPostBack)
				{
					...
					if (!Sql.IsEmptyGuid(gID) || !Sql.IsEmptyGuid(gDuplicateID))
					{
						...
						{
							string sSQL;
							// 02/09/2006 Paul.  SugarCRM uses the Resources_USERS table to allow each user to 
							// choose the Resources they want sync'd with Outlook. 
							// 02/09/2006 Paul.  Need to allow SYNC_USER_ID to be NULL, 
							// otherwise we will not get any results if the contact is not sync'd. 
							// 03/06/2006 Paul.  The join to Resources_USERS must occur external to the view. 
							// This is the only way to ensure that the record is always returned, with the sync flag set. 
							// 04/23/2006 Paul.  Bug fix.  vwRESOURCES_USERS does not have an ID, use RESOURCE_ID instead. 
							// 09/18/2015 Paul.  Add SERVICE_NAME to separate Exchange Folders from Resources Sync. 
							// 11/01/2017 Paul.  Use a module-based flag so that Record Level Security is only enabled when needed. 
							sSQL = "select " + m_sVIEW_NAME + ".*" + ControlChars.CrLf
								 + Sql.AppendRecordLevelSecurityField(m_sMODULE, "edit", m_sVIEW_NAME)
								 + "     , (case when vwRESOURCES_USERS.RESOURCE_ID          is null then 0 else 1 end) as SYNC_CONTACT   " + ControlChars.CrLf
								 + "     , (case when vwRESOURCES_USERS_EXCHANGE.RESOURCE_ID is null then 0 else 1 end) as EXCHANGE_FOLDER" + ControlChars.CrLf
								 + "  from            " + m_sVIEW_NAME + ControlChars.CrLf
								 + "  left outer join vwRESOURCES_USERS                                                                  " + ControlChars.CrLf
								 + "               on vwRESOURCES_USERS.RESOURCE_ID            = " + m_sVIEW_NAME + ".ID                  " + ControlChars.CrLf
								 + "              and vwRESOURCES_USERS.USER_ID               = @SYNC_USER_ID                            " + ControlChars.CrLf
								 + "              and vwRESOURCES_USERS.SERVICE_NAME is null                                             " + ControlChars.CrLf
								 + "  left outer join vwRESOURCES_USERS                         vwRESOURCES_USERS_EXCHANGE                " + ControlChars.CrLf
								 + "               on vwRESOURCES_USERS_EXCHANGE.RESOURCE_ID   = " + m_sVIEW_NAME + ".ID                  " + ControlChars.CrLf
								 + "              and vwRESOURCES_USERS_EXCHANGE.USER_ID      = @SYNC_USER_ID                            " + ControlChars.CrLf
								 + "              and vwRESOURCES_USERS_EXCHANGE.SERVICE_NAME = N'Exchange'                              " + ControlChars.CrLf;
							using (IDbCommand cmd = con.CreateCommand())
							{
								cmd.CommandText = sSQL;
								...
								using (DbDataAdapter da = dbf.CreateDataAdapter())
								{
									...
									using (DataTable dtCurrent = new DataTable())
									{
										...
										if (dtCurrent.Rows.Count > 0 && (SplendidCRM.Security.GetRecordAccess(dtCurrent.Rows[0], m_sMODULE, "edit", "ASSIGNED_USER_ID") >= 0))
										{
											...
											// 08/31/2010 Paul.  Save the PARTNER_ID so that we can detect a change. 
											ViewState["PARTNER_ID"] = Sql.ToGuid(rdr["PARTNER_ID"]);
											...
											{
												new DynamicControl(this, "RESOURCE_NUMBER").Text = String.Empty;
											}
											// 12/09/2008 Paul.  Throw an exception if the record has been edited since the last load. 
											...
											{
												PORTAL_PASSWORD        .Text = sEMPTY_PASSWORD;
												PORTAL_PASSWORD_CONFIRM.Text = sEMPTY_PASSWORD;
											}*/
											// 11/10/2010 Paul.  Apply Business Rules. 
					...
					else
					{
						...
						// 03/27/2006 Paul.  Prepopulate the Partner. 
						Guid gPARENT_ID = Sql.ToGuid(Request["PARENT_ID"]);
						if (!Sql.IsEmptyGuid(gPARENT_ID))
						{
							...
							if (!Sql.IsEmptyGuid(gPARENT_ID) && sMODULE == "Partners")
							{
								...
								// 12/31/2007 Paul.  spacecowboy suggested that we prepopulate the address fields from the partner. 
								DbProviderFactory dbf = DbProviderFactories.GetFactory();
								using (IDbConnection con = dbf.CreateConnection())
								{
									string sSQL;
							...
							else if (!Sql.IsEmptyGuid(gPARENT_ID) && sMODULE == "Resources")
							{
				...
				else
				{
					...
					// 08/31/2010 Paul.  Update the Address if we detect a change in the PARTNER_ID. 
					DynamicControl ctlPARTNER_ID = new DynamicControl(this, "PARTNER_ID");
					if (Sql.ToGuid(ViewState["PARTNER_ID"]) != ctlPARTNER_ID.ID)
					{
						UpdatePartner(ctlPARTNER_ID.ID, true, true);
						ViewState["PARTNER_ID"] = ctlPARTNER_ID.ID;
					}
				}
			}
			catch (Exception ex)
