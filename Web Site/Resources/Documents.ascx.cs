...
namespace SplendidCRM.Resources
{
	...
	public class Documents : SubPanelControl
	{
			...
			try
			{
				switch (e.CommandName)
				{
					...
					case "Documents.Remove":
						{
							Guid gDOCUMENT_ID = Sql.ToGuid(e.CommandArgument);
							SqlProcs.spRESOURCES_DOCUMENTS_Delete(gID, gDOCUMENT_ID);
							BindGrid();
							break;
						}
					case "Documents.GetLatest":
						{
							...
							{
								SqlProcs.spRESOURCES_DOCUMENTS_GetLatest(gID, gDOCUMENT_ID);
							}
							BindGrid();
							break;
						}
			...
			using (IDbConnection con = dbf.CreateConnection())
			{
				...
				using (IDbCommand cmd = con.CreateCommand())
				{
					...
					else
					{
						...
						Sql.AppendParameter(cmd, gID, "RESOURCE_ID");
					}
					// 10/05/2017 Paul.  Move Last Sort to the database.
					cmd.CommandText += grdMain.OrderByClause("DOCUMENT_NAME", "asc");

					if (bDebug)
						RegisterClientScriptBlock("vwRESOURCES_DOCUMENTS", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach (Guid gDELETE_ID in arrDELETED)
			{
				if (!Sql.IsEmptyGuid(gDELETE_ID))
					SqlProcs.spRESOURCES_DOCUMENTS_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach (Guid gUPDATE_ID in arrUPDATED)
			{
				if (!Sql.IsEmptyGuid(gUPDATE_ID))
					SqlProcs.spRESOURCES_DOCUMENTS_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!Sql.IsEmptyGuid(gDOCUMENT_ID))
			{
				try
				{
					...
					{
						SqlProcs.spRESOURCES_DOCUMENTS_Update(gID, gDOCUMENT_ID);
					}
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("RESOURCE_ASSIGNED_USER_ID");
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwRESOURCES_DOCUMENTS";
