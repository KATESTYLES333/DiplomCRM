...
namespace SplendidCRM.Resources
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Resource Resources_RESOURCE_LAST_NAME_Get(string sLAST_NAME)
		{
			...
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "Resources", "list");
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Resources.ERR_RESOURCE_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] Resources_RESOURCE_LAST_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or RESOURCE_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] Resources_LAST_NAME_List(string prefixText, int count)
		{
			return Resources_RESOURCE_LAST_NAME_List(prefixText, count);
		}

		// 07/27/2010 Paul.  Since we are using the ContextKey for the AutoComplete, we need to also use the Partner Name with the Get. 
		...
		[WebMethod(EnableSession=true)]
		public ResourceName Resources_RESOURCE_NAME_Get(string sNAME)
		{
			ResourceName item = new ResourceName();
			//try
			{
				if ( !Security.IsAuthenticated() )
					throw(new Exception("Authentication required"));
		...
		[WebMethod(EnableSession=true)]
		public string[] Resources_RESOURCE_NAME_List(string prefixText, int count)
		{
			string[] arrItems = new string[0];
			try
			{
		...
		// so the field name can be NAME or RESOURCE_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] Resources_NAME_List(string prefixText, int count)
		{
			return Resources_RESOURCE_NAME_List(prefixText, count);
		}

		// 09/16/2010 Paul.  We need separate functions for Resources_RESOURCE_NAME_Get, and Resources_BILLING_RESOURCE_NAME_Get. 
		[WebMethod(EnableSession=true)]
		public ResourceName Resources_BILLING_RESOURCE_NAME_Get(string sNAME, string contextKey)
		{
			ResourceName item = new ResourceName();
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						if ( !Sql.IsEmptyString(contextKey) )
							Sql.AppendParameter(cmd, contextKey, "PARTNER_NAME");
						// 07/12/2010 Paul.  Allow fuzzy searching during AutoComplete. 
						Sql.AppendParameter(cmd, sNAME, (Sql.ToBoolean(Application["CONFIG.AutoComplete.Contains"]) ? Sql.SqlFilterMode.Contains : Sql.SqlFilterMode.StartsWith), "NAME");
						// 07/02/2007 Paul.  Sort is important so that the first match is selected. 
						cmd.CommandText += " order by NAME" + ControlChars.CrLf;
		...
		[WebMethod(EnableSession=true)]
		public string[] Resources_BILLING_RESOURCE_NAME_List(string prefixText, int count, string contextKey)
		{
			string[] arrItems = new string[0];
			try
			{
		...
		/*
		[WebMethod(EnableSession=true)]
		public string[] Resources_BILLING_RESOURCE_NAME_List(string prefixText, int count)
		{
			return Resources_RESOURCE_NAME_List(prefixText, count, String.Empty);
		}
		*/

		// 09/16/2010 Paul.  We need separate functions for Resources_RESOURCE_NAME_Get, and Resources_BILLING_RESOURCE_NAME_Get. 
		[WebMethod(EnableSession=true)]
		public ResourceName Resources_SHIPPING_RESOURCE_NAME_Get(string sNAME, string contextKey)
		{
			return Resources_BILLING_RESOURCE_NAME_Get(sNAME, contextKey);
		}

		[WebMethod(EnableSession=true)]
		public string[] Resources_SHIPPING_RESOURCE_NAME_List(string prefixText, int count, string contextKey)
		{
			return Resources_BILLING_RESOURCE_NAME_List(prefixText, count, contextKey);
		}
		/*
		[WebMethod(EnableSession=true)]
		public string[] Resources_SHIPPING_RESOURCE_NAME_List(string prefixText, int count)
		{
			return Resources_RESOURCE_NAME_List(prefixText, count, String.Empty);
		}
		*/
		...
		[WebMethod(EnableSession=true)]
		public ResourceName Resources_REPORTS_TO_NAME_Get(string sNAME)
		{
			return Resources_RESOURCE_NAME_Get(sNAME);
		}

		[WebMethod(EnableSession=true)]
		public string[] Resources_REPORTS_TO_NAME_List(string prefixText, int count)
		{
			return Resources_RESOURCE_NAME_List(prefixText, count);
		}
	}
