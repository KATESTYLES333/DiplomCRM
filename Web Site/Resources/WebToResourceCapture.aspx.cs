...
{
	public partial class WebToResourceCapture : SplendidCRM.SplendidPage
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				string sCUSTOM_MODULE = "Resources";
					...
					using ( IDbTransaction trn = Sql.BeginTransaction(con) )
					{
						try
						{
							...
							SqlProcs.spRESOURCES_Update
								...
								, String.Empty // RESOURCE_NUMBER
								// 11/30/2017 Paul.  Add ASSIGNED_SET_ID for Dynamic User Assignment. 
								, String.Empty // ASSIGNED_SET_LIST
                                , Sql.ToString(Request["SKYPE"])
                                , Sql.ToString(Request["CVTOOL_LINK"])
                                , Sql.ToBoolean(Request["ENGAGED"])
                                , Sql.ToDateTime(Request["ENGAGED_TILL"])
                                , Sql.ToString(Request["ENGAGED_JIRAPROJECT"])
                                , Sql.ToString(Request["SCNSOFT_ACCOUNT"])
                                , trn
								);
