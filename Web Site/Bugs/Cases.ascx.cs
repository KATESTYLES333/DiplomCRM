	...
	public class Cases : SubPanelControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				...
				{
					//case "Cases.Create":
					//	Response.Redirect("~/Cases/edit.aspx?PARENT_ID=" + gID.ToString());
					//	break;
					case "Cases.Edit":
					{
					...
					case "Cases.Remove":
					{
						...
						if ( bEditView )
						{
							this.DeleteEditViewRelationship(gCASE_ID);
						}
						else
						{
							SqlProcs.spCASES_BUGS_Delete(gCASE_ID, gID);
						}
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwBUGS_CASES", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			{
				// 08/05/2010 Paul.  The Bug is the parent, so it should be the second parameter. 
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spCASES_BUGS_Delete(gDELETE_ID, gPARENT_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				...
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spCASES_BUGS_Update(gUPDATE_ID, gPARENT_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gCASE_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spCASES_BUGS_Update(gCASE_ID, gID);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
		...
		private void InitializeComponent()
		{
			...
			// 05/08/2008 Paul.  CASE_ID is used in the Grid. 
