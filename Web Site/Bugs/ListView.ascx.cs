...
namespace SplendidCRM.Bugs
{
	...
	public class ListView : SplendidControl
	{
			...
			try
			{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										while ( stk.Count > 0 )
										{
											...
											SqlProcs.spBUGS_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.STATUS, ctlMassUpdate.PRIORITY, ctlMassUpdate.RESOLUTION, ctlMassUpdate.TYPE, ctlMassUpdate.SOURCE, ctlMassUpdate.PRODUCT_CATEGORY, ctlMassUpdate.PRIMARY_TEAM_ID, ctlMassUpdate.TEAM_SET_LIST, ctlMassUpdate.ADD_TEAM_SET, ctlMassUpdate.TAG_SET_NAME, ctlMassUpdate.ADD_TAG_SET, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
							...
							using ( IDbConnection con = dbf.CreateConnection() )
							{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										...
										{
											string sIDs = Utils.BuildMassIDs(stk);
											SqlProcs.spBUGS_MassDelete(sIDs, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
				...
				else if ( e.CommandName == "Unsync" )
				{
					...
					if ( arrID != null )
					{
						...
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spBUGS_MassUnsync(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						grdMain.OrderByClause("BUG_NUMBER", "desc");
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("BUG_NUMBER");
