...
// 10/18/2015 Paul.  This file is in the Bugs namespace, not Opportunities namespace. 
namespace SplendidCRM.Bugs
{
	...
	public class Accounts : SubPanelControl
	{
			...
			try
			{
				switch ( e.CommandName )
				{
					...
					case "Accounts.Remove":
					{
						...
						{
							SqlProcs.spACCOUNTS_BUGS_Delete(gACCOUNT_ID, gID);
						}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bEditView && IsPostBack && arrUPDATED.Count > 0 )
					{
						...
						arrSelectFields.Remove("BUG_ASSIGNED_USER_ID"    );
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "     , ID                        as ACCOUNT_ID              " + ControlChars.CrLf
						     + "     , NAME                      as ACCOUNT_NAME            " + ControlChars.CrLf
						     + "     , @BUG_ID                   as BUG_ID                  " + ControlChars.CrLf
						     + "     , @BUG_NAME                 as BUG_NAME                " + ControlChars.CrLf
						     + "     , @BUG_ASSIGNED_USER_ID     as BUG_ASSIGNED_USER_ID    " + ControlChars.CrLf
						     + "  from vwACCOUNTS" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Sql.AddParameter(cmd, "@BUG_ID"              , gID);
						Sql.AddParameter(cmd, "@BUG_NAME"            , Sql.ToString(Page.Items["NAME"            ]));
						Sql.AddParameter(cmd, "@BUG_ASSIGNED_USER_ID", Sql.ToGuid  (Page.Items["ASSIGNED_USER_ID"]));
					...
					else
					{
						...
						Sql.AppendParameter(cmd, gID, "BUG_ID");
					}
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwBUGS_ACCOUNTS", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								// 10/05/2017 Paul.  Add Archive relationship view. 
								this.ApplyGridViewRules("Bugs." + this.LayoutListView, dt);
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spACCOUNTS_BUGS_Delete(gDELETE_ID, gPARENT_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spACCOUNTS_BUGS_Update(gUPDATE_ID, gPARENT_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gACCOUNT_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spACCOUNTS_BUGS_Update(gACCOUNT_ID, gID);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
			...
			if ( !IsPostBack )
			{
				...
				// 10/05/2017 Paul.  Add Archive relationship view. 
				ctlDynamicButtons.AppendButtons("Bugs." + this.LayoutListView, gASSIGNED_USER_ID, gID);
				// 02/21/2010 Paul.  The parent needs to be initialized when the page first loads. 
				ctlNewRecord.PARENT_ID   = gID;
				// 03/24/2011 Paul.  Need to set the parent type properly so that the ACCOUNTS_BUGS relationship will get created. 
				ctlNewRecord.PARENT_TYPE = "Bugs";
			}
		}
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("BUG_ASSIGNED_USER_ID");
			// 11/26/2005 Paul.  Add fields early so that sort events will get called. 
			// 06/07/2015 Paul.  Must include Page_Command in order for Preview to fire. 
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwBUGS_ACCOUNTS";
			if ( ArchiveViewExists() )
				m_sVIEW_NAME = m_sVIEW_NAME + "_ARCHIVE";
			this.LayoutListView = m_sMODULE + (ArchiveView() ? ".ArchiveView" : String.Empty);
			this.AppendGridColumns(grdMain, "Bugs." + this.LayoutListView, arrSelectFields, Page_Command);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Bugs." + this.LayoutListView, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
