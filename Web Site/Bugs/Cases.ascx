...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtons" Module="Cases" SubPanel="divBugsCases" Title="Cases.LBL_MODULE_NAME" Runat="Server" />
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript("Cases.NTC_REMOVE_FROM_BUG_CONFIRMATION") %>')">
						...
						<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, "Bugs", "remove", "BUG_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Cases.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "CASE_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_REMOVE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
