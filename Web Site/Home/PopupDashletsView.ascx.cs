...
{
	/// <summary>
	///		Summary description for PopupDashletsView.
	/// </summary>
	public class PopupDashletsView : SplendidControl
	{
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select *                 " + ControlChars.CrLf
					     + "  from vwDASHLETS_CATALOG" + ControlChars.CrLf
					     + " where 1 = 1             " + ControlChars.CrLf;
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						cmd.CommandText = sSQL;
						// 09/12/2009 Paul.  Filter the catalog by category. 
						string sCategory = Sql.ToString(Request["Category"]);
						Sql.AppendParameter(cmd, sCategory, "CATEGORY");
						cmd.CommandText += " order by MODULE_NAME, TITLE" + ControlChars.CrLf;
