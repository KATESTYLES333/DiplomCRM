...
namespace SplendidCRM.Projects
{
	...
	public class ListView : SplendidControl
	{
			...
			try
			{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										while ( stk.Count > 0 )
										{
											...
											SqlProcs.spPROJECTS_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.PRIMARY_TEAM_ID, ctlMassUpdate.TEAM_SET_LIST, ctlMassUpdate.ADD_TEAM_SET, ctlMassUpdate.START_DATE, ctlMassUpdate.END_DATE, ctlMassUpdate.STATUS, ctlMassUpdate.PRIORITY, ctlMassUpdate.TAG_SET_NAME, ctlMassUpdate.ADD_TAG_SET, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
							...
							using ( IDbConnection con = dbf.CreateConnection() )
							{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										...
										{
											string sIDs = Utils.BuildMassIDs(stk);
											SqlProcs.spPROJECTS_MassDelete(sIDs, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
				...
				else if ( e.CommandName == "Sync" )
				{
					...
					if ( arrID != null )
					{
						...
						{
							SqlProcs.spPROJECT_MassSync(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}
				else if ( e.CommandName == "Unsync" )
				{
					...
					if ( arrID != null )
					{
						...
						if ( !Sql.IsEmptyString(sIDs) )
						{
							SqlProcs.spPROJECT_MassUnsync(sIDs);
							Response.Redirect("default.aspx");
						}
					}
				}
		...
		private void InitializeComponent()
		{
			...
			m_sMODULE = "Project";
			SetMenu(m_sMODULE);
			// 04/27/2006 Paul.  The module is called Project, but we use the plural version in our lists.
