						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
									...
									SqlProcs.spPROJECTS_Update(ref gID
										...
										, new DynamicControl(this, rowCurrent, "ESTIMATED_START_DATE").DateValue
										, new DynamicControl(this, rowCurrent, "ESTIMATED_END_DATE"  ).DateValue
										, new DynamicControl(this, rowCurrent, "STATUS"              ).Text
										, new DynamicControl(this, rowCurrent, "PRIORITY"            ).Text
				...
				if ( !IsPostBack )
				{
					...
					if ( !Sql.IsEmptyGuid(gID) || !Sql.IsEmptyGuid(gDuplicateID) )
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							sSQL = "select vwPROJECTS_Edit.*                                                        " + ControlChars.CrLf
							     + "     , (case when vwPROJECTS_USERS.PROJECT_ID is null then 0 else 1 end) as EXCHANGE_FOLDER" + ControlChars.CrLf
							     + "  from            vwPROJECTS_Edit                                               " + ControlChars.CrLf
							     + "  left outer join vwPROJECTS_USERS                                              " + ControlChars.CrLf
							     + "               on vwPROJECTS_USERS.PROJECT_ID = vwPROJECTS_Edit.ID              " + ControlChars.CrLf
