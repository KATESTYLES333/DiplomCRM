...
namespace SplendidCRM.Projects
{
	...
	/// </summary>
	public class Partners : SubPanelControl
	{
		...
		protected HtmlInputHidden txtPARTNER_ID  ;
		...
		protected SplendidCRM.Partners.NewRecord ctlNewRecord   ;
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					//	Response.Redirect("~/Partners/edit.aspx?PARENT_ID=" + gID.ToString());
					//	break;
					case "Partners.Edit":
					{
						Guid gPARTNER_ID = Sql.ToGuid(e.CommandArgument);
						Response.Redirect("~/Partners/edit.aspx?ID=" + gPARTNER_ID.ToString());
						break;
					}
					case "Partners.Remove":
					{
						...
						if ( bEditView )
						{
							this.DeleteEditViewRelationship(gPARTNER_ID);
						}
						else
						{
							// 09/08/2012 Paul.  Project Relations data for Partners, Bugs, Cases, Resources, Requests and Quotes moved to separate tables. 
							SqlProcs.spPROJECTS_Partners_Delete(gID, gPARTNER_ID);
						}
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					{
						arrSelectFields.Remove("PARTNER_ID"              );
						arrSelectFields.Remove("PARTNER_NAME"            );
						arrSelectFields.Remove("PROJECT_ID"              );
						arrSelectFields.Remove("PROJECT_NAME"            );
						arrSelectFields.Remove("PROJECT_ASSIGNED_USER_ID");
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "     , ID                        as PARTNER_ID              " + ControlChars.CrLf
						     + "     , NAME                      as PARTNER_NAME            " + ControlChars.CrLf
						     + "     , @PROJECT_ID               as PROJECT_ID              " + ControlChars.CrLf
						     + "     , @PROJECT_NAME             as PROJECT_NAME            " + ControlChars.CrLf
						     + "     , @PROJECT_ASSIGNED_USER_ID as PROJECT_ASSIGNED_USER_ID" + ControlChars.CrLf
						     + "  from vwPartners" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Sql.AddParameter(cmd, "@PROJECT_ID"              , gID);
						Sql.AddParameter(cmd, "@PROJECT_NAME"            , Sql.ToString(Page.Items["NAME"            ]));
						Sql.AddParameter(cmd, "@PROJECT_ASSIGNED_USER_ID", Sql.ToGuid  (Page.Items["ASSIGNED_USER_ID"]));
					...
					else
					{
						...
						Sql.AppendParameter(cmd, gID, "PROJECT_ID");
					}
					// 06/20/2010 Paul.  Allow searching of the subpanel. 
					ctlSearchView.SqlSearchClause(cmd);
					// 04/26/2008 Paul.  Move Last Sort to the database.
					cmd.CommandText += grdMain.OrderByClause("PARTNER_NAME", "asc");

					if ( bDebug )
						RegisterClientScriptBlock("vwPROJECTS_Partners", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								// 10/05/2017 Paul.  Add Archive relationship view. 
								this.ApplyGridViewRules("Project." + this.LayoutListView, dt);
								...
								{
									this.CreateEditViewRelationships(dt, "PARTNER_ID");
								}
							}
						}
					}
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				...
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spPROJECTS_Partners_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				...
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spPROJECTS_Partners_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			Guid gPARTNER_ID = Sql.ToGuid(txtPARTNER_ID.Value);
			if ( !Sql.IsEmptyGuid(gPARTNER_ID) )
			{
				try
				{
					if ( bEditView )
					{
						this.UpdateEditViewRelationship(gPARTNER_ID);
					}
					else
					{
						// 09/08/2012 Paul.  Project Relations data for Partners, Bugs, Cases, Resources, Requests and Quotes moved to separate tables. 
						SqlProcs.spPROJECTS_Partners_Update(gID, gPARTNER_ID);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
					// 05/16/2008 Paul.  If we are not going to redirect,then we must clear the value. 
					txtPARTNER_ID.Value = String.Empty;
				}
				catch(Exception ex)
				{
					SplendidError.SystemError(new StackTrace(true).GetFrame(0), ex);
			...
			if ( !IsPostBack )
			{
				...
				// 10/05/2017 Paul.  Add Archive relationship view. 
				ctlDynamicButtons.AppendButtons("Project." + this.LayoutListView, gASSIGNED_USER_ID, gID);
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("PARTNER_ID"              );
			arrSelectFields.Add("PARTNER_NAME"            );
			arrSelectFields.Add("ASSIGNED_USER_ID"        );
			arrSelectFields.Add("PROJECT_ASSIGNED_USER_ID");
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwPROJECTS_Partners";
			if ( ArchiveViewExists() )
				m_sVIEW_NAME = m_sVIEW_NAME + "_ARCHIVE";
			this.LayoutListView = m_sMODULE + (ArchiveView() ? ".ArchiveView" : String.Empty);
			this.AppendGridColumns(grdMain, "Project." + this.LayoutListView, arrSelectFields, Page_Command);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Project." + this.LayoutListView, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
