...
namespace SplendidCRM.Projects
{
	...
	public class Activities : SplendidControl
	{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				else
				{
					sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
					     + "  from vwPROJECTS_ACTIVITIES_OPEN" + ControlChars.CrLf;
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						if ( bDebug )
							RegisterClientScriptBlock("vwPROJECTS_ACTIVITIES_OPEN", Sql.ClientScriptBlock(cmd));
				...
				// 10/05/2017 Paul.  Add Archive relationship view. 
				sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
				     + "  from vwPROJECTS_ACTIVITIES" + (ArchiveViewEnabled() ? "_ARCHIVE" : "_HISTORY") + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					cmd.CommandText = sSQL;
					// 11/27/2006 Paul.  Make sure to filter relationship data based on team access rights. 
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwPROJECTS_ACTIVITIES_HISTORY", Sql.ClientScriptBlock(cmd));
		...
		private void InitializeComponent()
		{
			...
			// 09/06/2006 Paul.  The GRID_NAME was changed in the database to be more consistent, but not here. 
			// 06/07/2015 Paul.  Must include Page_Command in order for Preview to fire. 
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwPROJECTS_ACTIVITIES";
			if ( !ArchiveViewEnabled() )
