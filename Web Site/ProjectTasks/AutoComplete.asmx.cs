...
namespace SplendidCRM.ProjectTask
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public ProjectTask PROJECT_TASK_PROJECT_TASK_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select ID        " + ControlChars.CrLf
					     ...
					     + "  from vwPROJECT_TASK" + ControlChars.CrLf;
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						cmd.CommandText = sSQL;
						Security.Filter(cmd, "ProjectTask", "list");
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("ProjectTask.ERR_PROJECT_TASK_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] PROJECT_TASK_PROJECT_TASK_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or PROJECT_TASK_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] PROJECT_TASK_NAME_List(string prefixText, int count)
		{
			return PROJECT_TASK_PROJECT_TASK_NAME_List(prefixText, count);
		}
	}
}
