...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtons" Module="Opportunities" SubPanel="divDocumentsOpportunities" Title="Opportunities.LBL_MODULE_NAME" Runat="Server" />
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript(".NTC_DELETE_CONFIRMATION") %>')">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "remove", "ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Opportunities.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "OPPORTUNITY_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' SkinID="delete_inline" Runat="server" />
						<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "remove", "ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Opportunities.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "OPPORTUNITY_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_REMOVE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
