...
namespace SplendidCRM.Documents
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public DocumentName DOCUMENTS_DOCUMENT_NAME_Get(string sDOCUMENT_NAME)
		{
			DocumentName item = new DocumentName();
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select ID        " + ControlChars.CrLf
					     + "     , DOCUMENT_NAME      " + ControlChars.CrLf
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Sql.AppendParameter(cmd, sDOCUMENT_NAME, (Sql.ToBoolean(Application["CONFIG.AutoComplete.Contains"]) ? Sql.SqlFilterMode.Contains : Sql.SqlFilterMode.StartsWith), "DOCUMENT_NAME");
						// 07/02/2007 Paul.  Sort is important so that the first match is selected. 
						cmd.CommandText += " order by DOCUMENT_NAME" + ControlChars.CrLf;
						using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
						{
							if ( rdr.Read() )
							{
								item.ID   = Sql.ToGuid   (rdr["ID"  ]);
								item.DOCUMENT_NAME = Sql.ToString (rdr["DOCUMENT_NAME"]);
							}
						}
					}
				}
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Documents.ERR_DOCUMENT_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] DOCUMENTS_DOCUMENT_NAME_List(string prefixText, int count)
		{
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "Documents", "list");
						// 07/12/2010 Paul.  Allow fuzzy searching during AutoComplete. 
						Sql.AppendParameter(cmd, prefixText, (Sql.ToBoolean(Application["CONFIG.AutoComplete.Contains"]) ? Sql.SqlFilterMode.Contains : Sql.SqlFilterMode.StartsWith), "DOCUMENT_NAME");
						cmd.CommandText += " order by DOCUMENT_NAME" + ControlChars.CrLf;
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								for ( int i=0; i < dt.Rows.Count; i++ )
									arrItems[i] = Sql.ToString(dt.Rows[i]["DOCUMENT_NAME"]);
							}
						}
					}
				}
		...
		// so the field name can be NAME or DOCUMENT_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] DOCUMENTS_NAME_List(string prefixText, int count)
		{
			return DOCUMENTS_DOCUMENT_NAME_List(prefixText, count);
		}
	}
}
