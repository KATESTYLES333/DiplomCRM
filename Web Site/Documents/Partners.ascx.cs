...
namespace SplendidCRM.Documents
{
	...
	public class Partners : SubPanelControl
	{
			...
			try
			{
				switch ( e.CommandName )
				{
					case "Documents.GetLatest":
					{
						...
						SqlProcs.spPartners_DOCUMENTS_GetLatest(gPARTNER_ID, gID);
						BindGrid();
						break;
					}
					case "Partners.Edit":
					...
					case "Partners.Create":
						if ( this.IsMobile || Sql.ToBoolean(Application["CONFIG.disable_editview_inline"]) )
							Response.Redirect("~/Partners/edit.aspx?PARENT_ID=" + gID.ToString()); //D.T.: WAS m_sMODULE
                        else
						{
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwDOCUMENTS_Partners", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spPartners_DOCUMENTS_Delete(gDELETE_ID, gPARENT_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spPartners_DOCUMENTS_Update(gUPDATE_ID, gPARENT_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gPARTNER_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spPartners_DOCUMENTS_Update(gPARTNER_ID, gID);
					}
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwDOCUMENTS_Partners";
