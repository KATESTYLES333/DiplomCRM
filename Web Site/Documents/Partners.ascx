<%@ Control CodeBehind="Partners.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Documents.Partners" %>
<script runat="server">
/**********************************************************************************************************************
...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtons" Module="Partners" SubPanel="divDocumentsPartners" Title="Partners.LBL_MODULE_NAME" Runat="Server" />

<div id="divDocumentsPartners" style='<%= "display:" + (CookieValue("divDocumentsPartners") != "1" ? "inline" : "none") %>'>
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript(".NTC_DELETE_CONFIRMATION") %>')">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "remove", "ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Partners.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "PARTNER_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' SkinID="delete_inline" Runat="server" />
						<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, m_sMODULE, "remove", "ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Partners.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "PARTNER_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_REMOVE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
