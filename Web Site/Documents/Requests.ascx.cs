...
namespace SplendidCRM.Documents
{
	...
	public class Requests : SubPanelControl
	{
			...
			try
			{
				switch ( e.CommandName )
				{
					case "Documents.GetLatest":
					{
						...
						SqlProcs.spRequests_DOCUMENTS_GetLatest(gREQUEST_ID, gID);
						BindGrid();
						break;
					}
					case "Requests.Edit":
					...
					case "Requests.Create":
						if ( this.IsMobile || Sql.ToBoolean(Application["CONFIG.disable_editview_inline"]) )
							Response.Redirect("~/Requests/edit.aspx?PARENT_ID=" + gID.ToString()); //was m_sMODULE
                        else
						{
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwDOCUMENTS_Requests", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spRequests_DOCUMENTS_Delete(gDELETE_ID, gPARENT_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spRequests_DOCUMENTS_Update(gUPDATE_ID, gPARENT_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gREQUEST_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spRequests_DOCUMENTS_Update(gREQUEST_ID, gID);
					}
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwDOCUMENTS_Requests";
