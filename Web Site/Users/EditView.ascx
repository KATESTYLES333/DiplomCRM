...
<script type="text/javascript">
	...
	function Office365Authorize()
	{
		...
		var redirect        = '<%= Request.Url.Scheme + "://" + Request.Url.Host + Sql.ToString(Application["rootURL"]) + "OAuth/Office365Landing.aspx" %>';
		...
		var authenticateUrl = 'https://login.microsoftonline.com/common/oauth2/authorize?response_type=code&client_id=' + client_id + '&redirect_uri=' + encodeURI(redirect) + '&resource=' + encodeURI(resource) + '&state=' + state;
...
<div id="divMain">
	...
	<SplendidCRM:FacebookButtons ID="ctlFacebookButtons" Visible="false" Runat="Server" />
	...
	<asp:Table ID="tblSmtpPanel" SkinID="tabForm" runat="server">
		<asp:TableRow>
			<asp:TableCell>
				...
				<asp:Button ID="btnSmtpTest" CommandName="Smtp.Test"  OnCommand="Page_Command" style="margin-top: 4px;" CssClass="button" Text='<%# "  " + L10n.Term("Configurator.LBL_EMAIL_TEST_OUTBOUND_SETTINGS") + "  " %>' Runat="server" />
				&nbsp;
	...
	<SplendidCRM:DynamicButtons ID="ctlFooterButtons" Visible="false" Runat="Server" />
	<asp:Table ID="tblICloudPanel" SkinID="tabForm" Visible="false" runat="server">
		<asp:TableRow>
			<asp:TableCell>
				...
				<asp:Button ID="btnICloudTest" CommandName="iCloud.Test"  OnCommand="Page_Command" style="margin-top: 4px;" CssClass="button" Text='<%# "  " + L10n.Term("Configurator.LBL_EMAIL_TEST_OUTBOUND_SETTINGS") + "  " %>' Runat="server" />
				&nbsp;
