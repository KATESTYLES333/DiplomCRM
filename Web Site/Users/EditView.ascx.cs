	...
	public class EditView : SplendidControl
	{
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			...
			else if ( e.CommandName == "iCloud.Test" )
			{
				try
				{
					...
					if ( txtICLOUD_USERNAME != null && txtICLOUD_PASSWORD != null )
					{
						...
						iCloudSync.Validate_iCloud(Application, txtICLOUD_USERNAME.Text, sICLOUD_PASSWORD, sbErrors);
			...
			else if ( e.CommandName == "GoogleApps.Authorize" )
			{
				try
				{
					...
					StringBuilder sbErrors = new StringBuilder();
					new DynamicControl(this, "EMAIL1").Text = SplendidCRM.GoogleApps.GetEmailAddress(Application, gID, sbErrors);
				}
				catch(Exception ex)
				{
			...
			else if ( e.CommandName == "Office365.Authorize" )
			{
				try
				{
					...
					Office365AccessToken token = SplendidCRM.ActiveDirectory.Office365AcquireAccessToken(Context, sOAuthClientID, sOAuthClientSecret, gID, OAUTH_CODE.Text);
					// 01/19/2017 Paul.  We want an OUTBOUND_EMAILS mapping to the office365 OAuth record. 
					...
					if ( profile != null )
					{
						new DynamicControl(this, "EMAIL1").Text = Sql.ToString(profile.EmailAddress);
					}
				}
				catch(Exception ex)
				{
			...
			else if ( e.CommandName == "Office365.Test" )
			{
				try
				{
					...
					string sFROM_ADDR = new DynamicControl(this, "EMAIL1").Text;
					string sFROM_NAME = (txtFIRST_NAME.Text + " " + txtLAST_NAME.Text).Trim();
					ExchangeUtils.SendTestMessage(Application, gID, sFROM_ADDR, sFROM_NAME, sFROM_ADDR, sFROM_NAME);
		...
		protected void MAIL_SENDTYPE_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			if ( lstMAIL_SENDTYPE != null )
			{
				...
				tblGoogleAppsPanel.Visible = (tblGoogleAppsOptions.Rows.Count > 1) && !Sql.IsEmptyGuid(gID);
				tblICloudPanel    .Visible = (tblICloudOptions    .Rows.Count > 1) && !Sql.IsEmptyGuid(gID);
				// 09/05/2015 Paul.  Google now uses OAuth 2.0. 
