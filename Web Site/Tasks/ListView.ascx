<%@ Control CodeBehind="ListView.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Tasks.ListView" %>
<script runat="server">
/**********************************************************************************************************************
...
<div id="divListView">
	...
	<SplendidCRM:HeaderButtons ID="ctlModuleHeader" Module="Tasks" Title=".moduleList.Home" EnablePrint="true" HelpName="index" EnableHelp="true" Runat="Server" />

	<%@ Register TagPrefix="SplendidCRM" Tagname="SearchView" Src="~/_controls/SearchView.ascx" %>
	<SplendidCRM:SearchView ID="ctlSearchView" Module="Tasks" Visible="<%# !PrintView %>" Runat="Server" />

	<%@ Register TagPrefix="SplendidCRM" Tagname="ExportHeader" Src="~/_controls/ExportHeader.ascx" %>
	<SplendidCRM:ExportHeader ID="ctlExportHeader" Module="Tasks" Title="Tasks.LBL_LIST_FORM_TITLE" Runat="Server" />
	...
	<SplendidCRM:SplendidGrid id="grdMain" SkinID="grdListView" AllowPaging="<%# !PrintView %>" EnableViewState="true" runat="server">
		<Columns>
			...
			<asp:TemplateColumn HeaderText="Tasks.LBL_LIST_CLOSE" SortExpression="STATUS" ItemStyle-Width="1%">
				<ItemTemplate>
					<asp:ImageButton CommandName="Task.Close" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ID") %>' Visible='<%# Sql.ToBoolean(DataBinder.Eval(Container.DataItem, "CAN_CLOSE")) %>' OnCommand="Page_Command" AlternateText='<%# L10n.Term("Tasks.LBL_LIST_CLOSE") %>' SkinID="close_inline" Runat="server" />
				</ItemTemplate>
			</asp:TemplateColumn>
