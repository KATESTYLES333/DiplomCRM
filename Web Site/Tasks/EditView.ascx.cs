							...
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
									...
									SqlProcs.spTASKS_Update
										...
										,(new DynamicControl(this, rowCurrent, "REMINDER_TIME"      ).IntegerValue > 0 ? new DynamicControl(this, rowCurrent, "REMINDER_TIME"      ).IntegerValue : 0)
										,(new DynamicControl(this, rowCurrent, "EMAIL_REMINDER_TIME").IntegerValue > 0 ? new DynamicControl(this, rowCurrent, "EMAIL_REMINDER_TIME").IntegerValue : 0)
										,(new DynamicControl(this, rowCurrent, "SMS_REMINDER_TIME"  ).IntegerValue > 0 ? new DynamicControl(this, rowCurrent, "SMS_REMINDER_TIME"  ).IntegerValue : 0)
				...
				if ( !IsPostBack )
				{
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 && (SplendidCRM.Security.GetRecordAccess(dtCurrent.Rows[0], m_sMODULE, "edit", "ASSIGNED_USER_ID") >= 0) )
										{
											...
											// 02/04/2011 Paul.  We gave the PARENT_TYPE a unique name, but we need to update all EditViews and NewRecords. 
											DropDownList lstPARENT_TYPE = FindControl("PARENT_ID_PARENT_TYPE") as DropDownList;
											...
											// 09/30/2008 Jake.  Both Close and Completed are used in lists. 
											if ( Sql.ToString(Request["Status"]) == "Close" || Sql.ToString(Request["Status"]) == "Completed" )
											{
					...
					else
					{
						...
						if ( !Sql.IsEmptyGuid(gCONTACT_ID) )
						{
							...
							// 11/30/2017 Paul.  Add ASSIGNED_SET_ID for Dynamic User Assignment. 
							Guid   gASSIGNED_SET_ID  = Guid.Empty;
							SqlProcs.spPARENT_GetWithTeam(ref gCONTACT_ID, ref sMODULE, ref sCONTACT_TYPE, ref sCONTACT_TYPE, ref gASSIGNED_USER_ID, ref sASSIGNED_TO, ref sASSIGNED_TO_NAME, ref gTEAM_ID, ref sTEAM_NAME, ref gTEAM_SET_ID, ref gASSIGNED_SET_ID);
							if ( !Sql.IsEmptyGuid(gCONTACT_ID) )
							{
