<%@ Control CodeBehind="Cases.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Emails.Cases" %>
<script runat="server">
/**********************************************************************************************************************
...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtons" Module="Cases" SubPanel="divEmailsCases" Title="Cases.LBL_MODULE_NAME" Runat="Server" />

<div id="divEmailsCases" style='<%= "display:" + (CookieValue("divEmailsCases") != "1" ? "inline" : "none") %>'>
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript(".NTC_REMOVE_CONFIRMATION") %>')">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, "Emails", "remove", "EMAIL_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Cases.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "CASE_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' SkinID="delete_inline" Runat="server" />
						<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, "Emails", "remove", "EMAIL_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Cases.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "CASE_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_REMOVE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
