...
namespace SplendidCRM.Emails
{
	...
	public class Opportunities : SubPanelControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					{
						Guid gOPPORTUNITY_ID = Sql.ToGuid(e.CommandArgument);
						// 06/21/2007 Paul.  Fix to support opportunities relationship. 
						if ( bEditView )
						{
						...
						{
							SqlProcs.spEMAILS_OPPORTUNITIES_Delete(gID, gOPPORTUNITY_ID);
						}
		...
		protected void BindGrid()
		{
			...
			{
				string sSQL;
				// 05/03/2006 Paul.  Fixed view name.  It was not vwEMAILS_OPPORTUNITIES. 
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwEMAILS_OPPORTUNITIES", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spEMAILS_OPPORTUNITIES_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spEMAILS_OPPORTUNITIES_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gOPPORTUNITY_ID) )
			{
				try
				{
					...
					{
						// 06/21/2007 Paul.  Fix to support opportunities relationship. 
						SqlProcs.spEMAILS_OPPORTUNITIES_Update(gID, gOPPORTUNITY_ID);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwEMAILS_OPPORTUNITIES";
