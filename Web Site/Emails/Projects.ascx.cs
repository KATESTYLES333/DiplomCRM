...
namespace SplendidCRM.Emails
{
	...
	public class Projects : SubPanelControl
	{
		...
		protected HtmlInputHidden txtPROJECT_ID  ;
			...
			try
			{
				switch ( e.CommandName )
				{
					...
					case "Project.Remove":
					{
						...
						{
							SqlProcs.spEMAILS_PROJECTS_Delete(gID, gPROJECT_ID);
						}
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwEMAILS_PROJECTS", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spEMAILS_PROJECTS_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spEMAILS_PROJECTS_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			Guid gPROJECT_ID = Sql.ToGuid(txtPROJECT_ID.Value);
			if ( !Sql.IsEmptyGuid(gPROJECT_ID) )
			{
				try
				{
					if ( bEditView )
					{
						this.UpdateEditViewRelationship(gPROJECT_ID);
					}
					else
					{
						SqlProcs.spEMAILS_PROJECTS_Update(gID, gPROJECT_ID);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
					// 05/16/2008 Paul.  If we are not going to redirect,then we must clear the value. 
					txtPROJECT_ID.Value = String.Empty;
				}
				catch(Exception ex)
				{
					SplendidError.SystemError(new StackTrace(true).GetFrame(0), ex);
			...
			if ( !IsPostBack )
			{
				...
				ctlNewRecord.PARENT_TYPE = "Emails";
			}
		}
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwEMAILS_PROJECTS";
