...
namespace SplendidCRM.ProspectLists
{
	...
	public class Resources : SubPanelControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					{
						Response.Redirect("~/Resources/import.aspx?PROSPECT_LIST_ID=" + gID.ToString());
						break;
					}
					case "Resources.Edit":
					{
					...
					case "Resources.Remove":
					{
						...
						{
							SqlProcs.spPROSPECT_LISTS_Resources_Delete(gID, gRESOURCE_ID);
						}
					...
					case "NewRecord":
					{
						...
						Guid gRESOURCE_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spPROSPECT_LISTS_Resources_Update(gID, gRESOURCE_ID);
						//BindGrid();
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwPROSPECT_LISTS_Resources", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spPROSPECT_LISTS_Resources_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spPROSPECT_LISTS_Resources_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
				...
				try
				{
					...
					if ( arrID != null )
					{
						...
						else
						{
							...
							System.Collections.Stack stk = Utils.FilterByACL_Stack(m_sMODULE, "list", arrID, "Resources");
							if ( stk.Count > 0 )
							{
								DbProviderFactory dbf = DbProviderFactories.GetFactory();
								using ( IDbConnection con = dbf.CreateConnection() )
									...
									using ( IDbTransaction trn = Sql.BeginTransaction(con) )
									{
										try
										{
											while ( stk.Count > 0 )
											{
												...
												SqlProcs.spPROSPECT_LISTS_Resources_MassUpdate(sIDs, gID, trn);
											}
											trn.Commit();
										}
										catch(Exception ex)
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwPROSPECT_LISTS_Resources";
