...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtons" Module="Leads" SubPanel="divProspectListsLeads" Title="Leads.LBL_MODULE_NAME" Runat="Server" />
			...
			<asp:TemplateColumn  HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript("ProspectLists.NTC_LEAD_REMOVE_PROSPECT_LISTS_CONFIRM") %>')">
						<asp:ImageButton Visible='<%# !Sql.ToBoolean(Eval("PROSPECT_DYNAMIC_LIST")) && SplendidCRM.Security.GetRecordAccess(Container, "ProspectLists", "remove", "PROSPECT_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Leads.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "LEAD_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' SkinID="delete_inline" Runat="server" />
						<asp:LinkButton  Visible='<%# !Sql.ToBoolean(Eval("PROSPECT_DYNAMIC_LIST")) && SplendidCRM.Security.GetRecordAccess(Container, "ProspectLists", "remove", "PROSPECT_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Leads.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "LEAD_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_REMOVE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
