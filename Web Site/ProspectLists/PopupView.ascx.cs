	...
	public class PopupView : SplendidControl
	{
			...
			try
			{
				...
				else if ( e.CommandName == "NewRecord" )
				{
					...
					if ( mgrAjax != null )
					{
						...
						ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "NewRecord", "SelectProspectList('" + gID.ToString() + "', '" + Sql.EscapeJavaScript(sNAME) + "');", true);
					}
					else
					{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				{
					string sSQL;
					// 12/15/2007 Paul.  We need to be able to reduce the list to those already in a campaign. 
					...
					if ( Sql.IsEmptyGuid(gCAMPAIGN_ID) )
					{
						...
						sSQL = "  from vwPROSPECT_LISTS_List" + ControlChars.CrLf;
					}
					else
					{
						// 02/08/2008 Paul.  We need to build a list of the fields used by the search clause. 
						sSQL = "  from vwCAMPAIGNS_PROSPECT_LISTS" + ControlChars.CrLf;
					}
