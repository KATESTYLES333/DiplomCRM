...
{
	/// <summary>
	///		Summary description for Prospects.
	/// </summary>
	public class Prospects : SubPanelControl
	{
		...
		protected HtmlInputHidden txtPROSPECT_ID ;
		...
		protected SplendidCRM.Prospects.NewRecord ctlNewRecord   ;
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					//	Response.Redirect("~/Prospects/edit.aspx?PARENT_ID=" + gID.ToString());
					//	break;
					// 09/06/2012 Paul.  Add import buttons. 
					case "Prospects.Import":
					{
						Response.Redirect("~/Prospects/import.aspx?PROSPECT_LIST_ID=" + gID.ToString());
						break;
					}
					case "Prospects.Edit":
					{
						Guid gPROSPECT_ID = Sql.ToGuid(e.CommandArgument);
						Response.Redirect("~/Prospects/edit.aspx?ID=" + gPROSPECT_ID.ToString());
						break;
					}
					case "Prospects.Remove":
					{
						Guid gPROSPECT_ID = Sql.ToGuid(e.CommandArgument);
						if ( bEditView )
						{
							this.DeleteEditViewRelationship(gPROSPECT_ID);
						}
						else
						{
							SqlProcs.spPROSPECT_LISTS_PROSPECTS_Delete(gID, gPROSPECT_ID);
						}
					...
					case "NewRecord":
					{
						...
						Guid gPROSPECT_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spPROSPECT_LISTS_PROSPECTS_Update(gID, gPROSPECT_ID);
						//BindGrid();
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					{
						arrSelectFields.Remove("PROSPECT_ID"              );
						arrSelectFields.Remove("PROSPECT_NAME"            );
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "     , ID                         as PROSPECT_ID              " + ControlChars.CrLf
						     + "     , NAME                       as PROSPECT_NAME            " + ControlChars.CrLf
					...
					// 10/14/2012 Paul.  Change default sort to Prospect Name. 
					cmd.CommandText += grdMain.OrderByClause("PROSPECT_NAME", "asc");

					if ( bDebug )
						RegisterClientScriptBlock("vwPROSPECT_LISTS_PROSPECTS", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								{
									this.CreateEditViewRelationships(dt, "PROSPECT_ID");
								}
							}
						}
					}
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spPROSPECT_LISTS_PROSPECTS_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spPROSPECT_LISTS_PROSPECTS_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyString(txtPROSPECT_ID.Value) )
			{
				try
				{
					// 10/04/2006 Paul.  ValidateIDs can throw an exception. 
					string[] arrID = txtPROSPECT_ID.Value.Split(',');
					if ( arrID != null )
					{
						if ( bEditView )
						{
						...
						else
						{
							...
							System.Collections.Stack stk = Utils.FilterByACL_Stack(m_sMODULE, "list", arrID, "PROSPECTS");
							if ( stk.Count > 0 )
							{
								DbProviderFactory dbf = DbProviderFactories.GetFactory();
								using ( IDbConnection con = dbf.CreateConnection() )
									...
									using ( IDbTransaction trn = Sql.BeginTransaction(con) )
									{
										try
										{
											while ( stk.Count > 0 )
											{
												...
												SqlProcs.spPROSPECT_LISTS_PROSPECTS_MassUpdate(sIDs, gID, trn);
											}
											trn.Commit();
										}
										catch(Exception ex)
								...
								txtPROSPECT_ID.Value = String.Empty;
							}
						}
					}
				}
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("PROSPECT_ID"              );
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwPROSPECT_LISTS_PROSPECTS";
