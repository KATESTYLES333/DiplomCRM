<%@ Control CodeBehind="Contacts.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.ProspectLists.Contacts" %>
<script runat="server">
/**********************************************************************************************************************
...
{
	return ModulePopup('Contacts', '<%= txtCONTACT_ID.ClientID %>', null, 'ClearDisabled=1', true, 'PopupMultiSelect.aspx');
}
</script>
...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtons" Module="Contacts" SubPanel="divProspectListsContacts" Title="Contacts.LBL_MODULE_NAME" Runat="Server" />

<div id="divProspectListsContacts" style='<%= "display:" + (CookieValue("divProspectListsContacts") != "1" ? "inline" : "none") %>'>
		...
		<Columns>
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					&nbsp;
					<span onclick="return confirm('<%= L10n.TermJavaScript("ProspectLists.NTC_CONTACT_REMOVE_PROSPECT_LISTS_CONFIRM") %>')">
						<asp:ImageButton Visible='<%# !Sql.ToBoolean(Eval("PROSPECT_DYNAMIC_LIST")) && SplendidCRM.Security.GetRecordAccess(Container, "ProspectLists", "remove", "PROSPECT_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Contacts.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "CONTACT_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' SkinID="delete_inline" Runat="server" />
						<asp:LinkButton  Visible='<%# !Sql.ToBoolean(Eval("PROSPECT_DYNAMIC_LIST")) && SplendidCRM.Security.GetRecordAccess(Container, "ProspectLists", "remove", "PROSPECT_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Contacts.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "CONTACT_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_REMOVE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
