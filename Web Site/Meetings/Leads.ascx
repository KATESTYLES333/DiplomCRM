...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtons" Module="Leads" SubPanel="divLeadsLeads" Title="Leads.LBL_MODULE_NAME" Runat="Server" />
			...
			<asp:TemplateColumn  HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript("Meetings.NTC_REMOVE_INVITEE") %>')">
						...
						<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetRecordAccess(Container, "Meetings", "remove", "MEETING_ASSIGNED_USER_ID") >= 0 && !ArchiveViewExists() %>' CommandName="Leads.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "LEAD_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_REMOVE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
