	...
	public class Resources : SubPanelControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					//	Response.Redirect("~/Resources/edit.aspx?MEETING_ID=" + gID.ToString());
					//	break;
					case "Resources.Edit":
					{
					...
					{
						Guid gRESOURCE_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spMEETINGS_Resources_Delete(gID, gRESOURCE_ID);
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwMEETINGS_Resources", Sql.ClientScriptBlock(cmd));
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gRESOURCE_ID) )
			{
				try
				{
					SqlProcs.spMEETINGS_Resources_Update(gID, gRESOURCE_ID, false, String.Empty);
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
		...
		private void InitializeComponent()
		{
			...
			// 10/05/2017 Paul.  Add Archive relationship view. 
			m_sVIEW_NAME = "vwMEETINGS_Resources";
