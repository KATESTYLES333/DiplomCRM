...
namespace SplendidCRM.Meetings
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Meeting MEETINGS_MEETING_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select ID        " + ControlChars.CrLf
					     ...
					     + "  from vwMEETINGS" + ControlChars.CrLf;
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						cmd.CommandText = sSQL;
						Security.Filter(cmd, "Meetings", "list");
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Meetings.ERR_MEETING_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] MEETINGS_MEETING_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or MEETING_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] MEETINGS_NAME_List(string prefixText, int count)
		{
			return MEETINGS_MEETING_NAME_List(prefixText, count);
		}
	}
}
