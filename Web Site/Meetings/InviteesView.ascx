		...
		<SplendidCRM:SplendidGrid id="grdMain" AllowPaging="true" AllowSorting="true" EnableViewState="true" runat="server">
			<Columns>
				<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Center">
					<ItemTemplate>
						<SplendidCRM:DynamicImage ImageSkinID='<%# Eval("INVITEE_TYPE") %>' Runat="server" />
					</ItemTemplate>
				</asp:TemplateColumn>
				<asp:BoundColumn    HeaderText="Meetings.LBL_NAME"  DataField="NAME"  SortExpression="NAME"  ItemStyle-Width="15%" />
				...
				<asp:BoundColumn    HeaderText="Meetings.LBL_EMAIL" DataField="EMAIL" SortExpression="EMAIL" ItemStyle-Width="15%" />
				<asp:BoundColumn    HeaderText="Meetings.LBL_PHONE" DataField="PHONE" SortExpression="PHONE" ItemStyle-Width="15%" />
				<asp:TemplateColumn HeaderText="" ItemStyle-Width="4%" ItemStyle-HorizontalAlign="Center">
					<ItemTemplate>
						<asp:Button CommandName="Invitees.Add" OnCommand="Page_Command" CommandArgument='<%# Eval("ID") %>' CssClass="button" Text='<%# " " + L10n.Term("Meetings.LBL_ADD_BUTTON") + " " %>' ToolTip='<%# L10n.Term("Meetings.LBL_ADD_BUTTON") %>' Enabled='<%# !IsExistingInvitee(Sql.ToString(Eval("ID"))) %>' Runat="server" />
					</ItemTemplate>
				</asp:TemplateColumn>
			</Columns>
