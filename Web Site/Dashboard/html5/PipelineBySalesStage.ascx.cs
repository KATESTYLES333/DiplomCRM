			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					if ( arrSALES_STAGE == null || arrSALES_STAGE.Length == 0 )
					{
						...
						{
							DataRow row = dt.Rows[i];
							// 05/22/2017 Paul.  We should be using NAME and not DISPLAY_NAME. 
							arrSALES_STAGE[i] = Sql.ToString(row["NAME"]);
						}
					}
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						if ( bDebug )
							RegisterClientScriptBlock("html5PipelineBySalesStage", Sql.ClientScriptBlock(cmd));
