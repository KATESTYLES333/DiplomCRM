...
namespace SplendidCRM.Calls
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Call CALLS_CALL_NAME_Get(string sNAME)
		{
			Call item = new Call();
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select ID        " + ControlChars.CrLf
					     ...
					     + "  from vwCALLS" + ControlChars.CrLf;
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Calls.ERR_CALL_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] CALLS_CALL_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or CALL_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] CALLS_NAME_List(string prefixText, int count)
		{
			return CALLS_CALL_NAME_List(prefixText, count);
		}
	}
}
