<%@ Control CodeBehind="Activities.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Cases.Activities" %>
<script runat="server">
/**********************************************************************************************************************
...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtonsOpen" Module="Activities" SubPanel="divCasesActivitiesOpen" Title="Activities.LBL_OPEN_ACTIVITIES" Runat="Server" />

<div id="divCasesActivitiesOpen" style='<%= "display:" + (CookieValue("divCasesActivitiesOpen") != "1" ? "inline" : "none") %>'>
		...
		<Columns>
			...
			<asp:TemplateColumn  HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<asp:HyperLink Visible='<%# Sql.ToString(DataBinder.Eval(Container.DataItem, "ACTIVITY_TYPE")) == "Emails" && SplendidCRM.Security.GetRecordAccess(Container, Sql.ToString(Eval("ACTIVITY_TYPE")), "view", "ACTIVITY_ASSIGNED_USER_ID") >= 0 %>' NavigateUrl='<%# "~/Emails/edit.aspx?type=reply&DuplicateID=" + Eval("ACTIVITY_ID") + "&CASE_ID=" + Eval("CASE_ID") %>' CssClass="listViewTdToolsS1" Runat="server">
						<asp:Image SkinID="CreateEmails" AlternateText='<%# L10n.Term("Emails.LBL_BUTTON_REPLY") %>' Runat="server" />&nbsp;<%# L10n.Term("Emails.LBL_BUTTON_REPLY") %>
					</asp:HyperLink>
					&nbsp;
...
<SplendidCRM:SubPanelButtons ID="ctlDynamicButtonsHistory" Module="Activities" SubPanel="divCasesActivitiesHistory" Title="Activities.LBL_HISTORY" Runat="Server" />

<div id="divCasesActivitiesHistory" style='<%= "display:" + (CookieValue("divCasesActivitiesHistory") != "1" ? "inline" : "none") %>'>
