...
namespace SplendidCRM.Tasks
{
	...
	public class ListView : SplendidControl
	{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										while ( stk.Count > 0 )
										{
											...
											SqlProcs.spTASKS_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.STATUS, T10n.ToServerTime(ctlMassUpdate.DATE_DUE), T10n.ToServerTime(ctlMassUpdate.DATE_START), ctlMassUpdate.PRIORITY, ctlMassUpdate.PRIMARY_TEAM_ID, ctlMassUpdate.TEAM_SET_LIST, ctlMassUpdate.ADD_TEAM_SET, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
							...
							using ( IDbConnection con = dbf.CreateConnection() )
							{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										...
										{
											string sIDs = Utils.BuildMassIDs(stk);
											SqlProcs.spTASKS_MassDelete(sIDs, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						grdMain.OrderByClause("DATE_DUE", "desc");
		...
		private void InitializeComponent()
		{
			...
			// 04/06.2008 Paul.  CAN_CLOSE is used in the grid. 
			arrSelectFields.Add("CAN_CLOSE");
