...
<SplendidCRM:ListHeader SubPanel="divBugsAccounts" Title="Accounts.LBL_LIST_FORM_TITLE" Runat="Server" />

<div id="divBugsAccounts" style='<%= "display:" + (CookieValue("divBugsAccounts") != "1" ? "inline" : "none") %>'>
			...
			<asp:TemplateColumn  HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript("Bugs.NTC_REMOVE_ACCOUNT_CONFIRMATION") %>')">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetUserAccess("Bugs", "edit", Sql.ToGuid(DataBinder.Eval(Container.DataItem, "BUG_ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Accounts.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ACCOUNT_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' SkinID="delete_inline" Runat="server" />
