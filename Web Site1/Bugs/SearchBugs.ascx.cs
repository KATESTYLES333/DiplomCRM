...
{
	/// <summary>
	///		Summary description for SearchBugs.
	/// </summary>
	public class SearchBugs : SplendidControl
	{
		...
		public static string UnifiedSearch(string sUnifiedSearch, IDbCommand cmd)
		{
			...
			SplendidDynamic.SearchGridColumns("Bugs.Search", arrSearchFields);
			...
			if ( !Sql.IsEmptyString(sUnifiedSearch.Trim()) )
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						if ( bDebug )
							RegisterClientScriptBlock("vwBUGS_List", Sql.ClientScriptBlock(cmd));
