...
namespace SplendidCRM.Bugs
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Bug BUGS_BUG_NAME_Get(string sNAME)
		{
			Bug item = new Bug();
			//try
			{
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Bugs.ERR_BUG_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] BUGS_BUG_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or BUG_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] BUGS_NAME_List(string prefixText, int count)
		{
			return BUGS_BUG_NAME_List(prefixText, count);
		}

		[WebMethod(EnableSession=true)]
		public Bug BUGS_BUG_NUMBER_Get(string sNAME)
		{
			Bug item = new Bug();
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Sql.AppendParameter(cmd, sNAME, Sql.SqlFilterMode.StartsWith, "BUG_NUMBER");
						// 07/02/2007 Paul.  Sort is important so that the first match is selected. 
						...
						using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
						{
							...
							{
								item.ID   = Sql.ToGuid   (rdr["ID"  ]);
								item.NAME = Sql.ToString (rdr["BUG_NUMBER"]);
							}
						}
					}
				}
		...
		// 03/29/2007 Paul.  In order for AutoComplete to work, the parameter names must be "prefixText" and "count". 
		[WebMethod(EnableSession=true)]
		public string[] BUGS_BUG_NUMBER_List(string prefixText, int count)
		{
			string[] arrItems = new string[0];
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "Bugs", "list");
						// 07/12/2010 Paul.  It does not make sense to allow a fuzzy search for a number. 
						Sql.AppendParameter(cmd, prefixText, Sql.SqlFilterMode.StartsWith, "BUG_NUMBER");
						cmd.CommandText += " order by BUG_NUMBER" + ControlChars.CrLf;
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							((IDbDataAdapter)da).SelectCommand = cmd;
							using ( DataTable dt = new DataTable() )
							{
								...
								arrItems = new string[dt.Rows.Count];
								for ( int i=0; i < dt.Rows.Count; i++ )
									arrItems[i] = Sql.ToString(dt.Rows[i]["BUG_NUMBER"]);
							}
						}
					}
				}
