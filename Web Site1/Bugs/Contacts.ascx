<%@ Control CodeBehind="Contacts.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Bugs.Contacts" %>
<script runat="server">
/**********************************************************************************************************************
...
<SplendidCRM:ListHeader SubPanel="divBugsContacts" Title="Contacts.LBL_MODULE_NAME" Runat="Server" />

<div id="divBugsContacts" style='<%= "display:" + (CookieValue("divBugsContacts") != "1" ? "inline" : "none") %>'>
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript(".NTC_REMOVE_CONFIRMATION") %>')">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetUserAccess("Bugs", "edit", Sql.ToGuid(DataBinder.Eval(Container.DataItem, "BUG_ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Contacts.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "CONTACT_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' SkinID="delete_inline" Runat="server" />
						<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetUserAccess("Bugs", "edit", Sql.ToGuid(DataBinder.Eval(Container.DataItem, "BUG_ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Contacts.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "CONTACT_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_REMOVE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
