<%@ Control CodeBehind="Cases.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Bugs.Cases" %>
<script runat="server">
/**********************************************************************************************************************
...
<SplendidCRM:ListHeader SubPanel="divBugsCases" Title="Cases.LBL_MODULE_NAME" Runat="Server" />

<div id="divBugsCases" style='<%= "display:" + (CookieValue("divBugsCases") != "1" ? "inline" : "none") %>'>
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript("Cases.NTC_REMOVE_FROM_BUG_CONFIRMATION") %>')">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetUserAccess("Bugs", "edit", Sql.ToGuid(DataBinder.Eval(Container.DataItem, "BUG_ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Cases.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "CASE_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' SkinID="delete_inline" Runat="server" />
						<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetUserAccess("Bugs", "edit", Sql.ToGuid(DataBinder.Eval(Container.DataItem, "BUG_ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Cases.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "CASE_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_REMOVE") %>' Runat="server" />
					</span>
				</ItemTemplate>
			</asp:TemplateColumn>
		</Columns>
