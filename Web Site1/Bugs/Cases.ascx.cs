	...
	public class Cases : SubPanelControl
	{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					else
					{
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "  from vwBUGS_CASES" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								this.ApplyGridViewRules("Bugs." + m_sMODULE, dt);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !IsPostBack )
			{
				...
				ctlDynamicButtons.AppendButtons("Bugs." + m_sMODULE, gASSIGNED_USER_ID, gID);
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Bugs." + m_sMODULE, arrSelectFields);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Bugs." + m_sMODULE, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
