					...
					<ItemTemplate>
						<asp:Panel Visible='<%# DateTime.Now > Sql.ToDateTime(Eval("DATE_START")).AddSeconds(-Sql.ToInteger(Eval("REMINDER_TIME"))) %>' runat="server">
							<asp:Table SkinID="tabEditViewButtons" Visible="<%# !PrintView %>" runat="server">
								<asp:TableRow>
									<asp:TableCell>
										<asp:Button Visible='<%# SplendidCRM.Security.GetUserAccess(Sql.ToString(Eval("ACTIVITY_TYPE")), "edit", Sql.ToGuid(Eval("ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Edit"    CommandArgument='<%# Eval("ID") %>' OnCommand="Page_Command" CssClass="button" style="margin-right: 3px;" Text='<%# "  " + L10n.Term(".LBL_EDIT_BUTTON_LABEL"   ) + "  " %>' ToolTip='<%# L10n.Term(".LBL_EDIT_BUTTON_TITLE"    ) %>' Runat="server" />
							...
							<asp:Table Width="100%" CellPadding="0" CellSpacing="0" CssClass="tabDetailView" runat="server">
								...
								<asp:TableRow>
									...
									<asp:TableCell Width="15%" VerticalAlign="Top" CssClass="tabDetailViewDL"><%# L10n.Term(Sql.ToString(Eval("ACTIVITY_TYPE")) + ".LBL_DURATION") %></asp:TableCell>
									<asp:TableCell Width="35%" VerticalAlign="Top" CssClass="tabDetailViewDF"><%# Eval("DURATION_HOURS") + " " + L10n.Term("Calls.LBL_HOURS_ABBREV") + " " + Eval("DURATION_MINUTES") + " " + L10n.Term("Calls.LBL_MINSS_ABBREV") %></asp:TableCell>
								</asp:TableRow>
