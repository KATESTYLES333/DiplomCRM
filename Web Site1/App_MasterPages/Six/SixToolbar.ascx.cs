...
namespace SplendidCRM.Themes.Six
{
	...
	public class SixToolbar : SplendidControl
	{
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			if ( e.CommandName == "Toolbar.Show" )
			{
				...
				if ( arrArguments.Length == 3 )
				{
					...
					{
						Guid   gPARENT_ID   = Sql.ToGuid  (Request["ID"]);
		...
		new protected void AppendEditViewRelationships(string sEDIT_NAME, PlaceHolder plc, bool bNewRecord)
		{
			Guid   gPARENT_ID   = Sql.ToGuid  (Request["ID"]);
			string sPARENT_TYPE = Sql.ToString(Page.Items["ActiveTabMenu"]);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( bAjaxAutoComplete )
			{
				...
				{
					ServiceReference svc = new ServiceReference("~/Administration/Teams/AutoComplete.asmx");
					ScriptReference  scr = new ScriptReference ("~/Administration/Teams/AutoComplete.js"  );
					if ( !mgrAjax.Services.Contains(svc) )
						mgrAjax.Services.Add(svc);
					if ( !mgrAjax.Scripts.Contains(scr) )
						mgrAjax.Scripts.Add(scr);
		...
		private void InitializeComponent()
		{
			...
			AppendEditViewRelationships("Home.EditView", plcSubPanel, true);
