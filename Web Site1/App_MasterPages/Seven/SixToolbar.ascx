<%@ Control Language="c#" AutoEventWireup="false" Codebehind="SixToolbar.ascx.cs" Inherits="SplendidCRM.Themes.Seven.SixToolbar" TargetSchema="http://schemas.microsoft.com/intellisense/ie5"%>
<script runat="server">
/**********************************************************************************************************************
				...
				<table cellpadding="0" cellspacing="0" class="MoreActionsInnerTable">
					<tr>
						<td class="MoreActionsInnerCell">
							...
							<asp:HyperLink  ID="lnkEmployees" Text='<%# L10n.Term(".LBL_EMPLOYEES" ) %>' NavigateUrl="~/Employees/default.aspx"              CssClass="ModuleActionsMenuItems" Visible='<%# SplendidCRM.Security.GetUserAccess("Employees", "access") >= 0                              %>' Runat="server" />
							<asp:HyperLink  ID="lnkAdmin"     Text='<%# L10n.Term(".LBL_ADMIN"     ) %>' NavigateUrl="~/Administration/default.aspx"         CssClass="ModuleActionsMenuItems" Visible='<%# SplendidCRM.Security.IS_ADMIN || SplendidCRM.Security.IS_ADMIN_DELEGATE                     %>' Runat="server" />
							<asp:HyperLink  ID="lnkTraining"  Text='<%# L10n.Term(".LBL_TRAINING"  ) %>' NavigateUrl="~/Home/TrainingPortal.aspx"            CssClass="ModuleActionsMenuItems" Visible='<%# !Sql.ToBoolean(Application["CONFIG.hide_training"])                                         %>' Runat="server" />
							...
							<asp:LinkButton ID="lnkReload"    Text='<%# L10n.Term(".LBL_RELOAD"    ) %>' CommandName="Admin.Reload" OnCommand="Page_Command" CssClass="ModuleActionsMenuItems" Visible='<%# bDebug && (SplendidCRM.Security.IS_ADMIN || SplendidCRM.Security.IS_ADMIN_DELEGATE) %>' Runat="server" />
						</td>
					</tr>
				</table>
			</asp:Panel>
