...
<div id="divImportView">
	...
	<SplendidCRM:ModuleHeader ID="ctlModuleHeader" Module="Import" Title="Import.LBL_MODULE_NAME" EnableModuleLabel="false" EnablePrint="false" HelpName="ImportView" EnableHelp="true" Runat="Server" />
	<p>
	...
	function OAuthTokenUpdate(oauth_token, oauth_verifier, realmId)
	{
	...
	function SelectSourceFormat()
	{
		...
		// 06/03/2014 Paul.  QuickBooks Online is going to use a different API than standard QuickBooks. 
		document.getElementById('spnStep3Upload'  ).style.display = (!(bLinkedIn || bTwitter || bFacebook || bSalesforce || bQuickBooks || bQuickBooksOnline) ? 'inline' : 'none');
		document.getElementById('spnStep3Connect' ).style.display = ( (bLinkedIn || bTwitter || bFacebook || bSalesforce || bQuickBooks || bQuickBooksOnline) ? 'inline' : 'none');
		document.getElementById('tblUploadFile'   ).style.display = (!(bLinkedIn || bTwitter || bFacebook || bSalesforce || bQuickBooks || bQuickBooksOnline) ? 'inline' : 'none');
		document.getElementById('tblUploadConnect').style.display = ( (bLinkedIn || bTwitter || bFacebook || bSalesforce || bQuickBooks || bQuickBooksOnline) ? 'inline' : 'none');
