...
namespace SplendidCRM.Opportunities
{
	...
	/// </summary>
	public class Projects : SubPanelControl
	{
		...
		protected SplendidCRM.Projects.NewRecord ctlNewRecord   ;
			...
			try
			{
				switch ( e.CommandName )
				{
					...
					case "Project.Remove":
					{
						...
						if ( bEditView )
						{
							this.DeleteEditViewRelationship(gPROJECT_ID);
						}
						else
						{
							...
							SqlProcs.spPROJECTS_OPPORTUNITIES_Delete(gPROJECT_ID, gID);
						}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bEditView && IsPostBack && arrUPDATED.Count > 0 )
					{
						...
						arrSelectFields.Remove("OPPORTUNITY_ASSIGNED_USER_ID");
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "     , ID                            as PROJECT_ID                  " + ControlChars.CrLf
						     + "     , NAME                          as PROJECT_NAME                " + ControlChars.CrLf
						     + "     , @OPPORTUNITY_ID               as OPPORTUNITY_ID              " + ControlChars.CrLf
						     + "     , @OPPORTUNITY_NAME             as OPPORTUNITY_NAME            " + ControlChars.CrLf
						     + "     , @OPPORTUNITY_ASSIGNED_USER_ID as OPPORTUNITY_ASSIGNED_USER_ID" + ControlChars.CrLf
						     + "  from vwPROJECTS" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Sql.AddParameter(cmd, "@OPPORTUNITY_ID"              , gID);
						Sql.AddParameter(cmd, "@OPPORTUNITY_NAME"            , Sql.ToString(Page.Items["NAME"            ]));
						Sql.AddParameter(cmd, "@OPPORTUNITY_ASSIGNED_USER_ID", Sql.ToGuid  (Page.Items["ASSIGNED_USER_ID"]));
					...
					else
					{
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "  from vwOPPORTUNITIES_PROJECTS" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwOPPORTUNITIES_PROJECTS", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								this.ApplyGridViewRules("Opportunities." + m_sMODULE, dt);
								...
								{
									this.CreateEditViewRelationships(dt, "PROJECT_ID");
								}
							}
						}
					}
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				...
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spPROJECTS_OPPORTUNITIES_Delete(gDELETE_ID, gPARENT_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				...
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spPROJECTS_OPPORTUNITIES_Update(gUPDATE_ID, gPARENT_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !IsPostBack )
			{
				...
				ctlDynamicButtons.AppendButtons("Opportunities." + m_sMODULE, gASSIGNED_USER_ID, gID);
				// 05/09/2010 Paul.  The parent needs to be initialized when the page first loads. 
				ctlNewRecord.PARENT_ID    = gID;
				ctlNewRecord.PARENT_TYPE = "Opportunities";
			}
		}
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("PROJECT_ID"                  );
			arrSelectFields.Add("ASSIGNED_USER_ID"            );
			arrSelectFields.Add("OPPORTUNITY_ASSIGNED_USER_ID");
			// 11/26/2005 Paul.  Add fields early so that sort events will get called. 
			// 04/28/2008 Paul.  Fix GRID_NAME to be consistent with all Project names being singular. 
			this.AppendGridColumns(grdMain, "Opportunities." + m_sMODULE, arrSelectFields);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Opportunities." + m_sMODULE, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
