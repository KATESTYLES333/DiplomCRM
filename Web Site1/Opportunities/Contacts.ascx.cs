			...
			try
			{
				switch ( e.CommandName )
				{
					...
					case "Contacts.Remove":
					{
						...
						{
							SqlProcs.spOPPORTUNITIES_CONTACTS_Delete(gID, gCONTACT_ID);
						}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					else
					{
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "  from vwOPPORTUNITIES_CONTACTS" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwOPPORTUNITIES_CONTACTS", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spOPPORTUNITIES_CONTACTS_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spOPPORTUNITIES_CONTACTS_Update(gPARENT_ID, gUPDATE_ID, String.Empty, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gCONTACT_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spOPPORTUNITIES_CONTACTS_Update(gID, gCONTACT_ID, String.Empty);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
			...
			if ( !IsPostBack )
			{
				...
				// 10/04/2010 Paul.  Cache the Account for use when creating a contact inline. 
