<%@ Control CodeBehind="PipelineByMonthByOutcome.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Opportunities.xaml.PipelineByMonthByOutcome" %>
...
<Canvas x:Name="container" Loaded="mainVerticalCanvasLoaded" MouseMove="whenMouseMoves" MouseLeave="mainVerticalCanvasMouseLeave" xmlns="http://schemas.microsoft.com/client/2007" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml">
	<Canvas x:Name="bound_box" <%= L10n.IsLanguageRTL() ? "FlowDirection=\"RightToLeft\"" : String.Empty %> Canvas.Top="10" Canvas.Left="10" Width="780" Height="380" Background="White">
		...
		<Canvas Canvas.Top="24">
			<Canvas.Clip>
				<RectangleGeometry Rect="0, 0, 612, <%# nGridHeight + 1 %>" />
			</Canvas.Clip>
			<Canvas x:Name="myScroller1_Container" Canvas.Left="602" Height="<%# nGridHeight %>">
...
<asp:Repeater DataSource="<%# nlDataRows %>" runat="server">
	<ItemTemplate>
		<asp:Repeater DataSource=<%# (Container.DataItem as System.Xml.XmlNode).SelectNodes("bar") %> runat="server">
			<HeaderTemplate>
					<Canvas Canvas.Top="0" Canvas.Left="<%# (Container.Parent.Parent as RepeaterItem).ItemIndex * (nAxis_xData_length + dAxis_xData_section) + (dAxis_xData_section) / 2 %>" Width="<%# nAxis_xData_length + 3 %>" Height="<%# nGridHeight %>" Visibility="<%# ((Container.Parent.Parent as RepeaterItem).DataItem as System.Xml.XmlNode).Attributes.GetNamedItem("endLabel").Value == "0" ? "Collapsed" : "Visible" %>">
						<Rectangle Canvas.Top="<%# nGridHeight - 3 - ((Sql.ToInteger(((Container.Parent.Parent as RepeaterItem).DataItem as System.Xml.XmlNode).Attributes.GetNamedItem("endLabel").Value) - nAxis_yData_min) * nGridHeight) / (nAxis_yData_max + dAxis_yData_section) %>" Canvas.Left="2.5" 
							Height="<%# 1 + ((Sql.ToInteger(((Container.Parent.Parent as RepeaterItem).DataItem as System.Xml.XmlNode).Attributes.GetNamedItem("endLabel").Value) - nAxis_yData_min) * nGridHeight) / (nAxis_yData_max + dAxis_yData_section) %>" Width="<%# nAxis_xData_length %>" Fill="#cccccc" />
			</HeaderTemplate>
			<ItemTemplate>
			...
			<FooterTemplate>
						<TextBlock Text="<%# HttpUtility.HtmlEncode(((Container.Parent.Parent as RepeaterItem).DataItem as System.Xml.XmlNode).Attributes.GetNamedItem("endLabel").Value) %>" 
							Canvas.Top="<%# nGridHeight - 15 - ((Sql.ToInteger(((Container.Parent.Parent as RepeaterItem).DataItem as System.Xml.XmlNode).Attributes.GetNamedItem("endLabel").Value) - nAxis_yData_min) * nGridHeight) / (nAxis_yData_max + dAxis_yData_section) %>" Canvas.Left="<%# nAxis_xData_length / 2 %>" Foreground="#395163" FontFamily="Arial" FontSize="11" />
					</Canvas>
			</FooterTemplate>
		</asp:Repeater>
	</ItemTemplate>
