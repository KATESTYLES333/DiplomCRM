<%@ Control Language="c#" AutoEventWireup="false" Codebehind="NewRecord.ascx.cs" Inherits="SplendidCRM.Notes.NewRecord" TargetSchema="http://schemas.microsoft.com/intellisense/ie5"%>
<script runat="server">
/**********************************************************************************************************************
...
function FileNameChanged(fld)
{
	...
	var txtNAME = document.getElementById('<%= new SplendidCRM.DynamicControl(this, "NAME").ClientID %>');
	if ( txtNAME != null )
		txtNAME.value = sNAME;
}
</script>
...
<div id="divNewRecord">
	...
	<SplendidCRM:HeaderLeft ID="ctlHeaderLeft" Title="Notes.LBL_NEW_FORM_TITLE" Width=<%# uWidth %> Visible="<%# ShowHeader %>" Runat="Server" />
	...
	<asp:Panel ID="pnlMain" Width="100%" CssClass="leftColumnModuleS3" runat="server">
		...
		<asp:Panel ID="pnlEdit" CssClass="" style="margin-bottom: 4px;" Width=<%# uWidth %> runat="server">
			<asp:Literal Text='<%# "<h4>" + L10n.Term("Notes.LBL_NEW_FORM_TITLE") + "</h4>" %>' Visible="<%# ShowInlineHeader %>" runat="server" />
