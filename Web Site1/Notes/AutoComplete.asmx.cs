...
namespace SplendidCRM.Notes
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Note NOTES_NOTE_NAME_Get(string sNAME)
		{
			Note item = new Note();
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "Notes", "list");
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Notes.ERR_NOTE_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] NOTES_NOTE_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or NOTE_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] NOTES_NAME_List(string prefixText, int count)
		{
			return NOTES_NOTE_NAME_List(prefixText, count);
		}
	}
}
