...
namespace SplendidCRM.Project
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Project PROJECT_PROJECT_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select ID        " + ControlChars.CrLf
					     ...
					     + "  from vwPROJECT" + ControlChars.CrLf;
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Project.ERR_PROJECT_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] PROJECT_PROJECT_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or PROJECT_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] PROJECT_NAME_List(string prefixText, int count)
		{
			return PROJECT_PROJECT_NAME_List(prefixText, count);
		}
	}
}
