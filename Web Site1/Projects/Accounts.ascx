...
<SplendidCRM:ListHeader SubPanel="divProjectsAccounts" Title="Accounts.LBL_LIST_FORM_TITLE" Runat="Server" />
			...
			<asp:TemplateColumn  HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Left" ItemStyle-Wrap="false">
				<ItemTemplate>
					...
					<span onclick="return confirm('<%= L10n.TermJavaScript("Accounts.ACCOUNT_REMOVE_PROJECT_CONFIRM") %>')">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetUserAccess("Project", "edit", Sql.ToGuid(DataBinder.Eval(Container.DataItem, "PROJECT_ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Accounts.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ACCOUNT_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' SkinID="delete_inline" Runat="server" />
