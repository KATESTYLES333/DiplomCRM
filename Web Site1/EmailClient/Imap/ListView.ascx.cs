	...
	public class ListView : SplendidControl
	{
		...
		protected void QuickCreate_Load(Object sender, EventArgs e)
		{
			if ( !Sql.IsEmptyString(hidSelectedUniqueID.Value) && plcQuickCreate.Controls.Count > 0 )
			{
				...
				DataTable dtMain = ImapUtils.GetMessage(Context, sSERVER_URL, nPORT, bMAILBOX_SSL, sEMAIL_USER, sDECRYPTED_PASSWORD, sFOLDER_ID, sUNIQUE_ID);
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				...
				else if ( e.CommandName == "Compose" )
				{
					...
					ctlEditView.lstSIGNATURE_Changed(null, null);
				}
					...
					if ( grdMain.SelectedItem != null )
					{
						...
						if ( vw != null && vw.Count >= nItemIndex )
						{
							...
							if ( dtMain.Rows.Count > 0 )
							{
								...
								Guid gEMAIL_ID = ImapUtils.ImportMessage(Context, Session, sPARENT_TYPE, gPARENT_ID, sSERVER_URL, nPORT, bMAILBOX_SSL, sEMAIL_USER, sDECRYPTED_PASSWORD, Security.USER_ID, gASSIGNED_USER_ID, gTEAM_ID, sTEAM_SET_LIST, sFOLDER_ID, sUNIQUE_ID);
				...
				else if ( e.CommandName == "Delete" )
				{
					...
					{
						string sFOLDER_ID = Sql.ToString(ViewState["LastSelectedNodeValue"]);
						ImapUtils.DeleteMessage(Context, sSERVER_URL, nPORT, bMAILBOX_SSL, sEMAIL_USER, sDECRYPTED_PASSWORD, sFOLDER_ID, hidSelectedUniqueID.Value);
						hidSelectedUniqueID.Value = String.Empty;
						grdMain.SelectedIndex = -1;
				...
				else if ( e.CommandName == "Import.Save" )
				{
					if ( !Sql.IsEmptyString(hidSelectedUniqueID.Value) )
					{
						...
						ImapUtils.ImportMessage(Context, Session, sPARENT_TYPE, gPARENT_ID, sSERVER_URL, nPORT, bMAILBOX_SSL, sEMAIL_USER, sDECRYPTED_PASSWORD, Security.USER_ID, gASSIGNED_USER_ID, gTEAM_ID, sTEAM_SET_LIST, sFOLDER_ID, sUNIQUE_ID);
					}
					ctlImportView.Visible = false;
				}
		...
		protected void treeMain_TreeNodeExpanded(object sender, TreeNodeEventArgs e)
		{
			try
			{
				...
				if ( e.Node.Parent != null )
				{
					...
					xml.LoadXml(Sql.ToString(Session["Imap.xmlFolders"]));
					...
					if ( xFolder != null )
					{
						ImapUtils.UpdateFolderTreeNodeCounts(Context, sSERVER_URL, nPORT, bMAILBOX_SSL, sEMAIL_USER, sDECRYPTED_PASSWORD, xFolder);
						Session["Imap.xmlFolders"] = xml.OuterXml;
		...
		protected void treeMain_SelectedNodeChanged(Object sender, EventArgs e)
		{
			try
			{
				...
				if ( treeMain.SelectedNode != null )
				{
					...
					int nTotalCount = 0;
					int nUnreadCount = 0;
					ImapUtils.GetFolderCount(Context, sSERVER_URL, nPORT, bMAILBOX_SSL, sEMAIL_USER, sDECRYPTED_PASSWORD, sFOLDER_ID, ref nTotalCount, ref nUnreadCount);
		...
		protected void BindGrid()
		{
			try
			{
				...
				if ( !Sql.IsEmptyString(sFOLDER_ID) )
				{
					...
					dtMain = ImapUtils.GetFolderMessages(Context, sSERVER_URL, nPORT, bMAILBOX_SSL, sEMAIL_USER, sDECRYPTED_PASSWORD, sFOLDER_ID);
					...
					if ( dtMain.Rows.Count > 0 )
					{
						...
						try
						{
							...
							foreach ( DataRow rowAttachment in dtATTACHMENTS.Rows )
							{
								...
								string sURL       = "Attachment.aspx?FOLDER_ID=" + sFOLDER_ID + "&UNIQUE_ID=" + sUNIQUE_ID + "&ATTACHMENT_ID=" + sID;
								rowAttachment["URL"] = "Imap/" + sURL;
		...
		protected void BindTree()
		{
			...
			ViewState["LastSelectedNodeValuePath"] = String.Empty;
			XmlDocument xmlFolders = ImapUtils.GetFolderTree(Context, sSERVER_URL, nPORT, bMAILBOX_SSL, sEMAIL_USER, sDECRYPTED_PASSWORD);
			...
			Session["Imap.xmlFolders"] = xmlFolders.OuterXml;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if ( !IsPostBack )
				{
					...
					using ( IDbConnection con = dbf.CreateConnection() )
					{
						...
						using ( IDbCommand cmd = con.CreateCommand() )
						{
							...
							Sql.AddParameter(cmd, "@SERVICE" , "imap");
							...
							using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
							{
								if ( rdr.Read() )
								{
									...
									Session["Imap.SERVER_URL"    ] = sSERVER_URL    ;
									Session["Imap.EMAIL_USER"    ] = sEMAIL_USER    ;
									Session["Imap.EMAIL_PASSWORD"] = sEMAIL_PASSWORD;
									Session["Imap.PORT"          ] = nPORT          ;
									Session["Imap.MAILBOX_SSL"   ] = bMAILBOX_SSL   ;
									Session["Imap.MAILBOX"       ] = sMAILBOX       ;
				...
				{
					sSERVER_URL     = Sql.ToString (Session["Imap.SERVER_URL"    ]);
					sEMAIL_USER     = Sql.ToString (Session["Imap.EMAIL_USER"    ]);
					sEMAIL_PASSWORD = Sql.ToString (Session["Imap.EMAIL_PASSWORD"]);
					nPORT           = Sql.ToInteger(Session["Imap.PORT"          ]);
					bMAILBOX_SSL    = Sql.ToBoolean(Session["Imap.MAILBOX_SSL"   ]);
					sMAILBOX        = Sql.ToString (Session["Imap.MAILBOX"       ]);
