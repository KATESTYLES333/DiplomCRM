...
{
	/// <summary>
	///		Summary description for UserCalendarPopupView.
	/// </summary>
	public class UserCalendarPopupView : SplendidControl
	{
		...
		protected DateTime      dtDATE_START   ;
		protected DateTime      dtDATE_END     ;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				dtDATE_START = Sql.ToDateTime(Request["DATE_START"]);
				dtDATE_END   = Sql.ToDateTime(Request["DATE_END"  ]);
				if ( !IsPostBack )
				{
		...
		{    
			...
			this.AppendGridColumns(grdMain, m_sMODULE + ".UserCalendarPopupView" + (Sql.ToString(Request["FULL_NAME"]) == "1" ? "Name" : ""), arrSelectFields);
