...
namespace SplendidCRM.EmailMarketing
{
	...
	public class ProspectLists : SubPanelControl
	{
		...
		protected Guid            gCAMPAIGN_ID       = Guid.Empty;
			...
			try
			{
				switch ( e.CommandName )
				{
					...
					case "ProspectLists.Remove":
					{
						...
						{
							SqlProcs.spEMAIL_MKTG_PRSPT_LST_Delete(gID, gPROSPECT_LIST_ID);
						}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bEditView && IsPostBack && arrUPDATED.Count > 0 )
					{
						...
						arrSelectFields.Remove("EMAIL_MARKETING_ID"       );
						arrSelectFields.Remove("EMAIL_MARKETING_NAME"     );
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     ...
						     + "     , @EMAIL_MARKETING_ID        as EMAIL_MARKETING_ID       " + ControlChars.CrLf
						     + "     , @EMAIL_MARKETING_NAME      as EMAIL_MARKETING_NAME     " + ControlChars.CrLf
						     + "     , null                       as CAMPAIGN_ID              " + ControlChars.CrLf
						     + "     , null                       as CAMPAIGN_ASSIGNED_USER_ID" + ControlChars.CrLf
						     + "     , null                       as LIST_ID                  " + ControlChars.CrLf
						     + "  from vwPROSPECT_LISTS" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Sql.AddParameter(cmd, "@EMAIL_MARKETING_ID"       , gID);
						Sql.AddParameter(cmd, "@EMAIL_MARKETING_NAME"     , Sql.ToString(Page.Items["NAME"            ]));
					...
					else
					{
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "  from vwEMAIL_MARKETING_PROSPECT_LST" + ControlChars.CrLf
						     + " where 1 = 1                         " + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Sql.AppendParameter(cmd, gID, "EMAIL_MARKETING_ID");
					}
					// 04/26/2008 Paul.  Move Last Sort to the database.
					cmd.CommandText += grdMain.OrderByClause("DATE_ENTERED", "desc");

					if ( bDebug )
						RegisterClientScriptBlock("vwEMAIL_MARKETING_PROSPECT_LST", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								this.ApplyGridViewRules("EmailMarketing." + m_sMODULE, dt);
								if ( dt.Rows.Count > 0 )
									gCAMPAIGN_ID = Sql.ToGuid(dt.Rows[0]["CAMPAIGN_ID"]);
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spEMAIL_MKTG_PRSPT_LST_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spEMAIL_MKTG_PRSPT_LST_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gPROSPECT_LIST_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spEMAIL_MKTG_PRSPT_LST_Update(gID, gPROSPECT_LIST_ID);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
			...
			if ( !IsPostBack )
			{
				...
				ctlDynamicButtons.AppendButtons("EmailMarketing." + m_sMODULE, gASSIGNED_USER_ID, gID);
				// 10/21/2010 Paul.  Make sure to initialize the parent. 
				ctlNewRecord.PARENT_ID   = gID;
				ctlNewRecord.PARENT_TYPE = "EmailMarketing";
			}
		}
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "EmailMarketing." + m_sMODULE, arrSelectFields);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("EmailMarketing." + m_sMODULE, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
