		...
		if ( arrDetailViewRelationship.length == 0 || arrValidModules[sMODULE_NAME] !== undefined )
		{
			...
			if ( sUSER_AGENT == 'WIN' || sUSER_AGENT.indexOf('Android') > 0 || sUSER_AGENT.indexOf('iPad') > 0 || sUSER_AGENT.indexOf('iPhone') > 0 || sUSER_AGENT.indexOf('Touch') > 0 || sUSER_AGENT.indexOf('MSAppHost') > 0 )
			{
				...
				//if ( sMENU_ACTIVE_MODULE == sMODULE_NAME )
				{
					...
					ctlActiveMenu.ActionsBar(sTabMenuCtl, sLayoutPanel, sActionsPanel, sMODULE_NAME, sDISPLAY_NAME, ctlTabMenu, (sMENU_ACTIVE_MODULE == sMODULE_NAME));
				}
			}
			// 11/14/2012 Paul.  Let's try having the action bar and the popdowns. 
			//else
			{
				ctlActiveMenu.ActionsPopup(sTabMenuCtl, sLayoutPanel, sActionsPanel, sMODULE_NAME, sDISPLAY_NAME, tabMenuInner, td);
			}
		}
	}
...
TabMenuUI_Six.prototype.ActionsPopup = function(sTabMenuCtl, sLayoutPanel, sActionsPanel, sMODULE_NAME, sDISPLAY_NAME, ctlTabMenu_tabMenuInner, tdMenuInner)
{
	try
	{
		/*
...
// 02/01/2013 Paul.  Render all action bars, just all but the active one. 
TabMenuUI_Six.prototype.Load = function(sLayoutPanel, sActionsPanel, sMODULE_NAME, sDISPLAY_NAME, ctlTabMenu, bActive)
{
	try
	{
		var sTabMenuCtl = 'ctlTabMenu';
	...
	{
		SplendidError.SystemAlert(e, 'TabMenuUI_Six.ActionsBar ' + sMODULE_NAME);
	}
}
