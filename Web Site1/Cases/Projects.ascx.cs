...
namespace SplendidCRM.Cases
{
	...
	public class Projects : SubPanelControl
	{
			...
			try
			{
				switch ( e.CommandName )
				{
					...
					case "Project.Remove":
					{
						...
						{
							// 09/08/2012 Paul.  Project Relations data for Cases, Bugs, Cases, Contacts, Opportunities and Quotes moved to separate tables. 
							SqlProcs.spPROJECTS_CASES_Delete(gPROJECT_ID, gID);
						}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					else
					{
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "  from vwCASES_PROJECTS" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
					...
					cmd.CommandText += grdMain.OrderByClause("ESTIMATED_START_DATE", "asc");

					if ( bDebug )
						RegisterClientScriptBlock("vwCASES_PROJECTS", Sql.ClientScriptBlock(cmd));
		...
		// 01/27/2010 Paul.  This method is only called when in EditMode. 
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				...
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spPROJECTS_CASES_Delete(gDELETE_ID, gPARENT_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				...
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spPROJECTS_CASES_Update(gUPDATE_ID, gPARENT_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gPROJECT_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spPROJECTS_CASES_Update(gPROJECT_ID, gID);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("ESTIMATED_START_DATE" );
