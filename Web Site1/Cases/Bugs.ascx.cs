...
namespace SplendidCRM.Cases
{
	...
	public class Bugs : SubPanelControl
	{
			...
			try
			{
				switch ( e.CommandName )
				{
					...
					case "Bugs.Remove":
					{
						...
						{
							SqlProcs.spCASES_BUGS_Delete(gID, gBUG_ID);
						}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					else
					{
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "  from vwCASES_BUGS" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwCASES_BUGS", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spCASES_BUGS_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spCASES_BUGS_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			// 03/04/2006 Paul.  Added the CASES_BUGS assignment. 
			...
			if ( !Sql.IsEmptyGuid(gBUG_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spCASES_BUGS_Update(gID, gBUG_ID);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
