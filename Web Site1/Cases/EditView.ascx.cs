						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
									...
									// 08/06/2009 Paul.  CASE_NUMBER now uses our number sequence table. 
									...
									SqlProcs.spCASES_Update
										...
										, new DynamicControl(this, rowCurrent, "RESOLUTION"      ).Text
										, sMODULE
										, gPARENT_ID
										, new DynamicControl(this, rowCurrent, "CASE_NUMBER"     ).Text
										...
										, new DynamicControl(this, rowCurrent, "TYPE"            ).Text
				...
				if ( !IsPostBack )
				{
					...
					if ( !Sql.IsEmptyGuid(gID) || !Sql.IsEmptyGuid(gDuplicateID) )
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							sSQL = "select vwCASES_Edit.*                                                        " + ControlChars.CrLf
							     + "     , (case when vwCASES_USERS.CASE_ID is null then 0 else 1 end) as EXCHANGE_FOLDER" + ControlChars.CrLf
							     + "  from            vwCASES_Edit                                               " + ControlChars.CrLf
							     + "  left outer join vwCASES_USERS                                              " + ControlChars.CrLf
							     + "               on vwCASES_USERS.CASE_ID = vwCASES_Edit.ID                    " + ControlChars.CrLf
								...
								using ( IDbConnection con = dbf.CreateConnection() )
								{
									...
									using ( IDbCommand cmd = con.CreateCommand() )
									{
										...
										using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
										{
											if ( rdr.Read() )
											{
												// 09/05/2008 Paul.  When pulling account from contact, the field names are ACCOUNT_ID and ACCOUNT_NAME.
