...
{
	/// <summary>
	///		Summary description for MyCases.
	/// </summary>
	public class MyCases : DashletControl
	{
		...
		protected void Bind(bool bBind)
		{
			...
			{
				string sSQL;
				sSQL = "  from vwCASES_MyList" + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					grdMain.OrderByClause("PRIORITY", "asc");
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwCASES_MyList", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								this.ApplyGridViewRules(m_sMODULE + ".MyCases", dt);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( this.Visible && !Sql.IsEmptyString(sDetailView) )
			{
				...
				vwFields.RowFilter = "CONTROL_NAME = '~/Cases/MyCases'";
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, m_sMODULE + ".MyCases", arrSelectFields);
		}
		#endregion
	}
}
