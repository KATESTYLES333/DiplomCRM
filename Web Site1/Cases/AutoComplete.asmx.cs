...
namespace SplendidCRM.Cases
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Case CASES_CASE_NAME_Get(string sNAME)
		{
			Case item = new Case();
			//try
			{
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Cases.ERR_CASE_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] CASES_CASE_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or CASE_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] CASES_NAME_List(string prefixText, int count)
		{
			return CASES_CASE_NAME_List(prefixText, count);
		}

		[WebMethod(EnableSession=true)]
		public Case CASES_CASE_NUMBER_Get(string sNAME)
		{
			Case item = new Case();
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					string sSQL;
					sSQL = "select ID         " + ControlChars.CrLf
					     + "     , CASE_NUMBER" + ControlChars.CrLf
					     + "  from vwCASES    " + ControlChars.CrLf;
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						cmd.CommandText = sSQL;
						Security.Filter(cmd, "Cases", "list");
						// 07/12/2010 Paul.  It does not make sense to allow a fuzzy search for a number. 
						Sql.AppendParameter(cmd, sNAME, Sql.SqlFilterMode.StartsWith, "CASE_NUMBER");
						// 07/02/2007 Paul.  Sort is important so that the first match is selected. 
						...
						using ( IDataReader rdr = cmd.ExecuteReader(CommandBehavior.SingleRow) )
						{
							...
							{
								item.ID   = Sql.ToGuid   (rdr["ID"         ]);
								item.NAME = Sql.ToString (rdr["CASE_NUMBER"]);
							}
						}
					}
				}
		...
		// 03/29/2007 Paul.  In order for AutoComplete to work, the parameter names must be "prefixText" and "count". 
		[WebMethod(EnableSession=true)]
		public string[] CASES_CASE_NUMBER_List(string prefixText, int count)
		{
			string[] arrItems = new string[0];
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "Cases", "list");
						// 07/12/2010 Paul.  It does not make sense to allow a fuzzy search for a number. 
						Sql.AppendParameter(cmd, prefixText, Sql.SqlFilterMode.StartsWith, "CASE_NUMBER");
						cmd.CommandText += " order by CASE_NUMBER" + ControlChars.CrLf;
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							((IDbDataAdapter)da).SelectCommand = cmd;
							using ( DataTable dt = new DataTable() )
							{
								...
								arrItems = new string[dt.Rows.Count];
								for ( int i=0; i < dt.Rows.Count; i++ )
									arrItems[i] = Sql.ToString(dt.Rows[i]["CASE_NUMBER"]);
							}
						}
					}
				}
