...
{
	/// <summary>
	///		Summary description for SearchChatMessages.
	/// </summary>
	public class SearchChatMessages : SplendidControl
	{
		...
		public static string UnifiedSearch(string sUnifiedSearch, IDbCommand cmd)
		{
			...
			SplendidDynamic.SearchGridColumns("ChatMessages.Search", arrSearchFields);
			...
			if ( !Sql.IsEmptyString(sUnifiedSearch.Trim()) )
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					sSQL = "select " + Sql.FormatSelectFields(arrSelectFields) + ControlChars.CrLf
					     + "  from vwCHAT_MESSAGES_List" + ControlChars.CrLf;
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						if ( bDebug )
							RegisterClientScriptBlock("vwCHAT_MESSAGES_List", Sql.ClientScriptBlock(cmd));
