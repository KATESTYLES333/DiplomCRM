...
namespace SplendidCRM.ProspectLists
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public ProspectList PROSPECT_LISTS_PROSPECT_LIST_NAME_Get(string sNAME)
		{
			ProspectList item = new ProspectList();
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Security.Filter(cmd, "ProspectLists", "list");
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("ProspectLists.ERR_PROSPECT_LIST_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] PROSPECT_LISTS_PROSPECT_LIST_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or PROSPECT_LIST_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] PROSPECT_LISTS_NAME_List(string prefixText, int count)
		{
			return PROSPECT_LISTS_PROSPECT_LIST_NAME_List(prefixText, count);
		}
	}
}
