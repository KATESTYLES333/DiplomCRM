...
namespace SplendidCRM.ProspectLists
{
	...
	public class EditView : SplendidControl
	{
		...
		protected Panel           pnlDynamicSQL                ;
		protected CheckBox        chkDYNAMIC_LIST              ;
					...
					if ( Page.IsValid )
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							using ( IDbTransaction trn = Sql.BeginTransaction(con) )
							{
								try
								{
									// 01/10/2010 Paul.  We added new dynamic fields to a Prospect List. 
									bool bDYNAMIC_LIST = new DynamicControl(this, rowCurrent, "DYNAMIC_LIST").Checked;
									// 04/2s4/2006 Paul.  Upgrade to SugarCRM 4.2 Schema. 
									// 11/18/2007 Paul.  Use the current values for any that are not defined in the edit view. 
									...
									if ( bDYNAMIC_LIST )
									{
										...
										if ( ctlQueryBuilder.Visible )
										{
											SqlProcs.spPROSPECT_LISTS_SQL_Update
												( gID
												, ctlQueryBuilder.ReportSQL
												, ctlQueryBuilder.ReportRDL
												, trn
												);
											//SqlProcs.spPROSPECT_LISTS_UpdateDynamic(gID, trn);
		...
		private void chkDYNAMIC_LIST_CheckedChanged(object sender, EventArgs e)
		{
			chkDYNAMIC_LIST = FindControl("DYNAMIC_LIST") as CheckBox;
			if ( chkDYNAMIC_LIST != null )
				pnlDynamicSQL.Visible = chkDYNAMIC_LIST.Checked;
		}
				...
				if ( !IsPostBack )
				{
					...
					if ( !Sql.IsEmptyGuid(gID) || !Sql.IsEmptyGuid(gDuplicateID) )
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							// 01/14/2010 Paul.  Move DYNAMIC_SQL to a separate table so that it cannot be imported or exported. 
							sSQL = "select *                       " + ControlChars.CrLf
							     + "  from vwPROSPECT_LISTS_EditSQL" + ControlChars.CrLf;
							using ( IDbCommand cmd = con.CreateCommand() )
							{
								cmd.CommandText = sSQL;
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 )
										{
											...
											string sDYNAMIC_RDL = Sql.ToString(rdr["DYNAMIC_RDL"]);
											chkDYNAMIC_LIST = FindControl("DYNAMIC_LIST") as CheckBox;
											if ( chkDYNAMIC_LIST != null )
											{
												// 12/04/2012 Paul.  Make sure to hide the dynamic list for the Community Edition. 
												// 08/25/2013 Paul.  File IO is slow, so cache existance test. 
												chkDYNAMIC_LIST.Visible = Utils.CachedFileExists(Context, "~/Reports/EditView.ascx");
												Literal litDYNAMIC_LIST = FindControl("DYNAMIC_LIST_LABEL") as Literal;
												if ( litDYNAMIC_LIST != null )
													litDYNAMIC_LIST.Visible = chkDYNAMIC_LIST.Visible;
												
												pnlDynamicSQL.Visible = chkDYNAMIC_LIST.Checked;
												chkDYNAMIC_LIST.AutoPostBack = true;
												chkDYNAMIC_LIST.CheckedChanged += new EventHandler(chkDYNAMIC_LIST_CheckedChanged);
											}
											// 08/22/2012 Paul.  If the RDL is blank, but the SQL is not, then prevent access to the builder as this is a read-only SQL. 
											if ( Sql.IsEmptyString(sDYNAMIC_RDL) && !Sql.IsEmptyString(rdr["DYNAMIC_SQL"]) && Sql.ToBoolean(rdr["DYNAMIC_LIST"]) )
											{
												ctlQueryBuilder.Visible = false;
												if ( chkDYNAMIC_LIST != null )
													chkDYNAMIC_LIST.Enabled = false;
											}
											else
											{
												ctlQueryBuilder.LoadRdl(sDYNAMIC_RDL);
											}
											...
											ViewState ["PROSPECT_DYNAMIC_LIST"] = Sql.ToBoolean(rdr["DYNAMIC_LIST"         ]);
											Page.Items["NAME"                 ] = ViewState ["NAME"                 ];
											Page.Items["ASSIGNED_USER_ID"     ] = ViewState ["ASSIGNED_USER_ID"     ];
											Page.Items["PROSPECT_DYNAMIC_LIST"] = ViewState ["PROSPECT_DYNAMIC_LIST"];
											// 11/10/2010 Paul.  Apply Business Rules. 
