...
namespace SplendidCRM.Campaigns
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Campaign CAMPAIGNS_CAMPAIGN_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Campaigns.ERR_CAMPAIGN_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] CAMPAIGNS_CAMPAIGN_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or CAMPAIGN_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] CAMPAIGNS_NAME_List(string prefixText, int count)
		{
			return CAMPAIGNS_CAMPAIGN_NAME_List(prefixText, count);
		}
	}
}
