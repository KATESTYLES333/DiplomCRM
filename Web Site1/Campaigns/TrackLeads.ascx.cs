...
{
	/// <summary>
	///		Summary description for TrackLeads.
	/// </summary>
	public class TrackLeads : SplendidControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				...
				{
					string sSQL  = "select TARGET_ID from vwCAMPAIGN_LOG_TrackLeads where CAMPAIGN_ID = '" + gID.ToString() + "'";
					string sNAME = Sql.ToString(Page.Items["NAME"]) + " - " + L10n.Term("Campaigns.LBL_LOG_ENTRIES_LEAD_TITLE");
					if ( sNAME.Length > 50 )
						sNAME = Sql.ToString(Page.Items["NAME"]).Substring(0, Sql.ToString(Page.Items["NAME"]).Length - (sNAME.Length - 50)) + " - " + L10n.Term("Campaigns.LBL_LOG_ENTRIES_LEAD_TITLE");
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
				     + "  from vwCAMPAIGN_LOG_TrackLeads  " + ControlChars.CrLf
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwCAMPAIGN_LOG_TrackLeads", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								this.ApplyGridViewRules(m_sMODULE + ".TrackLeads", dt);
			...
			if ( !IsPostBack )
			{
				...
				ctlDynamicButtons.AppendButtons(m_sMODULE + ".TrackLeads", gASSIGNED_USER_ID, gID);
			}
		}
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, m_sMODULE + ".TrackLeads", arrSelectFields);
			// 08/22/2012 Paul.  Allow prospect list to be created from the tracked users. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons(m_sMODULE + ".TrackLeads", Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
