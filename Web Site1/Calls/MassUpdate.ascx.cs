...
namespace SplendidCRM.Calls
{
	...
	public class MassUpdate : SplendidCRM.MassUpdate
	{
		...
		protected DropDownList lstDIRECTION       ;
		...
		public string DIRECTION
		{
			get
			{
				return lstDIRECTION.SelectedValue;
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			try
			{
				if ( !IsPostBack )
				{
					...
					lstSTATUS          .DataSource = SplendidCache.List("call_status_dom");
					lstSTATUS          .DataBind();
					lstSTATUS          .Items.Insert(0, new ListItem(L10n.Term(".LBL_NONE"), ""));
					lstDIRECTION       .DataSource = SplendidCache.List("call_direction_dom");
					lstDIRECTION       .DataBind();
					lstDIRECTION       .Items.Insert(0, new ListItem(L10n.Term(".LBL_NONE"), ""));
				}
			}
			catch(Exception ex)
			{
