<%@ Control CodeBehind="PopupView.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.ProjectTasks.PopupView" %>
<script runat="server">
/**********************************************************************************************************************
...
<div id="divPopupView">
	...
	function SelectProjectTask(sPARENT_ID, sPARENT_NAME)
	{
		...
		{
			alert('Original window has closed.  Project Task cannot be assigned.' + '\n' + sPARENT_ID + '\n' + sPARENT_NAME);
		}
	}
	function SelectChecked()
	{
		if ( window.opener != null && window.opener.ChangeProjectTask != null )
		{
			...
			window.opener.ChangeProjectTask(sSelectedItems, '');
			window.close();
		}
		else
		{
	...
	function Clear()
	{
		if ( window.opener != null && window.opener.ChangeProjectTask != null )
		{
			window.opener.ChangeProjectTask('', '');
			window.close();
		}
		else
		{
	...
	<asp:UpdatePanel UpdateMode="Conditional" Visible='<%# !Sql.ToBoolean(Application["CONFIG.disable_popupview_inline"]) %>' runat="server">
		<ContentTemplate>
			<asp:Button ID="btnCreateInline" CommandName="NewRecord.Show" OnCommand="Page_Command" Text='<%# L10n.Term(m_sMODULE + ".LNK_NEW_PROJECT_TASK") %>' CssClass="button" style="margin-bottom: 4px;" Visible="<%# !this.IsMobile %>" runat="server" />
