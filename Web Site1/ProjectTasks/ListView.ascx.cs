...
namespace SplendidCRM.ProjectTasks
{
	...
	public class ListView : SplendidControl
	{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										while ( stk.Count > 0 )
										{
											...
											SqlProcs.spPROJECT_TASKS_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.STATUS, T10n.ToServerTime(ctlMassUpdate.DATE_DUE), T10n.ToServerTime(ctlMassUpdate.DATE_START), ctlMassUpdate.PRIORITY, ctlMassUpdate.PRIMARY_TEAM_ID, ctlMassUpdate.TEAM_SET_LIST, ctlMassUpdate.ADD_TEAM_SET, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
							...
							using ( IDbConnection con = dbf.CreateConnection() )
							{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										...
										{
											string sIDs = Utils.BuildMassIDs(stk);
											SqlProcs.spPROJECT_TASKS_MassDelete(sIDs, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
		...
		private void InitializeComponent()
		{
			...
			SetMenu(m_sMODULE);
			// 04/27/2006 Paul.  The module is called ProjectTask, but we use the plural version in our lists.
