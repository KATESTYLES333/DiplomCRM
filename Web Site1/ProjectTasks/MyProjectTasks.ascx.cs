...
{
	/// <summary>
	///		Summary description for MyProjectTasks.
	/// </summary>
	public class MyProjectTasks : DashletControl
	{
		...
		protected void Bind(bool bBind)
		{
			...
			{
				string sSQL;
				sSQL = "  from vwPROJECT_TASKS_MyList" + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					grdMain.OrderByClause("DATE_DUE", "asc");
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwPROJECT_TASKS_MyList", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								this.ApplyGridViewRules(m_sMODULE + ".MyProjectTasks", dt);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( this.Visible && !Sql.IsEmptyString(sDetailView) )
			{
				...
				vwFields.RowFilter = "CONTROL_NAME = '~/ProjectTasks/MyProjectTasks'";
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, m_sMODULE + ".MyProjectTasks", arrSelectFields);
		}
		#endregion
	}
}
