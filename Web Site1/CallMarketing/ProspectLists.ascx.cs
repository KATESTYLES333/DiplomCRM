...
namespace SplendidCRM.CallMarketing
{
	...
	public class ProspectLists : SubPanelControl
	{
			...
			try
			{
				switch ( e.CommandName )
				{
					...
					case "ProspectLists.Remove":
					{
						...
						{
							SqlProcs.spCALL_MKTG_PRSPT_LST_Delete(gID, gPROSPECT_LIST_ID);
						}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bEditView && IsPostBack && arrUPDATED.Count > 0 )
					{
						...
						arrSelectFields.Remove("CALL_MARKETING_ID"       );
						arrSelectFields.Remove("CALL_MARKETING_NAME"     );
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     ...
						     + "     , @CALL_MARKETING_ID         as CALL_MARKETING_ID        " + ControlChars.CrLf
						     + "     , @CALL_MARKETING_NAME       as CALL_MARKETING_NAME      " + ControlChars.CrLf
						...
						Sql.AddParameter(cmd, "@CALL_MARKETING_ID"       , gID);
						Sql.AddParameter(cmd, "@CALL_MARKETING_NAME"     , Sql.ToString(Page.Items["NAME"            ]));
					...
					else
					{
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "  from vwCALL_MARKETING_PROSPECT_LST" + ControlChars.CrLf
						     + " where 1 = 1                        " + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Sql.AppendParameter(cmd, gID, "CALL_MARKETING_ID");
					}
					// 04/26/2008 Paul.  Move Last Sort to the database.
					cmd.CommandText += grdMain.OrderByClause("DATE_ENTERED", "desc");

					if ( bDebug )
						RegisterClientScriptBlock("vwCALL_MARKETING_PROSPECT_LST", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								this.ApplyGridViewRules("CallMarketing." + m_sMODULE, dt);
								if ( dt.Rows.Count > 0 )
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spCALL_MKTG_PRSPT_LST_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spCALL_MKTG_PRSPT_LST_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gPROSPECT_LIST_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spCALL_MKTG_PRSPT_LST_Update(gID, gPROSPECT_LIST_ID);
					}
			...
			if ( !IsPostBack )
			{
				...
				ctlDynamicButtons.AppendButtons("CallMarketing." + m_sMODULE, gASSIGNED_USER_ID, gID);
				ctlNewRecord.PARENT_ID   = gID;
				ctlNewRecord.PARENT_TYPE = "CallMarketing";
			}
		}
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "CallMarketing." + m_sMODULE, arrSelectFields);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("CallMarketing." + m_sMODULE, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
