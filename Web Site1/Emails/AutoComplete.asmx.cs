...
namespace SplendidCRM.Emails
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Email EMAILS_EMAIL_NAME_Get(string sNAME)
		{
			...
			//try
			{
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Emails.ERR_EMAIL_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] EMAILS_EMAIL_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or EMAIL_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] EMAILS_NAME_List(string prefixText, int count)
		{
			return EMAILS_EMAIL_NAME_List(prefixText, count);
		}
	}
}
