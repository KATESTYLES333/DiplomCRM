	...
	public class PopupView : SplendidControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				{
					string sSQL;
					// 08/15/2014 Paul.  PARENT_ID is null will exclude images from incoming emails and just show previously attached images from the CKEditor. 
					sSQL = "  from vwEMAIL_IMAGES" + ControlChars.CrLf
					     + " where PARENT_ID is null" + ControlChars.CrLf;
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						cmd.CommandText = sSQL;
						grdMain.OrderByClause("FILENAME", "asc");
		...
		{    
			...
			m_sMODULE = "EmailImages";
			arrSelectFields = new UniqueStringCollection();
			arrSelectFields.Add("ID"      );
			arrSelectFields.Add("FILENAME");
