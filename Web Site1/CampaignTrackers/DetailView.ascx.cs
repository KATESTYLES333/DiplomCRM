	...
	public class DetailView : SplendidControl
	{
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			try
			{
				...
				{
					SqlProcs.spCAMPAIGN_TRKRS_Delete(gID);
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 )
										{
											...
											Guid   gTRACKER_ID   = Sql.ToGuid   (rdr["ID"          ]);
											string sTRACKER_NAME = Sql.ToString (rdr["TRACKER_NAME"]);
											string sTRACKER_URL  = Sql.ToString (rdr["TRACKER_URL" ]);
											bool   bIS_OPTOUT    = Sql.ToBoolean(rdr["IS_OPTOUT"   ]);
											...
											if ( bIS_OPTOUT )
											{
												...
												// 06/23/2010 Paul.  Instead of using the same placeholder as SugarCRM, lets use an empty Guid. 
												// This will allow the sample URL to be a valid URL, but it should not enter any data. 
												//sTrackerPath += "MESSAGE_ID";
												sTrackerPath += Guid.Empty.ToString();
											}
											else
											{
												sTrackerPath  = sSiteURL;
												sTrackerPath += "campaign_trackerv2.aspx?identifier=";
												// 06/23/2010 Paul.  Instead of using the same placeholder as SugarCRM, lets use an empty Guid. 
												// This will allow the sample URL to be a valid URL, but it should not enter any data. 
												//sTrackerPath += "MESSAGE_ID";
												sTrackerPath += Guid.Empty.ToString();
											...
											new DynamicControl(this, "MESSAGE_URL").Text = sTrackerPath;
