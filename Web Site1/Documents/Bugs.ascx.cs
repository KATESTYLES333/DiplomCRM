			...
			try
			{
				switch ( e.CommandName )
				{
					case "Documents.GetLatest":
					{
						...
						SqlProcs.spDOCUMENTS_BUGS_GetLatest(gBUG_ID, gID);
						BindGrid();
						break;
					}
					case "Bugs.Edit":
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					{
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "  from vwDOCUMENTS_BUGS" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Security.Filter(cmd, m_sMODULE, "list");
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwDOCUMENTS_BUGS", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spDOCUMENTS_BUGS_Delete(gDELETE_ID, gPARENT_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spDOCUMENTS_BUGS_Update(gUPDATE_ID, gPARENT_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gBUG_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spDOCUMENTS_BUGS_Update(gBUG_ID, gID);
					}
