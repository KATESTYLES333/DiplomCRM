...
namespace SplendidCRM.Documents
{
	...
	public class Accounts : SubPanelControl
	{
			...
			try
			{
				switch ( e.CommandName )
				{
					case "Documents.GetLatest":
					{
						...
						SqlProcs.spACCOUNTS_DOCUMENTS_GetLatest(gACCOUNT_ID, gID);
						BindGrid();
						break;
					}
					case "Accounts.Edit":
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					{
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "  from vwDOCUMENTS_ACCOUNTS" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Security.Filter(cmd, m_sMODULE, "list");
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwDOCUMENTS_ACCOUNTS", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								this.ApplyGridViewRules("Documents." + m_sMODULE, dt);
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spACCOUNTS_DOCUMENTS_Delete(gDELETE_ID, gPARENT_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spACCOUNTS_DOCUMENTS_Update(gUPDATE_ID, gPARENT_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gACCOUNT_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spACCOUNTS_DOCUMENTS_Update(gACCOUNT_ID, gID);
					}
			...
			if ( !IsPostBack )
			{
				...
				ctlDynamicButtons.AppendButtons("Documents." + m_sMODULE, gASSIGNED_USER_ID, gID);
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Documents." + m_sMODULE, arrSelectFields);
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Documents." + m_sMODULE, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
