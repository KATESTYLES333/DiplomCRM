<%@ Control CodeBehind="PopupView.ascx.cs" Language="c#" AutoEventWireup="false" Inherits="SplendidCRM.Documents.PopupView" %>
<script runat="server">
/**********************************************************************************************************************
...
<div id="divPopupView">
	...
	<SplendidCRM:SearchView ID="ctlSearchView" Module="Documents" IsPopupSearch="true" ShowSearchTabs="false" Visible="<%# !PrintView %>" Runat="Server" />

	<script type="text/javascript">
	function SelectDocument(sPARENT_ID, sPARENT_NAME)
	{
		if ( window.opener != null && window.opener.ChangeDocument != null )
		{
			window.opener.ChangeDocument(sPARENT_ID, sPARENT_NAME);
			window.close();
		}
		else
		{
			alert('Original window has closed.  Document cannot be assigned.' + '\n' + sPARENT_ID + '\n' + sPARENT_NAME);
		}
	}
	function SelectChecked()
	{
		if ( window.opener != null && window.opener.ChangeDocument != null )
		{
			...
			window.opener.ChangeDocument(sSelectedItems, '');
			window.close();
		}
		else
		{
	...
	<SplendidCRM:ListHeader Title="Documents.LBL_LIST_FORM_TITLE" Runat="Server" />
	...
	<asp:UpdatePanel UpdateMode="Conditional" Visible='<%# !Sql.ToBoolean(Application["CONFIG.disable_popupview_inline"]) %>' runat="server">
		<ContentTemplate>
			<asp:Button ID="btnCreateInline" CommandName="NewRecord.Show" OnCommand="Page_Command" Text='<%# L10n.Term(m_sMODULE + ".LNK_NEW_DOCUMENT") %>' CssClass="button" style="margin-bottom: 4px;" Visible="<%# !this.IsMobile %>" runat="server" />
