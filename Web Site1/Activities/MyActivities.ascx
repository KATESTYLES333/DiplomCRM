	...
	<SplendidCRM:SplendidGrid id="grdMain" SkinID="grdListView" AllowPaging="<%# !PrintView %>" EnableViewState="true" runat="server">
		<Columns>
			...
			<asp:TemplateColumn HeaderText="Activities.LBL_LIST_CLOSE" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Center">
				<ItemTemplate>
					<asp:HyperLink Visible='<%# SplendidCRM.Security.GetUserAccess(Sql.ToString(Eval("ACTIVITY_TYPE")), "edit", Sql.ToGuid(Eval("ASSIGNED_USER_ID"))) >= 0 %>' NavigateUrl='<%# "~/" + Eval("ACTIVITY_TYPE") + "/edit.aspx?id=" + Eval("ID") + "&Status=Close" %>' Runat="server">
			...
			<asp:TemplateColumn  HeaderText="Activities.LBL_LIST_DATE" SortExpression="DATE_START" ItemStyle-Width="15%" ItemStyle-Wrap="false">
				<ItemTemplate>
					<font CssClass="<%# (Sql.ToDateTime(Eval("DATE_START")) < DateTime.Now) ? "overdueTask" : "futureTask" %>"><%# Sql.ToDateString(T10n.FromServerTime(Sql.ToDateTime(Eval("DATE_START")))) %></font>
				</ItemTemplate>
			</asp:TemplateColumn>
			<asp:TemplateColumn HeaderText="Activities.LBL_ACCEPT_THIS" ItemStyle-Width="5%" ItemStyle-Wrap="false">
				<ItemTemplate>
					<div style="DISPLAY: <%# String.Compare((Eval("ACCEPT_STATUS") as string), "none", true) == 0 ? "inline" : "none" %>">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetUserAccess(Sql.ToString(Eval("ACTIVITY_TYPE")), "edit", Sql.ToGuid(Eval("ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Activity.Accept"    CommandArgument='<%# Eval("ID") %>' OnCommand="Page_Command" AlternateText='<%# L10n.Term(".dom_meeting_accept_options.accept"   ) %>' SkinID="accept_inline"    Runat="server" />
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetUserAccess(Sql.ToString(Eval("ACTIVITY_TYPE")), "edit", Sql.ToGuid(Eval("ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Activity.Tentative" CommandArgument='<%# Eval("ID") %>' OnCommand="Page_Command" AlternateText='<%# L10n.Term(".dom_meeting_accept_options.tentative") %>' SkinID="tentative_inline" Runat="server" />
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetUserAccess(Sql.ToString(Eval("ACTIVITY_TYPE")), "edit", Sql.ToGuid(Eval("ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Activity.Decline"   CommandArgument='<%# Eval("ID") %>' OnCommand="Page_Command" AlternateText='<%# L10n.Term(".dom_meeting_accept_options.decline"  ) %>' SkinID="decline_inline"   Runat="server" />
					</div>
					<div style="DISPLAY: <%# String.Compare((Eval("ACCEPT_STATUS") as string), "none", true) != 0 ? "inline" : "none" %>">
			...
			<asp:TemplateColumn HeaderText="" ItemStyle-Width="1%" ItemStyle-HorizontalAlign="Center" ItemStyle-Wrap="false">
				<ItemTemplate>
					<asp:HyperLink Visible='<%# SplendidCRM.Security.GetUserAccess(Sql.ToString(Eval("ACTIVITY_TYPE")), "edit", Sql.ToGuid(Eval("ASSIGNED_USER_ID"))) >= 0 %>' NavigateUrl='<%# "~/" + Sql.ToString(Eval("ACTIVITY_TYPE")) + "/edit.aspx?id=" + Eval("ID") %>' ToolTip='<%# L10n.Term(".LNK_EDIT") %>' Runat="server">
						<asp:Image SkinID="edit_inline" Runat="server" />
					</asp:HyperLink>
					<asp:HyperLink Visible='<%# SplendidCRM.Security.GetUserAccess(Sql.ToString(Eval("ACTIVITY_TYPE")), "view", Sql.ToGuid(Eval("ASSIGNED_USER_ID"))) >= 0 %>' NavigateUrl='<%# "~/" + Sql.ToString(Eval("ACTIVITY_TYPE")) + "/view.aspx?id=" + Eval("ID") %>' ToolTip='<%# L10n.Term(".LNK_VIEW") %>' Runat="server">
