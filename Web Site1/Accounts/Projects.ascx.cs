	...
	public class Projects : SubPanelControl
	{
				...
				switch ( e.CommandName )
				{
					...
					case "Project.Remove":
					{
						...
						else
						{
							...
							SqlProcs.spPROJECTS_ACCOUNTS_Delete(gPROJECT_ID, gID);
						}
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					else
					{
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "  from vwACCOUNTS_PROJECTS" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwACCOUNT_PROJECTS", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								this.ApplyGridViewRules("Accounts." + m_sMODULE, dt);
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				...
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spPROJECTS_ACCOUNTS_Delete(gDELETE_ID, gPARENT_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				...
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spPROJECTS_ACCOUNTS_Update(gUPDATE_ID, gPARENT_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !IsPostBack )
			{
				...
				ctlDynamicButtons.AppendButtons("Accounts." + m_sMODULE, gASSIGNED_USER_ID, gID);
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Accounts." + m_sMODULE, arrSelectFields);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Accounts." + m_sMODULE, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
