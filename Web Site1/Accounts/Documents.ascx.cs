...
namespace SplendidCRM.Accounts
{
	...
	public class Documents : SubPanelControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					{
						Guid gDOCUMENT_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spACCOUNTS_DOCUMENTS_Delete(gID, gDOCUMENT_ID);
						BindGrid();
						break;
					}
					case "Documents.GetLatest":
					{
						...
						{
							SqlProcs.spACCOUNTS_DOCUMENTS_GetLatest(gID, gDOCUMENT_ID);
						}
						BindGrid();
						break;
					}
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					{
						sSQL = "select *                       " + ControlChars.CrLf
						     + "  from vwACCOUNTS_DOCUMENTS    " + ControlChars.CrLf
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwACCOUNTS_DOCUMENTS", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spACCOUNTS_DOCUMENTS_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spACCOUNTS_DOCUMENTS_Update(gPARENT_ID, gUPDATE_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gDOCUMENT_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spACCOUNTS_DOCUMENTS_Update(gID, gDOCUMENT_ID);
					}
		...
		private void InitializeComponent()
		{
			...
			m_sMODULE = "Documents";
			this.AppendGridColumns(grdMain, "Accounts." + m_sMODULE);
		}
		#endregion
	}
}
