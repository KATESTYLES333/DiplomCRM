...
{
	/// <summary>
	///		Summary description for SearchAccounts.
	/// </summary>
	public class SearchAccounts : SplendidControl
	{
		...
		public static string UnifiedSearch(string sUnifiedSearch, IDbCommand cmd)
		{
			...
			SplendidDynamic.SearchGridColumns("Accounts.Search", arrSearchFields);
			...
			if ( !Sql.IsEmptyString(sUnifiedSearch.Trim()) )
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						if ( bDebug )
							RegisterClientScriptBlock("vwACCOUNTS_List", Sql.ClientScriptBlock(cmd));
