...
namespace Pop3 {
  ...
  public class Pop3MailClient {
	...
	// 01/20/2008 Paul.  We need a way to process a message from a memory stream. 
	public Pop3MailClient(Stream mem)
	{
		pop3Stream = mem;
		pop3StreamReader = new StreamReader(pop3Stream, Encoding.ASCII);
		bEmbeddedStream = true;
	}

	protected bool bEmbeddedStream = false;

	public bool IsEmbeddedEOF()
	{
		// 01/20/2008 Paul.  We don't want to dramatically change the logic, so only return EOF if this is a stream. 
		return bEmbeddedStream && pop3StreamReader.EndOfStream;
	}
    ...
    /// Send TOP command to POP 3 server to fetch one particular message
    /// </summary>
    /// <param name="MessageNo">ID of message required</param>
    /// <returns>false: negative server respond, message not delivered</returns>
    protected bool SendTopCommand(int MessageNo, int NumberOfLines) {
      EnsureState(Pop3ConnectionStateEnum.Connected);
      // retrieve mail with message number
      string response;
      if (!executeCommand("TOP "+ MessageNo.ToString() + " " + NumberOfLines.ToString(), out response)) {
        CallWarning("GetRawEmail", response, "negative response for email (ID: {0}) request", MessageNo);
        return false;
      }
      return true;
    ...
    protected bool readMultiLine(out string response) {
      response = null;
      // 01/29/2008 Paul.  We don't want to dramatically change the logic, so only return EOF if this is a memory stream. 
      if ( IsEmbeddedEOF() )
		  return false;
      response = pop3StreamReader.ReadLine();
      if (response==null) {
