...
namespace Pop3 {
  ...
  public class Pop3MimeClient:Pop3MailClient {
    ...
    public Pop3MimeClient(
      ...
      bool useSSL,
      string Username, 
      string Password) : base (PopServer, Port, useSSL, Username, Password)
	{
	}

	public Pop3MimeClient(Stream mem) : base (mem)
	{
	}
    ...
    /// <summary>
    /// // 01/09/2008 Paul.  Add support for TOP command. 
    /// Gets email headers from POP3 server and processes it.
    /// </summary>
    /// <param name="MessageNo">Email Id to be fetched from POP3 server</param>
    /// <param name="Message">decoded email</param>
    /// <returns>false: no email received or email not properly formatted</returns>
    public bool GetHeaders(int MessageNo, out RxMailMessage Message) {
      Message = null;

      //request email, send TOP command to POP3 server
      if (!SendTopCommand(MessageNo, 0)) {
        return false;
      }
        ...
        switch (headerLineType) {
          ...
          case "content-disposition":
			try
			{
				...
				// 06/02/2008 Paul.  The Microsoft implementation of ContentDisposition has a problem with date parsing. 
				// 06/02/2008 Paul.  The Mono implementation is not perfect either.  We had to remove the quotes and fix the date parsing. 
				// http://msdn.microsoft.com/en-us/library/system.net.mime.contentdisposition.aspx
				// inline; filename="image001.gif"; size=3095; creation-date="Wed, 28 May 2008 08:21:24 GMT"; modification-date="Wed, 28 May 2008 08:21:24 GMT"
				SplendidCRM.ContentDisposition cd = new SplendidCRM.ContentDisposition(headerLineContent);
				message.ContentDisposition = new System.Net.Mime.ContentDisposition();
				message.ContentDisposition.CreationDate     = cd.CreationDate    ;
				message.ContentDisposition.ModificationDate = cd.ModificationDate;
				message.ContentDisposition.DispositionType  = cd.DispositionType ;
				message.ContentDisposition.FileName         = cd.FileName        ;
				message.ContentDisposition.Size             = cd.Size            ;
				//message.ContentDisposition.Inline           = cd.Inline          ;
				//message.ContentDisposition.ReadDate         = cd.ReadDate        ;
			}
			catch  //(Exception ex)
			{
				// 06/02/2008 Paul.  In case of failure, let the message continue to be imported. 
			}
			break;
          case "content-id":
          ...
          case "sender":
            ...
            break;
          case "subject":
            // 07/18/2010 Paul.  Lets make use of the Imap decoder to correct issues with the subject. 
            message.Subject = Koolwired.Imap.ImapDecode.Decode(headerLineContent);
            break;
          case "received":
    ...
    public MailAddress ConvertToMailAddress(string address) {
      ...
      {
        // 07/18/2010 Paul.  Lets make use of the Imap decoder to correct issues with the subject. 
        address = Koolwired.Imap.ImapDecode.Decode(address);
      }
      if (address=="<>") {
    ...
    private TransferEncoding ConvertToTransferEncoding(string TransferEncodingString) {
      ...
      switch (TransferEncodingString.Trim().ToLowerInvariant()) {
        case "7bit":
        case "8bit":
        // 09/29/2008 Paul.  Binary is a valid type and implies 8bit.  Import with no transfer encoding. 
        case "binary":
...
//          throw new Pop3Exception("SMPT does not support binary transfer encoding");
    ...
    private void saveMessageBody(RxMailMessage message, string contentString) {
      ...
      // 02/07/2010 Paul.  Defensive programming, the StreamWriter has a dispose interface, so use it. 
    ...
    private void SaveAttachment(RxMailMessage message) {
      if (message.Parent==null) {
        ...
        if (message.ContentDisposition!=null) {
          ...
          // 10/08/2008 Paul.  The date may be invalid and may throw an exception.  Just ignore the date error. 
          try
          {
		  ...
		  {
			// 06/12/2008 Paul.  Bad messages can have duplicate entries.  Protect against exception. 
			// This issue was discovered by Calvin when working with Exchange 2007. 
