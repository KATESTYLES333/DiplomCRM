...
namespace Spring.Social.LinkedIn.Api.Impl.Json
{
    ...
    class LinkedInFullProfilesDeserializer : PaginatedResultDeserializer
    {
        public override object Deserialize(JsonValue json, JsonMapper mapper)
        {
            ...
            LinkedInFullProfiles profiles = (LinkedInFullProfiles)base.Deserialize(peopleJson, mapper);
            profiles.Profiles = mapper.Deserialize<IList<LinkedInFullProfile>>(peopleJson);
            return profiles;
        }

        protected override PaginatedResult CreatePaginatedResult()
        {
            return new LinkedInFullProfiles();
        }
    }
}
