	...
	public class DynamicControl
	{
		...
		public bool Exists
		{
			get
			{
				...
				// 08/01/2001 Paul.  Allow TEAM_SET_LIST to also imply TEAM_SET_NAME. 
				if ( ctl == null && sNAME == "TEAM_SET_LIST" )
					ctl = ctlPARENT.FindControl("TEAM_SET_NAME");
				// 08/01/2001 Paul.  Allow KBTAG_SET_LIST to also imply KBTAG_NAME. 
		...
		public string Text
		{
			...
			set
			{
				...
				if ( ctl != null )
				{
					...
					*/
					// 04/14/2103 Paul.  If we are getting a text string from TeamSelect, then it should return the list. 
					else if ( ctl.GetType().BaseType == typeof(_controls.TeamSelect) )
					{
						_controls.TeamSelect ts = ctl as _controls.TeamSelect;
						ts.TEAM_SET_LIST = value;
					}
					// 04/14/2103 Paul.  If we are getting a text string from KBTagSelect, then it should return the list. 
					else if ( ctl.GetType().BaseType == typeof(_controls.KBTagSelect) )
					{
						_controls.KBTagSelect kbt = ctl as _controls.KBTagSelect;
