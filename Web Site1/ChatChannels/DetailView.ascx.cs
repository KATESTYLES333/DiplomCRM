	...
	public class DetailView : SplendidControl
	{
		...
		protected void Page_Command(Object sender, CommandEventArgs e)
		{
			...
			else if ( e.CommandName == "Delete" )
			{
				try
				{
					SqlProcs.spCHAT_CHANNELS_Delete(gID);
					SplendidCache.ClearChatChannels();
				}
				catch(Exception ex)
				{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			ctlDynamicButtons.Visible = (SplendidCRM.Security.GetUserAccess(m_sMODULE, "edit") >= 0);
			try
			{
					...
					if ( !Sql.IsEmptyGuid(gID) )
					{
						...
						using ( IDbConnection con = dbf.CreateConnection() )
						{
							...
							sSQL = "select *                   " + ControlChars.CrLf
							     + "  from vwCHAT_CHANNELS_Edit" + ControlChars.CrLf;
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 )
										{
											...
											sUSER_CHAT_CHANNELS = gID.ToString();
