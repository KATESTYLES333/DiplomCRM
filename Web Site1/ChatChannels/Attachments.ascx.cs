...
namespace SplendidCRM.ChatChannels
{
	...
	/// </summary>
	public class Attachments : SplendidControl
	{
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
				     + "  from vwCHAT_CHANNELS_ATTACHMENTS" + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwCHAT_CHANNELS_ATTACHMENTS", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							{
								da.Fill(dt);
								this.ApplyGridViewRules("ChatChannels." + m_sMODULE, dt);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !IsPostBack )
			{
				Guid gASSIGNED_USER_ID = Sql.ToGuid(Page.Items["CREATED_USER_ID"]);
		...
		private void InitializeComponent()
		{
			...
			arrSelectFields.Add("CREATED_USER_ID" );
			this.AppendGridColumns(grdMain, "ChatChannels." + m_sMODULE, arrSelectFields);
			if ( IsPostBack )
