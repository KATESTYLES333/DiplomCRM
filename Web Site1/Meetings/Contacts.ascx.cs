...
namespace SplendidCRM.Meetings
{
	...
	public class Contacts : SubPanelControl
	{
		...
		protected void Page_Command(object sender, CommandEventArgs e)
		{
			try
			{
				switch ( e.CommandName )
				{
					...
					//	// 05/31/2007 Paul.  The meeting will not be the parent of the contact, so pass a meeting variable. 
					//	Response.Redirect("~/Contacts/edit.aspx?MEETING_ID=" + gID.ToString());
					//	break;
					case "Contacts.Edit":
					{
					...
					{
						Guid gCONTACT_ID = Sql.ToGuid(e.CommandArgument);
						SqlProcs.spMEETINGS_CONTACTS_Delete(gID, gCONTACT_ID);
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
				     + "  from vwMEETINGS_CONTACTS" + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwMEETINGS_CONTACTS", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								this.ApplyGridViewRules("Meetings." + m_sMODULE, dt);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			// 05/30/2007 Paul.  Allow contact to be assigned to the meeting. This should have been done long ago. 
			Guid gCONTACT_ID = Sql.ToGuid(txtCONTACT_ID.Value);
			if ( !Sql.IsEmptyGuid(gCONTACT_ID) )
			{
				try
				{
					SqlProcs.spMEETINGS_CONTACTS_Update(gID, gCONTACT_ID, false, String.Empty);
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
			...
			if ( !IsPostBack )
			{
				...
				ctlDynamicButtons.AppendButtons("Meetings." + m_sMODULE, gASSIGNED_USER_ID, gID);
				// 02/21/2010 Paul.  The meeting needs to be initialized when the page first loads. 
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Meetings." + m_sMODULE, arrSelectFields);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Meetings." + m_sMODULE, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
