			...
			try
			{
				switch ( e.CommandName )
				{
					case "Users.Remove":
					{
						...
						SqlProcs.spMEETINGS_USERS_Delete(gID, gUSER_ID);
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
				     + "  from vwMEETINGS_USERS" + ControlChars.CrLf
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwMEETINGS_USERS", Sql.ClientScriptBlock(cmd));
