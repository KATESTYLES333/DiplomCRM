...
<SplendidCRM:ListHeader SubPanel="divMeetingsUsers" Title="Users.LBL_MODULE_NAME" Runat="Server" />
				...
				<ItemTemplate>
					<div style="DISPLAY: <%# Sql.ToGuid(DataBinder.Eval(Container.DataItem, "MEETING_ASSIGNED_USER_ID")) != Sql.ToGuid(DataBinder.Eval(Container.DataItem, "USER_ID")) ? "inline" : "none" %>">
						<span onclick="return confirm('<%= L10n.TermJavaScript("Meetings.NTC_REMOVE_INVITEE") %>')">
							<asp:ImageButton Visible='<%# SplendidCRM.Security.GetUserAccess("Meetings", "edit", Sql.ToGuid(DataBinder.Eval(Container.DataItem, "MEETING_ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Users.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "USER_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term(".LNK_REMOVE") %>' SkinID="delete_inline" Runat="server" />
							<asp:LinkButton  Visible='<%# SplendidCRM.Security.GetUserAccess("Meetings", "edit", Sql.ToGuid(DataBinder.Eval(Container.DataItem, "MEETING_ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Users.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "USER_ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" Text='<%# L10n.Term(".LNK_REMOVE") %>' Runat="server" />
						</span>
					</div>
				</ItemTemplate>
			</asp:TemplateColumn>
