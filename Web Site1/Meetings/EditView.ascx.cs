		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if ( !IsPostBack )
				{
					...
					BindSchedule();
					// 12/26/2012 Paul.  Add send invites button. 
					ctlDynamicButtons.EnableButton("Save.SendInvites", !Utils.IsOfflineClient && !Sql.IsEmptyString(Application["CONFIG.smtpserver"]) && !Sql.IsEmptyString(Application["CONFIG.smtpuser"]));
					ctlFooterButtons .EnableButton("Save.SendInvites", !Utils.IsOfflineClient && !Sql.IsEmptyString(Application["CONFIG.smtpserver"]) && !Sql.IsEmptyString(Application["CONFIG.smtpuser"]));
				}
				else
				{
