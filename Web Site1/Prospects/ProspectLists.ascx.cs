...
namespace SplendidCRM.Prospects
{
	...
	public class ProspectLists : SubPanelControl
	{
			...
			try
			{
				switch ( e.CommandName )
				{
					...
					case "ProspectLists.Remove":
					{
						...
						{
							SqlProcs.spPROSPECT_LISTS_PROSPECTS_Delete(gPROSPECT_LIST_ID, gID);
						}
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bEditView && IsPostBack && arrUPDATED.Count > 0 )
					{
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     ...
						     + "     , @PROSPECT_ID               as PROSPECT_ID              " + ControlChars.CrLf
						     + "     , @PROSPECT_NAME             as PROSPECT_NAME            " + ControlChars.CrLf
						     + "     , @PROSPECT_ASSIGNED_USER_ID as PROSPECT_ASSIGNED_USER_ID" + ControlChars.CrLf
						     + "  from vwPROSPECT_LISTS" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
						Sql.AddParameter(cmd, "@PROSPECT_ID"              , gID);
						Sql.AddParameter(cmd, "@PROSPECT_NAME"            , Sql.ToString(Page.Items["NAME"            ]));
					...
					else
					{
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "  from vwPROSPECTS_PROSPECT_LISTS" + ControlChars.CrLf
						     + " where 1 = 1                     " + ControlChars.CrLf;
						cmd.CommandText = sSQL;
					}
					Sql.AppendParameter(cmd, gID, "PROSPECT_ID");
					// 04/26/2008 Paul.  Move Last Sort to the database.
					cmd.CommandText += grdMain.OrderByClause("DATE_ENTERED", "desc");

					if ( bDebug )
						RegisterClientScriptBlock("vwPROSPECTS_PROSPECT_LISTS", Sql.ClientScriptBlock(cmd));
					...
					try
					{
						using ( DbDataAdapter da = dbf.CreateDataAdapter() )
						{
							...
							using ( DataTable dt = new DataTable() )
							{
								...
								this.ApplyGridViewRules("Prospects." + m_sMODULE, dt);
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spPROSPECT_LISTS_PROSPECTS_Delete(gDELETE_ID, gPARENT_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spPROSPECT_LISTS_PROSPECTS_Update(gUPDATE_ID, gPARENT_ID, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gPROSPECT_LIST_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spPROSPECT_LISTS_PROSPECTS_Update(gPROSPECT_LIST_ID, gID);
					}
					// 05/16/2008 Paul.  Instead of redirecting, just rebind the grid and AJAX will repaint. 
					//Response.Redirect("view.aspx?ID=" + gID.ToString());
			...
			if ( !IsPostBack )
			{
				...
				ctlDynamicButtons.AppendButtons("Prospects." + m_sMODULE, gASSIGNED_USER_ID, gID);
				// 10/21/2010 Paul.  Make sure to initialize the parent. 
				ctlNewRecord.PARENT_ID   = gID;
				ctlNewRecord.PARENT_TYPE = "Prospects";
			}
		}
		...
		private void InitializeComponent()
		{
			...
			this.AppendGridColumns(grdMain, "Prospects." + m_sMODULE, arrSelectFields);
			// 04/28/2008 Paul.  Make use of dynamic buttons. 
			if ( IsPostBack )
				ctlDynamicButtons.AppendButtons("Prospects." + m_sMODULE, Guid.Empty, Guid.Empty);
		}
		#endregion
	}
}
