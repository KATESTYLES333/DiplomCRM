...
{
	/// <summary>
	///		Summary description for SearchProspects.
	/// </summary>
	public class SearchProspects : SplendidControl
	{
		...
		public static string UnifiedSearch(string sUnifiedSearch, IDbCommand cmd)
		{
			...
			// 08/26/2010 Paul.  Allow custom fields in the Unified Search by pulling the fields out of the GridView. 
			UniqueStringCollection arrSearchFields = new UniqueStringCollection();
			SplendidDynamic.SearchGridColumns("Prospects.Search", arrSearchFields);
			...
			if ( !Sql.IsEmptyString(sUnifiedSearch.Trim()) )
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						cmd.CommandText += UnifiedSearch(sUnifiedSearch, cmd);
						// 08/09/2008 Paul.  Move Last Sort to the database.
						cmd.CommandText += grdMain.OrderByClause("NAME", "asc");

						if ( bDebug )
							RegisterClientScriptBlock("vwPROSPECTS_List", Sql.ClientScriptBlock(cmd));
