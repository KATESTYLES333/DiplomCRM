...
namespace SplendidCRM.Prospects
{
	...
	public class AutoComplete : System.Web.Services.WebService
	{
		...
		public Prospect PROSPECTS_PROSPECT_LAST_NAME_Get(string sLAST_NAME)
		{
			...
			//try
			{
				...
				if ( Sql.IsEmptyGuid(item.ID) )
				{
					...
					throw(new Exception(L10n.Term("Prospects.ERR_PROSPECT_NOT_FOUND")));
				}
			}
			//catch
			{
		...
		[WebMethod(EnableSession=true)]
		public string[] PROSPECTS_PROSPECT_LAST_NAME_List(string prefixText, int count)
		{
		...
		// so the field name can be NAME or PROSPECT_NAME. 
		[WebMethod(EnableSession=true)]
		public string[] PROSPECTS_LAST_NAME_List(string prefixText, int count)
		{
			return PROSPECTS_PROSPECT_LAST_NAME_List(prefixText, count);
		}
	}
}
