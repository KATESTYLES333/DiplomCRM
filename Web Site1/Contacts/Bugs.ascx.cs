...
namespace SplendidCRM.Contacts
{
	...
	public class Bugs : SubPanelControl
	{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					else
					{
						...
						sSQL = "select " + Sql.FormatSelectFields(arrSelectFields)
						     + "  from vwCONTACTS_BUGS" + ControlChars.CrLf;
						cmd.CommandText = sSQL;
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwCONTACTS_BUGS", Sql.ClientScriptBlock(cmd));
		...
		public override void Save(Guid gPARENT_ID, string sPARENT_TYPE, IDbTransaction trn)
		{
			...
			foreach ( Guid gDELETE_ID in arrDELETED )
			{
				if ( !Sql.IsEmptyGuid(gDELETE_ID) )
					SqlProcs.spCONTACTS_BUGS_Delete(gPARENT_ID, gDELETE_ID, trn);
			}
			...
			foreach ( Guid gUPDATE_ID in arrUPDATED )
			{
				if ( !Sql.IsEmptyGuid(gUPDATE_ID) )
					SqlProcs.spCONTACTS_BUGS_Update(gPARENT_ID, gUPDATE_ID, String.Empty, trn);
			}
		}
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !Sql.IsEmptyGuid(gBUG_ID) )
			{
				try
				{
					...
					{
						SqlProcs.spCONTACTS_BUGS_Update(gID, gBUG_ID, String.Empty);
					}
		...
		private void InitializeComponent()
		{
			...
			// 09/01/2009 Paul.  The CONTACT_ASSIGNED_USER_ID field is used in the table, so we need to make sure that it is included. 
