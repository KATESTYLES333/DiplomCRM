	...
	public class Activities : SplendidControl
	{
		...
		protected void BindGrid()
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				sSQL = "select distinct " + Sql.FormatSelectFields(arrSelectFields)
				     + "  from vwCONTACTS_ACTIVITIES_OPEN" + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwCONTACTS_ACTIVITIES_OPEN", Sql.ClientScriptBlock(cmd));
				...
				// 10/13/2012 Paul.  Create a separate activites view for the HTML5 Offline Client. 
				sSQL = "select distinct " + Sql.FormatSelectFields(arrSelectFields)
				     + "  from vwCONTACTS_ACTIVITIES_HISTORY" + ControlChars.CrLf;
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					cmd.CommandText = sSQL;
					// 11/27/2006 Paul.  Make sure to filter relationship data based on team access rights. 
					...
					if ( bDebug )
						RegisterClientScriptBlock("vwCONTACTS_ACTIVITIES_HISTORY", Sql.ClientScriptBlock(cmd));
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ( !IsPostBack )
			{
				...
				ctlNewRecordTask   .CONTACT_ID = gID;
				ctlNewRecordNote   .CONTACT_ID = gID;
