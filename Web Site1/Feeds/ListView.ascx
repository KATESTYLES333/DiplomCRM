...
<div id="divListView">
	...
	<SplendidCRM:ModuleHeader ID="ctlModuleHeader" Module="Feeds" Title=".moduleList.Home" EnablePrint="true" HelpName="index" EnableHelp="true" Runat="Server" />
	...
	<SplendidCRM:SplendidGrid id="grdMain" SkinID="grdListView" AllowPaging="<%# !PrintView %>" EnableViewState="true" runat="server">
		<Columns>
			...
			<asp:TemplateColumn ItemStyle-Width="60%">
				<ItemTemplate>
					<div style="DISPLAY: <%# Sql.IsEmptyGuid(DataBinder.Eval(Container.DataItem, "USERS_FEED_ID")) ? "inline" : "none" %>">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetUserAccess("Feeds", "edit", Sql.ToGuid(DataBinder.Eval(Container.DataItem, "ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Feeds.Add"    CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term("Feeds.LBL_ADD_FAV_BUTTON_LABEL") %>' SkinID="plus_inline" Runat="server" />
					...
					<div style="DISPLAY: <%# !Sql.IsEmptyGuid(DataBinder.Eval(Container.DataItem, "USERS_FEED_ID")) ? "inline" : "none" %>">
						<asp:ImageButton Visible='<%# SplendidCRM.Security.GetUserAccess("Feeds", "edit", Sql.ToGuid(DataBinder.Eval(Container.DataItem, "ASSIGNED_USER_ID"))) >= 0 %>' CommandName="Feeds.Remove" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "ID") %>' OnCommand="Page_Command" CssClass="listViewTdToolsS1" AlternateText='<%# L10n.Term("Feeds.LBL_DELETE_FAV_BUTTON_LABEL") %>' SkinID="minus_inline" Runat="server" />
